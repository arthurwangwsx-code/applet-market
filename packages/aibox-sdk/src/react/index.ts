...
export {
  createDragGesture, createLongPress, createSwipePager, defaultSwipeThreshold,
  useDragGesture, useLongPress, useSwipePager,
} from './gestures';
export type {
  DragAxis, DragGestureCore, DragGestureHandlers, DragGestureOptions, DragLockMode, DragSample,
  LongPressCore, LongPressHandlers, LongPressOptions,
  SwipePagerBinding, SwipePagerCore, SwipePagerCoreOptions, SwipePagerView, SwipeThreshold,
  TouchEventLike, TouchPointLike, UseDragGestureResult, UseSwipePagerOptions,
} from './gestures';

/** 订阅一个 `aibox.<ns>.on(event, …)`，组件卸载自动退订。 */
function useBridgeEvent(
  namespace: string,
  event: string,
  ...
  useEffect(() => {
    ...
    return () => {
      if (typeof unsubscribe === 'function') {
        try { (unsubscribe as () => void)(); } catch { /* 退订失败不该影响卸载 */ }
      }
    };
...
export function useTabs(): {
  ...
  useEffect(() => {
    ...
    host.tabs.getState().then((next: TabsState) => { if (!cancelled) setState(next); }).catch(() => undefined);
...
export function useKeyboardInset(): { height: number; animationMs: number; source: 'host' | 'viewport' } {
  ...
  const [inset, setInset] = useState<{ height: number; animationMs: number; source: 'host' | 'viewport' }>(
    () => ({ height: measure(), animationMs: 250, source: 'viewport' }));
...
export function useScene(): SceneState | null {
  ...
  useEffect(() => {
    ...
    host.scene.getState().then((next: SceneState) => { if (!cancelled) setState(next); }).catch(() => undefined);
    return () => { cancelled = true; };
  }, []);
