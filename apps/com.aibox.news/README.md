...
- **顶栏四态互斥**：`showBroadcast = (tab === 'feed') && 有TTS && 时间线非空`；
  ✨ 走 `toolbar.invoke`，⋯ 用 `role: hostMenu`，「朗读 / 停止朗读」经 `manifest.actions` +
  `scene.menu` 挂进宿主 ⋯ 菜单，标题随播报状态实时切换。
...
- `inputSchemaJSON` / `outputSchemaJSON` 都写全（含 enum 与 min/max），AI 不用猜参数；
...
而 `aibox.net.fetch` 的返回体是 UTF-8 字符串，二进制图片无法经它取回转 data: URL。
...
- **响应体上限 200KB**（桥的硬限制）。超长 feed 会被截断成不闭合的 XML，
  `feedParser` 会剪到最后一条完整的 `</item>` / `</entry>` 再补闭合标签重解析，
  所以是「少几条」而不是「整份丢失」。原生没有这个限制。
...
三个新平台能力（`aibox.tabs` / `aibox.toolbar` / `aibox.browser`）正在并行落地。
本应用**全部先探测再使用**，缺席时不留死按钮：

| 缺席的能力 | 降级 |
|---|---|
| `aibox.tabs`（或 `rendered: false`，如 card/sheet 形态） | 自绘悬浮胶囊底栏 |
| `aibox.toolbar` | 自绘顶栏（标题 + ✨ + 朗读按钮） |
| `toolbar.search`（如 fullscreen 无导航栏） | 自绘搜索框 |
| `aibox.browser` | 依次退 `browser.open` → `aibox.open.url` |
| `aibox.tts` | 播报入口整块不渲染 |
...
5. **Tab 图标能否原样渲染**。`manifest.icon = newspaper.fill` 在宿主的
   `AiBoxIconCatalog`（`.applet` 上下文）策展白名单里，装出来正确；但
   `scene.tabBar` 的 `newspaper` / `dot.radiowaves.up.forward` / `bookmark` 与顶栏的
   `sparkles` / `ellipsis` **不在**那份表里。若宿主对外壳图标也套 `.applet` 白名单，
   底栏会静默落成拼图块。已知会当面提醒外壳实现方，真机需复核。
   `toolbar.update` 是否接受 `hidden`（本应用用它做右上角四态互斥）同样待验，
   不接受时退回 `enabled`。
