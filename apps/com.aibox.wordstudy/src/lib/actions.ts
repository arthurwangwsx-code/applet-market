...
export function registerWordActions(refresh: () => void): void {
  registerActions({
    ...
    async word_translate(input) {
      ...
      const direction = (text(input?.direction) || 'auto') as TranslateDirection
      ...
      try {
        ...
        await saveTranslation({
          ...
          source,
          target: translated,
          srcLang: from as LangCode,
          dstLang: to as LangCode,
          at: Date.now(),
          starred: false,
        })
        refresh()
