...
import type { Currency, FinanceContext, PositionValuation, Translate } from '../lib/types.js'
...
function PositionRow({
  ...
  row: PositionValuation & { accountCurrency: Currency }
  t: Translate
  upIsRed: boolean
...
export default function PortfolioPage({ ctx }: { ctx: FinanceContext }) {
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: SPACE.s4, padding: SPACE.s4 }}>
        ...
        <Card title={t('finance.perf.title')}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: SPACE.s3 }}>
            <Stat
              label={t('finance.perf.annualized')}
              value={formatPercent(performance.annualized)}
              color={trendColor(performance.annualized, upIsRed)}
            />
            <Stat label={t('finance.perf.maxdd')} value={formatPercent(performance.maxDrawdown, false)} />
            <Stat label={t('finance.perf.sharpe')} value={formatPrice(performance.sharpe, 2)} />
            <Stat label={t('finance.perf.vol')} value={formatPercent(performance.volatility, false)} />
            <Stat label={t('finance.perf.winrate')} value={formatPercent(performance.winRate, false)} />
            <Stat label={t('finance.perf.trades')} value={performance.closed} />
          </div>
          ...
          <Card title={t('finance.attr.title')}>
            {contributor ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: SPACE.s2, padding: '4px 0' }}>
                <span className="fin-clamp-1" style={{ fontSize: 15, flex: '1 1 auto' }}>
                  {contributor.position.name}
                </span>
          ...
          {!diagnosis.isComplete ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: SPACE.s3 }}>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
                <span className="fin-mono" style={{ fontSize: 34, fontWeight: 500, color: C.brand }}>
                  {diagnosis.score}
                </span>
              ...
              {diagnosis.flags.length === 0 ? (
                ...
                diagnosis.flags.map((flag) => (
                  <span
                    key={flag}
                    style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: C.warning }}
                  >
          ...
          >
            {orders.map((order) => (
              <div key={order.id} style={{ display: 'flex', alignItems: 'center', gap: SPACE.s2, padding: '6px 0' }}>
                ...
                <span className="fin-clamp-1" style={{ fontSize: 14, flex: '1 1 auto' }}>
                  {order.name}
                </span>
                <span className="fin-mono" style={{ fontSize: 13, color: C.muted }}>
