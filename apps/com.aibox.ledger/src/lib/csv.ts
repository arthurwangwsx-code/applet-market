...
import type { AccountKind, FlowKind, LedgerTransaction, TransactionKind } from '../types.js'
...
export const CSV_ERRORS = {
  ...
  columns: (found: number) => `The CSV columns do not match the AiBox Ledger export format (found ${found} columns).`,
  rowColumns: (expected: number, found: number) => `Expected ${expected} columns but found ${found}.`,
...
export function exportCSV(store: LedgerStore): string {
...
const KIND_VALUES = new Set<string>(Object.values(KIND))

/**
 * 解析成 draft（有效行 + 问题列表），**不写任何数据**。
 * 返回 `{ rows, problems, columns }`；`rows` 是待落库的中间结构。
 */
export function parseImport(text: string, store: LedgerStore): CSVImportDraft {
  const problems: CSVProblem[] = []
  const table = parseCSV(text)
  if (table.length === 0) return { rows: [], problems: [{ row: 0, message: CSV_ERRORS.empty }] }

  const header = (table[0] ?? []).map((name) => String(name ?? '').trim())
  let columns: string[] | null = null
  if (header.length === CSV_COLUMNS.length) columns = CSV_COLUMNS
  else if (header.length === CSV_COLUMNS_V1.length) columns = CSV_COLUMNS_V1
  if (!columns) return { rows: [], problems: [{ row: 1, message: CSV_ERRORS.columns(header.length) }] }

  const index: Record<string, number> = Object.fromEntries(columns.map((name, position) => [name, position]))
  ...
  for (let line = 1; line < table.length; line += 1) {
    ...
    rows.push({
      ...
      baseAmountMinorAtPosting:
        get('base_amount_minor_at_posting') === '' ? null : Number.parseInt(get('base_amount_minor_at_posting'), 10),
      baseCurrencyAtPosting: get('base_currency_at_posting') || null,
      fxRateToBaseAtPosting:
        get('fx_rate_to_base_at_posting') === '' ? null : Number(get('fx_rate_to_base_at_posting')),
      occurredOn,
...
function findCategoryByPath(store: LedgerStore, path: string): CategoryPathResult {
  ...
  if (!childName) return { ok: true, id: root ? root.id : null, missingRoot: root ? null : rootName }
  if (!root) return { ok: true, id: null, missingRoot: rootName, missingChild: childName }
  const children = store.childCategories(root.id).filter((row) => row.name === childName)
  if (children.length > 1) return { ok: false }
  if (children.length === 1 && children[0]) return { ok: true, id: children[0].id }
 ...
 */
export async function performImport(store: LedgerStore, rows: readonly CSVImportRow[]) {
  const batchID = newID()
  ...
  let skipped = 0
  let failed = 0
  const staged: Array<{ txn: LedgerTransaction; row: CSVImportRow }> = []
  ...
  for (const row of rows) {
    ...
    let account =
      store.accounts.find((entry) => entry.name === row.accountName && entry.currency === row.currency) ?? null
    if (!account) account = store.accounts.find((entry) => entry.name === row.accountName) ?? null
    ...
    if (!account) account = store.defaultAccount()
    if (!account) {
      failed += 1
      continue
    }
    ...
    if (txn.baseAmountMinorAtPosting === null) store.applyPostingSnapshot(txn)
    staged.push({ txn, row })
    if (row.originalID) idMap.set(row.originalID, txn.id)
  }

  if (staged.length === 0) return { imported: 0, skipped, failed }

  // ② 用「原 id → 新 id」映射回填 transferPeerID / refundOfID，再一次性落库。
  for (const { txn, row } of staged) {
    if (row.transferPeerID) txn.transferPeerID = idMap.get(row.transferPeerID) ?? null
    if (row.refundOfID) {
      txn.refundOfID =
  ...
  const ok = await store.mutate((draft) => {
    for (const { txn } of staged) draft.putTx(txn)
  })
  if (!ok) return { imported: 0, skipped, failed: failed + staged.length }
...
async function ensureCategoryPath(
  ...
  if (!root) {
    ...
    if (!created.ok || !created.category) return { ok: false }
    root = created.category
  }
  if (!root) return { ok: false }
  if (parts.length === 1) return { ok: true, id: root.id }
...
const ACCOUNT_KINDS = new Set<string>(['cash', 'debit', 'credit', 'ewallet', 'prepaid', 'investment'])

function normalizeAccountKind(value: string): AccountKind {
  return ACCOUNT_KINDS.has(value) ? (value as AccountKind) : 'cash'
}
