...
/** 是否计入收支报表：**转账两腿与校准都不计**。 */
export function countsInFlow(txn: LedgerTransaction) {
  return txn.kind === KIND.expense || txn.kind === KIND.income
}

/** 只对账户余额有意义的有符号金额。 */
export function signedAmountMinor(txn: LedgerTransaction) {
  switch (txn.kind) {
    case KIND.expense:
...
export class Draft {
  ...
  readonly tables: Partial<LedgerTables>
  readonly months: Record<number, LedgerTransaction[]>
  readonly txMonth: Map<string, number | null>
  readonly dirtyTables: Set<TableName>
  readonly dirtyMonths: Set<number>
  ...
  /** 可写取表（首次访问时浅拷贝并标脏）。 */
  table<K extends TableName>(name: K): LedgerTables[K] {
    if (!(name in this.tables)) {
      this.tables[name] = this.store.tables[name].slice() as LedgerTables[K]
      this.dirtyTables.add(name)
    }
    return this.tables[name] as LedgerTables[K]
  }

  /** 只读取表（不标脏）。 */
  read<K extends TableName>(name: K): LedgerTables[K] {
    return (name in this.tables ? this.tables[name] : this.store.tables[name]) as LedgerTables[K]
  }

  month(key: number): LedgerTransaction[] {
  ...
  monthOf(id: string): number | null {
    if (this.txMonth.has(id)) return this.txMonth.get(id) ?? null
    return this.store.txMonth.get(id) ?? null
  }

  /** 读一笔流水（Draft 覆盖优先）。 */
  tx(id: string): LedgerTransaction | null {
  ...
  /** 整条 put（自动处理跨月搬家）。 */
  putTx(txn: LedgerTransaction): LedgerTransaction {
  ...
  operations(): DatabaseOperation[] {
    const ops: DatabaseOperation[] = []
    for (const name of this.dirtyTables) {
      const rows = this.tables[name]
      if (rows) ops.push({ c: 'tables', id: name, rows })
    }
    for (const key of this.dirtyMonths) {
      const rows = this.months[key] ?? []
      if (rows.length === 0) ops.push({ c: 'tx', id: txDocID(key), del: true })
      else ops.push({ c: 'tx', id: txDocID(key), rows })
    }
    return ops
  }
}
...
export class LedgerStore {
  ...
  problems: Array<{ reason: string; at: number }>
  revision: number
  locale: string
  tables: LedgerTables
  months: Record<number, LedgerTransaction[]>
  txMonth: Map<string, number>
  listeners: Set<() => void>
  private _allCache: LedgerTransaction[] | null
  private _allCacheRevision: number
  ...
  constructor() {
    ...
    this.revision = 0
    this.locale = 'en'
    this.tables = Object.fromEntries(TABLE_NAMES.map((name) => [name, []])) as unknown as LedgerTables
    this.months = {}
  ...
  recordProblem(reason: unknown): void {
  ...
  async open(locale: string): Promise<void> {
    ...
    try {
      ...
      for (const name of TABLE_NAMES) replaceTable(this.tables, name, tables[name])
      this.months = {}
      this.txMonth = new Map()
      for (const [key, rows] of Object.entries(tx)) {
        const monthKey = Number(key)
        this.months[monthKey] = rows
        for (const row of rows) this.txMonth.set(row.id, monthKey)
      }
      this.state = 'ready'
      this.stateReason = ''
   ...
   */
  async mutate(fn: (draft: Draft) => void | boolean): Promise<boolean> {
    ...
    for (const name of draft.dirtyTables) {
      ...
      if (rows) replaceTable(this.tables, name, rows)
    }
    for (const key of draft.dirtyMonths) {
      const rows = draft.months[key] ?? []
      if (rows.length === 0) delete this.months[key]
      else this.months[key] = rows
    }
    for (const [id, key] of draft.txMonth) {
      if (key === null) this.txMonth.delete(id)
      else this.txMonth.set(id, key)
    }
    this.lastMutationSucceeded = true
    this._allCache = null
    this.emit()
  ...
  async seedIfNeeded() {
    ...
    if (!needCategories && !needCurrencies && !needAccounts) return
      ...
      if (needAccounts) {
        ...
        for (const seed of ACCOUNT_SEEDS) {
          rows.push({
            id: newID(),
            name: seedName(seed, locale),
            kind: seed.kind as Account['kind'],
            currency: 'CNY',
  ...
  allTransactions(): LedgerTransaction[] {
    if (this._allCache && this._allCacheRevision === this.revision) return this._allCache
    const rows: LedgerTransaction[] = []
    for (const key of Object.keys(this.months).map(Number)) {
      for (const row of this.months[key] ?? []) if (!row.deletedAt) rows.push(row)
    }
  ...
  /** 含软删的全量（最近删除页用）。 */
  allTransactionsIncludingDeleted(): LedgerTransaction[] {
    const rows: LedgerTransaction[] = []
    for (const key of Object.keys(this.months).map(Number)) rows.push(...(this.months[key] ?? []))
    return rows
  }

  /** 指定月份的流水（未软删）。 */
  monthTransactions(monthKey: number): LedgerTransaction[] {
    return (this.months[monthKey] ?? []).filter((row) => !row.deletedAt)
  }

  /** `from` 之后（含）的全部未软删流水，倒序。 */
  transactionsSince(from: number): LedgerTransaction[] {
    return this.allTransactions().filter((row) => row.occurredOn >= from)
  }

  transaction(id: string): LedgerTransaction | null {
    const key = this.txMonth.get(id)
    if (key === null || key === undefined) return null
    return (this.months[key] ?? []).find((row) => row.id === id) ?? null
  }

  account(id: string | null): Account | null {
  ...
  /** 一级归并：二级返回其父 id。 */
  rootCategoryID(id: string | null): string | null {
    const row = this.category(id)
    if (!row) return null
  ...
  currencyRow(code: string): CurrencyRow | null {
    return currencyRow(this.currencies, code)
  }
  hasUsableRate(code: string): boolean {
    return hasUsableRate(this.currencies, code)
  }
  toBaseMinor(minor: number, code: string): number {
  ...
  reportingBaseMinor(txn: LedgerTransaction): number {
    return reportingBaseMinor(this.currencies, txn)
  }
  applyPostingSnapshot(txn: LedgerTransaction): LedgerTransaction {
    return applyPostingSnapshot(this.currencies, txn)
  }

  /** 新建一条空白流水骨架。 */
  makeTransaction(input: TransactionInput): LedgerTransaction {
    const now = Date.now()
    return {
      id: input.id ?? newID(),
...
/** 标签：去空白、丢空串。 */
export function normalizeTags(value: readonly unknown[] | null | undefined): string[] {
...
function replaceTable<K extends TableName>(tables: LedgerTables, name: K, rows: LedgerTables[K]): void {
  tables[name] = rows
}
