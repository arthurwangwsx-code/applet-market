...
import type { FetchRequest, InspectResult, VideoFormat, VideoInspection } from '../types.js'
...
export default function InspectSheet({
  ...
  open: boolean
  onClose: () => void
  onInspect: (url: string) => Promise<InspectResult>
  onDownload: (request: FetchRequest) => Promise<void>
  onPaste?: (() => Promise<string>) | null
}) {
  const [url, setUrl] = React.useState('')
  const [busy, setBusy] = React.useState(false)
  const [info, setInfo] = React.useState<VideoInspection | null>(null)
  const [formats, setFormats] = React.useState<VideoFormat[]>([])
      ...
      {info ? (
        <div style={{ marginTop: SPACE.s4 }}>
          ...
          <div style={{ marginTop: SPACE.s4, display: 'flex', gap: SPACE.s2, alignItems: 'center' }}>
            <Chip active={!audioOnly} onClick={() => setAudioOnly(false)}>
              视频
            </Chip>
            <Chip active={audioOnly} onClick={() => setAudioOnly(true)}>
              只要音频
            </Chip>
          </div>
          {audioOnly ? (
          ...
          {info.subtitles && info.subtitles.length ? (
            <div style={{ marginTop: SPACE.s2, fontSize: 12, color: C.muted }}>字幕：{info.subtitles.join('、')}</div>
          ) : null}
        </div>
      ) : null}
    </Sheet>
