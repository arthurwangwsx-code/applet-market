...
import type { DownloadQueueItem, ToolBlockVerdict, ToolCallResult } from '../types.js'
 ...
 */
export async function toolBlockReason(name: string): Promise<ToolBlockVerdict> {
  const api = bridge()
...
export async function callTool<T = unknown>(
  ...
  return result.ok ? (result as ToolCallResult<T>) : { ok: false, error: result.text, text: result.text }
}

export const queue = {
  /** 视频轨道与 HLS 离线包都在这里。 */
  async list(): Promise<DownloadQueueItem[]> {
    const api = bridge()
    if (!capabilities.download || !api?.download) return []
    try {
  ...
  async subscribe() {
    const api = bridge()
    if (!capabilities.download || !api?.download) return false
    try {
...
export function tap(style: 'light' | 'medium' | 'heavy' | 'soft' | 'rigid' = 'light'): void {
  const api = bridge()
  if (!capabilities.haptics || !api?.haptics) return
  try {
