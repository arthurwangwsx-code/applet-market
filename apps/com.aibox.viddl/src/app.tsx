...
import type { TabsState } from '@aibox/applet-sdk'
...
import type { DownloadDetail, DownloadJob, FetchRequest, LibraryActionName, LibraryResult } from './types.js'
...
function useJobs() {
  ...
  const [bytes, setBytes] = React.useState<Record<string, DownloadDetail>>({})
   ...
   */
  const playJob = React.useCallback(
    async (job: DownloadJob): Promise<LibraryResult | void> => {
  ...
  const refreshBytes = React.useCallback(async () => {
    ...
    const map: Record<string, DownloadDetail> = {}
...
export default function App() {
  ...
  const [notice, setNotice] = React.useState<{ tone: 'info' | 'error' | 'success'; text: string } | null>(null)
  ...
  const act = React.useCallback(
    async (action: LibraryActionName, jobId: string) => {
      tap('light')
  ...
  // —— 宿主外壳接线 ——
  const addRef = React.useRef<(() => void) | null>(null)
  ...
  React.useEffect(() => {
    ...
    const wire = async () => {
      ...
      if (api && api.tabs && typeof api.tabs.getState === 'function') {
        ...
        offs.push(
          onNamespaceEvent('tabs', 'changed', (payload) => {
            const state = payload as TabsState | null
            if (!state) return
      ...
      if (api && api.toolbar && typeof api.toolbar.getState === 'function') {
        ...
        offs.push(
          onNamespaceEvent('toolbar', 'invoke', (raw) => {
            const payload = raw as { id?: string } | null
            if (payload?.id === 'add' && addRef.current) addRef.current()
          }),
        )
      }
      offs.push(
        onEvent('lifecycle.foreground', () => {
          refresh()
        }),
      )
    }
              ...
              {visible.map((job, index) => (
                <div key={job.jobId} style={index ? { borderTop: `1px solid ${C.line}` } : undefined}>
                  <JobRow
                    ...
                    onPlay={(j) => {
                      playJob(j).then((r) => {
                        if (r && r.ok === false) setNotice({ tone: 'error', text: r.text || '播放失败' })
                      })
                    }}
