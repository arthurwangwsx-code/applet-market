...
export default function SearchPage({ onOpen }: { onOpen: (video: VideoSummary) => void }) {
  ...
  React.useEffect(() => {
    loadPref<string[]>(RECENT_KEY, []).then(setRecent)
  ...
  function Chips({ title, values, onPick }: ChipsProps) {
  ...
  if (state === 'loading') body = <Spinner label="搜索中" />
    ...
    body = (
      <div style={{ paddingTop: SPACE.s2 }}>
        ...
        {history.length ? (
          <div>
            <div style={{ padding: `0 ${SPACE.s4}px ${SPACE.s2}px`, fontSize: 13, color: C.sub }}>最近观看</div>
