...
import type { FinSymbol } from '../types.js'
...
async function rows(url: string, headers: Record<string, string>): Promise<Array<Record<string, unknown>>> {
  const result = await getJSON<DatacenterResponse>(url, { headers })
  if (!result.ok) return []
...
export async function fetchFinancials(symbol: FinSymbol) {
...
export async function fetchDividends(symbol: FinSymbol) {
  if (symbol.market !== 'ashare') return []
  const url =
...
export async function fetchDragonBoard(limit = 40) {
  ...
  for (const row of data) {
    if (str(row.TRADE_DATE).slice(0, 10) !== latest) continue
...
export async function fetchAnnouncements(symbol: FinSymbol, limit = 20) {
  if (symbol.market !== 'ashare') return []
  const url =
    'https://np-anotice-stock.eastmoney.com/api/security/ann' +
    `?sr=-1&page_size=${Math.max(1, Math.min(50, limit))}&page_index=1&ann_type=A&client_source=web&stock_list=${symbol.code}`
  const result = await getJSON<AnnouncementsResponse>(url, { headers: DATA })
  if (!result.ok) return []
