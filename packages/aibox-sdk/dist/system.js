...
export async function openInBrowser(url, options = {}) {
    ...
    const result = await withTimeout(host.browser.open(options.mode ? { url, mode: options.mode } : { url }), options.timeoutMs ?? 12_000, null);
    return result ? result.opened : false;
}
/** 以阅读器形态打开一篇文章；宿主没有阅读器时退回普通打开。 */
export async function openArticle(article, options = {}) {
    const host = bridge();
...
export async function hasSession(host_) {
    const host = bridge();
...
/** 清除会话；返回清掉几条。 */
export async function clearSession(host_) {
    const host = bridge();
