...
export type FailureKind = (typeof FAILURE)[keyof typeof FAILURE]
export type ResponseType = 'text' | 'base64' | 'json'
...
function classify(message: unknown): FailureKind {
 ...
 */
export async function request(url: string, options: RequestOptions = {}): Promise<HTTPResult<unknown>> {
  const api = bridge()
  ...
  try {
    const payload: aibox.net.FetchOptions = {
      method,
      headers: { 'User-Agent': USER_AGENT, ...headers },
      responseType,
    }
    if (maxBytes) payload.maxBytes = maxBytes
    if (body !== undefined) payload.body = body
    const response = await Promise.race([api.net.fetch(url, payload), timeout])
    if ('__timeout' in response) return { ok: false, failure: FAILURE.timeout }
    const status = Number(response.status || 0)
    if (status < 200 || status >= 300) return { ok: false, failure: FAILURE.http, status }
    return {
      ok: true,
      status,
      body: response.body,
...
export async function getText(url: string, options: RequestOptions = {}): Promise<HTTPResult<string>> {
...
export async function getJSON<T = unknown>(url: string, options: RequestOptions = {}): Promise<HTTPResult<T>> {
  const result = await request(url, { ...options, responseType: 'json' })
  if (!result.ok) return result
  if (result.body && typeof result.body === 'object') return { ...result, body: result.body as T }
  try {
    return { ...result, body: JSON.parse(String(result.body || '')) as T }
  } catch (error) {
 ...
 * 必须走 base64 —— 见文件头注释。
 */
export async function getGBK(url: string, options: RequestOptions = {}): Promise<HTTPResult<string>> {
  ...
  if (text === null) return { ok: false, failure: FAILURE.decoding }
  return { ...result, body: text }
}

/** base64 → GB18030 文本。解不出来返回 null（调用方按失败处理，不伪造数据）。 */
export function decodeGBK(base64: unknown): string | null {
  try {
...
export function coalesce<T>(key: string, factory: () => T | PromiseLike<T>): Promise<T> {
...
export function unescapeUnicode(text: unknown): string {
