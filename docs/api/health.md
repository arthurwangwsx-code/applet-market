...
> Read Apple Health metrics, motion activity and workouts through the host health tools.

**分组** 系统能力投影 ｜ **方法数** 4 ｜ **声明要求** 需要在 `manifest.permissions.capabilities` 里声明 `"health"`。**声明 ≠ 授权**，用户仍会被逐项询问。
...
| 方法 | 背后的宿主工具 | 档位 |
|---|---|---|
| `metric` | `health_metric` | `read` |
| `summary` | `health_summary` | `read` |
| `activity` | `media_activity` | `read` |
| `workouts` | `media_workouts` | `read` |

### `aibox.health.metric()`

投影自宿主工具 `health_metric`；摘要与参数以该工具的真实定义为准。

**副作用档位** `read`（读取）— 只读，不改任何状态；不触发用户确认。

**参数**

参数取自宿主工具 `health_metric` 的真实 `ToolDefinition.parametersJSON`，随宿主版本变化。
...
```js
await aibox.health.metric({ metric: 'steps', range: 'today' })
```

### `aibox.health.summary()`

投影自宿主工具 `health_summary`；摘要与参数以该工具的真实定义为准。

**副作用档位** `read`（读取）— 只读，不改任何状态；不触发用户确认。

**参数**

参数取自宿主工具 `health_summary` 的真实 `ToolDefinition.parametersJSON`，随宿主版本变化。
写代码前用 `await aibox.capabilities()` 或 `applet_read action=capabilities` 读当前 schema。

**返回** `{ok, text, permission, details?, progress, artifacts}`

```js
await aibox.health.summary({ range: 'today' })
```

### `aibox.health.activity()`

投影自宿主工具 `media_activity`；摘要与参数以该工具的真实定义为准。

**副作用档位** `read`（读取）— 只读，不改任何状态；不触发用户确认。

**参数**

参数取自宿主工具 `media_activity` 的真实 `ToolDefinition.parametersJSON`，随宿主版本变化。
写代码前用 `await aibox.capabilities()` 或 `applet_read action=capabilities` 读当前 schema。

**返回** `{ok, text, permission, details?, progress, artifacts}`

### `aibox.health.workouts()`

投影自宿主工具 `media_workouts`；摘要与参数以该工具的真实定义为准。

**副作用档位** `read`（读取）— 只读，不改任何状态；不触发用户确认。

**参数**

参数取自宿主工具 `media_workouts` 的真实 `ToolDefinition.parametersJSON`，随宿主版本变化。
写代码前用 `await aibox.capabilities()` 或 `applet_read action=capabilities` 读当前 schema。
...
- **宿主变体**：Full ✅ ／ Lean ⚠️ 取决于壳是否装了健康模块
- **manifest 声明**：需声明；且**宿主必须真的装了对应模块**——工具不在场时整条命名空间不注册。
- **iOS 系统授权**：需要用户在系统弹窗里逐项授权健康数据读取；**模拟器上没有健康数据**。
