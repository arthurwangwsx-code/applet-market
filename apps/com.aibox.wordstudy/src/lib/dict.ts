...
export function parseYoudao(html: string, word: string): WordLookupPayload | null {
  ...
  const forms: WordLookupPayload['forms'] = []
  ...
  const examples: WordLookupPayload['examples'] = []
  doc.querySelectorAll('.sents_con .mcols').forEach((node) => {
    if (examples.length >= 3) return // 最多取 3 条
...
async function scrapeYoudao(word: string): Promise<WordLookupPayload | null> {
  let url: string
  try {
...
export function lenientJSON<T>(raw: string): T | null {
...
function lookupPrompt(word: string): string {
...
function array<T>(value: unknown, map: (item: Record<string, unknown>) => T | null): T[] {
...
function ai(): typeof aibox.ai | undefined {
 ...
 */
export async function lookupWord(word: string): Promise<WordLookupPayload> {
  ...
  const parsed = lenientJSON<Record<string, unknown>>(raw)
...
export async function translateStream(input: {
  ...
  if (typeof api.generateStream === 'function') {
    ...
    if (!received && full.trim()) input.onDelta(full)
    return full
  }
...
function dailyPrompt(dateKey: string): string {
...
export async function generateDaily(dateKey: string): Promise<{ en: string; zh: string; author: string } | null> {
  const api = ai()
  if (!api) return null
  try {
