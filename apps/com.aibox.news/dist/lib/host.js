...
// 不是桥胶水，不进 SDK。
import { bridge, events, system, intelligence } from 'aibox/sdk';
// MARK: - net（原生代理）
...
function classifyError(message) {
    ...
    if (text.includes('declined network access'))
        return FAILURE.permission;
    if (text.includes('networkallowed'))
...
const failedHTTP = (failure, httpStatus = null, legacyBridge = false) => ({
    ok: false,
    body: '',
    status: httpStatus ?? 0,
    truncated: false,
...
export async function httpGet(url, { timeoutMs = HTTP_TIMEOUT_MS, maxBytes } = {}) {
    ...
    const attempt = () => {
        ...
        const raced = Promise.race([call, timeout]).then((value) => {
            clear();
            return value;
        }, (error) => {
            clear();
    ...
    try {
        ...
        if (response && typeof response === 'object' && '__timeout' in response)
            return failedHTTP(FAILURE.timeout);
