#!/usr/bin/env node
// 用**运行时同一份 Sucrase** 转译 com.aibox.ledger 的每个源文件，再用桩 react 真正 import 一遍。
// 目的：在提交前抓住「转译期不报错、运行时炸掉整个模块 → 白屏」的那类问题。
//
//   node apps/com.aibox.ledger/tests/jsx-check.mjs
...
const OUT = path.join(process.env.SCRATCH ?? '/tmp', 'ledger-jsx-check')
