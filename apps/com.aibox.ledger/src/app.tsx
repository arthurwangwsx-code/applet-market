...
type TabID = 'transactions' | 'reports' | 'accounts' | 'budget' | 'projects'
type Route = { name: 'account' | 'project'; id: string }
const TABS = [
...
const FAB_LABEL: Record<TabID, string> = {
...
export default function App() {
  ...
  const storeRef = React.useRef<LedgerStore>(new LedgerStore())
  const store = storeRef.current

  const [ready, setReady] = React.useState(false)
  const [tab, setTab] = React.useState<TabID>('transactions')
  ...
  const [sheet, setSheet] = React.useState<LedgerSheetState | null>(null)
  const [menuItems, setMenuItems] = React.useState<MenuItem[] | null>(null)
  const [undo, setUndo] = React.useState<{ id: string } | null>(null)
  ...
  const submitRef = React.useRef<() => { valid: boolean }>(() => ({ valid: false }))
  ...
  const contextRef = React.useRef<LedgerActionContext>({ store, locale, labels })
  ...
  React.useEffect(() => {
    ...
    const wire = async () => {
      ...
      if (api && api.tabs && typeof api.tabs.getState === 'function') {
        ...
        offs.push(
          onNamespaceEvent('tabs', 'changed', (state: { rendered?: boolean; selected?: string }) => {
            if (!state) return
      ...
      if (api && api.toolbar && typeof api.toolbar.getState === 'function') {
        ...
        offs.push(
          onNamespaceEvent('toolbar', 'searchChanged', (payload: { query?: string }) => {
  ...
  const actions = React.useMemo<LedgerUIActions>(
    () => ({
      setQuery,
      setMonthKey,
      showMenu: (items: MenuItem[]) => setMenuItems(items),
      editEntry: (txn: LedgerTransaction) => setSheet({ kind: 'entry', editing: txn }),

      deleteEntry: async (txn: LedgerTransaction) => {
      ...
      restoreEntry: async (txn: LedgerTransaction) => {
        await restoreEntry(store, txn.id)
        await failIfNeeded()
      },
      purgeEntry: async (txn: LedgerTransaction) => {
      ...
      activateProject: async (project: Project) => {
        await activateProject(store, project.id)
        await failIfNeeded()
      },
      archiveProject: async (project: Project, archived: boolean) => {
        await updateProject(store, project.id, { isArchived: archived })
        await failIfNeeded()
      },
      editProject: (project: Project) => setSheet({ kind: 'project', editing: project }),
      openProject: (project: Project) => setRoute({ name: 'project', id: project.id }),
      recordIntoProject: async (project: Project) => {
        // 点「记账到此项目」会**先把该项目设为「当前项目」**再打开记一笔（副作用照抄原生）。
        await activateProject(store, project.id)
        setSheet({ kind: 'entry', editing: null })
      },

      openAccount: (account: Account) => setRoute({ name: 'account', id: account.id }),
      editAccount: (account: Account) => setSheet({ kind: 'account', editing: account }),
      archiveAccount: async (account: Account) => {
        await archiveAccount(store, account.id, true)
        await failIfNeeded()
      },
      reconcileAccount: (account: Account) => setSheet({ kind: 'reconcile', account }),

      openCurrencies: () => setSheet({ kind: 'currencies' }),
      openAddCurrency: () => setSheet({ kind: 'addCurrency' }),
      addCurrency: async (code: string) => {
      ...
      editRate: (code: string) => setSheet({ kind: 'rate', code }),
      ...
      editBudget: (categoryID: string | null) => setSheet({ kind: 'budget', categoryID }),

      addMember: async (project: Project) => {
      ...
      editMember: (member: Member) => setSheet({ kind: 'member', projectID: member.projectID, editing: member }),
      removeMember: async (member: Member) => {
        await removeMember(store, member.id)
        await failIfNeeded()
      },
      settleUp: async (project: Project, row: SettlementPlanRow) => {
        const from = store.member(row.fromMemberID)
        const to = store.member(row.toMemberID)
        const confirmed = await nativeConfirm({
          title: t(
            'prj.recordSettlement',
            from ? from.name : '',
            to ? to.name : '',
      ...
      openSplitEditor: (request: SplitEditorRequest) => setSheet({ kind: 'split', request }),

      saveEntry: async (payload: EntryEditorPayload, editing: LedgerTransaction | null) => {
        if (editing) {
  ...
  const ctx = React.useMemo<LedgerUIContext>(
    () => ({
      store,
      t,
      locale,
    ...
    [store, store.revision, t, locale, query, monthKey, canMutate, actions, labels],
...
function routeTitle(route: Route, store: LedgerStore, t: import('./types.js').Translate): string {
  if (!route) return ''
