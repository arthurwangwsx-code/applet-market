...
/** 兼容旧调用点：只关心能不能播。 */
export async function videoAvailable(): Promise<boolean> {
...
export { stageAspect as aspectFor, resolveVideo, playVideo } from '@aibox/applet-sdk'

export async function openStage(settings: PlaybackSettings, aspect?: string): Promise<VideoStageResult> {
  const api = bridge()
...
export async function videoStatus(): Promise<VideoProgress | null> {
  const api = bridge()
