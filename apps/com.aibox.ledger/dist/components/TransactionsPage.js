...
export function EntryRow({ store, txn, t, onOpen, onDelete, canMutate }) {
    ...
    const body = (_jsxs("button", { type: "button", className: "lg-btn", onClick: onOpen, style: { display: 'flex', alignItems: 'center', gap: SPACE.s3, width: '100%', padding: SPACE.s3 }, children: [_jsx(IconBadge, { name: glyph.icon, size: 36, color: glyph.color, background: alpha(glyph.color, 0.16) }), _jsxs("div", { style: { flex: '1 1 auto', minWidth: 0, display: 'flex', flexDirection: 'column', gap: 3 }, children: [_jsx("span", { className: "lg-clamp-1", style: { fontSize: 15, fontWeight: 500, color: C.ink }, children: entryTitle(store, txn, t) }), subtitle || txn.source === 'ai' ? (_jsxs("span", { style: { display: 'flex', alignItems: 'center', gap: 4, minWidth: 0 }, children: [_jsx("span", { className: "lg-clamp-1", style: { fontSize: 12, color: C.muted }, children: subtitle }), txn.source === 'ai' ? (_jsx(Icon, { name: "sparkles", size: 9, color: C.brand, style: { flex: '0 0 auto' } })) : null] })) : null] }), _jsxs("div", { style: { display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 2, flex: '0 0 auto' }, children: [_jsx("span", { className: "lg-mono", style: { fontSize: 16, fontWeight: 500, color: TONE[amount.tone] }, children: amount.text }), conversion ? (_jsx("span", { className: "lg-mono", style: { fontSize: 10, color: TONE[conversion.tone] }, children: conversion.text })) : null] })] }));
    if (!canMutate)
        return body;
...
export default function TransactionsPage({ ctx }) {
    ...
    const filtered = React.useMemo(() => loaded.filter((txn) => {
        if (typeFilter === 'expense' && txn.kind !== KIND.expense)
            return false;
        if (typeFilter === 'income' && txn.kind !== KIND.income)
            return false;
        if (typeFilter === 'transfer' && txn.kind !== KIND.transferOut && txn.kind !== KIND.transferIn)
            return false;
        if (accountFilter && txn.accountID !== accountFilter)
            return false;
        if (projectFilter && txn.projectID !== projectFilter)
            return false;
