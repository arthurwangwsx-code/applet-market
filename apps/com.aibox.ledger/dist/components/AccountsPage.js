...
export default function AccountsPage({ ctx }) {
    ...
    const grouped = ACCOUNT_KIND_ORDER.map((kind) => ({
        kind,
        rows: store.activeAccounts().filter((account) => account.kind === kind),
        ...
        actions.showMenu([
            {
                ...
                icon: 'equal.circle',
                onSelect: () => actions.reconcileAccount(account),
            },
            { id: 'edit', label: t('x.edit'), icon: 'pencil', onSelect: () => actions.editAccount(account) },
            {
                ...
                icon: 'archivebox',
                destructive: true,
                onSelect: () => actions.archiveAccount(account),
            },
        ]);
    });
