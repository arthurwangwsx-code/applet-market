...
export default function BudgetPage({ ctx }: { ctx: LedgerUIContext }) {
        ...
        <Card>
          <div style={{ display: 'flex', alignItems: 'center', marginBottom: hasBudget ? SPACE.s3 : 0 }}>
            ...
            {hasBudget ? (
              <button
                type="button"
                className="lg-btn"
                onClick={() => actions.editBudget(null)}
                aria-label={t('x.edit')}
              >
          ...
          {hasBudget ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: SPACE.s2 }}>
              ...
              <span style={{ fontSize: 12, color: C.muted }}>{over ? t('bud.overBudget') : t('bud.leftToSpend')}</span>
              ...
              <div style={{ display: 'flex', alignItems: 'center', fontSize: 12, color: C.muted }}>
                ...
                <div style={{ flex: '1 1 auto' }} />
                <span className="lg-mono">{t('bud.perDay', money(payload.dailyRemainingMinor, store.baseCode))}</span>
              </div>
            </div>
          ) : (
