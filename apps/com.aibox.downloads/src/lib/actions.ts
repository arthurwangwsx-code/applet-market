...
export const uiHooks: {
  pauseAll: (() => Promise<ActionResult>) | null
  resumeAll: (() => Promise<ActionResult>) | null
  clearFinished: (() => Promise<ActionResult>) | null
  refresh: (() => Promise<void>) | null
} = { pauseAll: null, resumeAll: null, clearFinished: null, refresh: null }

function targetOf(
  destination?: AddDownloadsInput['destination'],
  folder?: string,
): { kind: NonNullable<AddDownloadsInput['destination']>; path: string } {
  const path = folder || ''
  if (destination === 'iCloud') return { kind: 'iCloud', path }
  if (destination === 'externalFiles') return { kind: 'externalFiles', path }
  if (destination === 'vault') return { kind: 'vault', path }
  return { kind: 'sandbox', path }
}

function describe(task: DownloadTask): string {
...
export async function addDownloads({
  urls,
  filename,
  destination,
  folder,
  priority,
}: AddDownloadsInput): Promise<AddDownloadsResult> {
  ...
  const tasks: Array<Pick<DownloadTask, 'taskId' | 'url' | 'filename' | 'artifactRef'>> = []
...
export async function listDownloads({ state, limit }: { state?: DownloadFilterState; limit?: number } = {}) {
...
export async function controlDownloads({ action, taskId }: { action: DownloadControlAction; taskId?: string }) {
...
export function registerActions() {
  ...
  const register = api.action.register as unknown as (name: string, handler: (input: unknown) => unknown) => void
  register('add', (input) => addDownloads(input as AddDownloadsInput))
  register('list', (input) => listDownloads((input || {}) as { state?: DownloadFilterState; limit?: number }))
  register('control', (input) => controlDownloads(input as { action: DownloadControlAction; taskId?: string }))
  register('pauseAll', async () => {
    if (uiHooks.pauseAll) return uiHooks.pauseAll()
    return controlDownloads({ action: 'pause' })
  })
  register('resumeAll', async () => {
    if (uiHooks.resumeAll) return uiHooks.resumeAll()
    return controlDownloads({ action: 'resume' })
  })
  register('clearFinished', async () => {
    if (uiHooks.clearFinished) return uiHooks.clearFinished()
