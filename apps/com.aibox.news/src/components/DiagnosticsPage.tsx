...
import type { NewsContext, SourceRefreshState, SourceRefreshStatus, Translate } from '../types.js'

const STATUS_META: Record<SourceRefreshStatus, { icon: IconName; color: string; key: string }> = {
...
function failureText(state: SourceRefreshState, t: Translate) {
...
export default function DiagnosticsPage({ ctx }: { ctx: NewsContext }) {
    ...
    <div className="news-scroll">
      ...
      {report ? (
        <>
          <Card>
            <Row
              title={t('news.diagnostics.duration')}
              detail={duration((report.finishedAt - report.startedAt) / 1000, t)}
            />
      ...
      <Card>
        {report && report.sourceStates.length > 0 ? (
          report.sourceStates.map((state, i) => {
