...
import { pickBestFormat as pickFormat } from '@aibox/applet-sdk'
import {
  aspectFor,
  closeStage,
  copyText,
...
export default function DetailPage({ bvid, onOpen }: { bvid: string; onOpen: (video: VideoSummary) => void }) {
  ...
  const [relatedList, setRelated] = React.useState<VideoSummary[]>([])
  ...
  const pendingProgress = React.useRef<PendingProgress | null>(null)
  React.useEffect(() => {
  ...
  const play = React.useCallback(
    async (cid?: number) => {
      ...
      try {
        ...
        const pageURL =
          index > 0 ? `https://www.bilibili.com/video/${bvid}?p=${index + 1}` : `https://www.bilibili.com/video/${bvid}`
  ...
  if (!detail) return <EmptyState title="视频详情缺失" detail="请返回后重试。" />
  ...
  return (
    <div className="bl-scroll" style={{ height: '100%', overflowY: 'auto', background: C.bg }}>
      ...
      <div style={{ padding: SPACE.s4 }}>
        <div style={{ fontSize: 17, fontWeight: 600, color: C.text, lineHeight: 1.4 }}>{detail.title}</div>
      ...
      {detail.pages.length > 1 ? (
        <div style={{ paddingBottom: SPACE.s3 }}>
          ...
          <div
            className="bl-scroll"
            style={{ display: 'flex', gap: SPACE.s2, overflowX: 'auto', padding: `0 ${SPACE.s4}px` }}
          >
