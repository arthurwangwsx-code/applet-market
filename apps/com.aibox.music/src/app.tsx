...
import type { DialogAction } from '@aibox/applet-sdk'
import type {
  LyricsPayload,
  MusicActions,
  MusicAppContext,
...
export default function App() {
  ...
  const refs = React.useRef<{ store: MusicStore; music: MusicController } | null>(null)
  ...
  const [tab, setTab] = React.useState<TabID>('player')
  // 子页栈 = 宿主原生页栈的镜像（框架资产 `aibox/ui`）：进详情走 `aibox.navigation.push`，
  // 返回一律经 popstate 回来，于是最左缘左滑是**系统自己的** interactive pop。
  const subpages = useSubpageStack<MusicRoute>({
    pathFor: routePath,
    titleFor: (row) => routeTitle(row, tRef.current),
  })
  const { stack } = subpages
  const [sheet, setSheet] = React.useState<'favorites' | 'history' | null>(null)
  const [menuOpen, setMenuOpen] = React.useState(false)
  const [picker, setPicker] = React.useState<OptionRequest | null>(null)
  ...
  const [mode, setMode] = React.useState<'album' | 'lyrics'>('album')
  const [lyrics, setLyrics] = React.useState<LyricsPayload>({ state: 'none', synced: false, lines: [] })
  const [artwork, setArtwork] = React.useState<{ url: string | null; color: RGBColor | null }>({
    url: null,
    color: null,
  })
  const [favorites, setFavorites] = React.useState<{ loading: boolean; rows: MusicTrack[] }>({
    loading: true,
    rows: [],
  })
  ...
  React.useEffect(() => {
    ...
    const wire = async () => {
      ...
      offs.push(
        onNamespaceEvent<{ rendered?: boolean; selected?: string }>('tabs', 'changed', (payload) => {
          if (!payload) return
          ...
          if (payload.selected) {
            haptics.selection()
            if (isTabID(payload.selected)) setTab(payload.selected)
            resetRef.current()
            setMode('album')
          }
        }),
      ...
      offs.push(
        onNamespaceEvent<{ query?: string }>('toolbar', 'searchChanged', (payload) => {
  ...
  React.useEffect(() => {
    ...
    fetchLyrics(callMusic, current).then((payload) => {
      if (!cancelled) setLyrics(payload)
    })
    return () => {
      cancelled = true
    }
  ...
  const refreshFavorites = React.useCallback(async () => {
    const result = await callMusic<MusicTrack[]>('library', { action: 'list_favorites' })
  ...
  const overlayInvoke = React.useRef<((event: { controlId?: string }) => void) | null>(null)
  overlayInvoke.current = (event: { controlId?: string }) => {
  ...
  React.useEffect(() => {
    ...
    const wire = async () => {
      ...
      setShell((current) =>
        current.overlayRendered === rendered ? current : { ...current, overlayRendered: rendered },
      )
      offs.push(
        onNamespaceEvent<{ controlId?: string }>('overlay', 'invoke', (payload) => {
      ...
      offs.push(
        onNamespaceEvent<{ rendered?: boolean }>('overlay', 'changed', (payload) => {
  ...
  const favoriteKeys = React.useMemo(
    () => new Set<string>(favorites.rows.map((row) => stableKey(row)).filter((key): key is string => Boolean(key))),
    [favorites.rows],
  )

  const toggleFavorite = React.useCallback(
    async (track: MusicTrack | null, next?: boolean) => {
      if (!track) return
  ...
  const addToPlaylist = React.useCallback(
    async (track: MusicTrack) => {
      const listed = await callMusic<{ playlists?: MusicCollection[] }>('playlist', { action: 'list' })
  ...
  const actions = React.useMemo<MusicActions>(
    () => ({
      navigate,
      back,
      notice: showNotice,
      ...
      playTrack: (track: MusicTrack, group?: MusicTrack[]) => {
      ...
      shufflePlay: async (tracks: MusicTrack[]) => {
        // 「随机」= 先开启随机模式，再从第一首起播（与原生同序）。
        await music.setShuffle(true)
        const first = tracks[0]
        if (first) void music.play(first, tracks)
      },
      openCollection: (item: MusicCollection) => {
        store.rememberArtwork(item)
        navigate({ name: 'collection', item })
      },
      playCollection: async (item: MusicCollection) => {
      ...
      confirmClear: async (message: string, run: () => void) => {
      ...
      trackMenu: async (track: MusicTrack, options: TrackMenuOptions = {}) => {
        const key = stableKey(track)
        ...
        else if (picked === 'remove' && options.queueIndex !== undefined) music.removeFromQueue(options.queueIndex)
      ...
      collectionMenu: async (item: MusicCollection) => {
        const picked = await actionSheet({
  ...
  const ctx = React.useMemo<MusicAppContext>(
    () => ({
      t,
      locale,
      store,
    ...
    [t, locale, store, music, actions, favorites, current, music.version, store.version],
  ...
  const overlaySent = React.useRef<string | null>(null)
  React.useEffect(() => {
  ...
  React.useEffect(() => {
    ...
    registerAction('setRepeat', (input) => {
      ...
      if (value === 'off' || value === 'one' || value === 'all') void music.setRepeat(value)
      return null
    })
  ...
  return (
    <div className="mu-root">
        ...
        <>
          {tab === 'player' ? (
            <NowPlaying
              ...
              isFavorite={Boolean(stableKey(current) && favoriteKeys.has(stableKey(current)!))}
      ...
      {showMini && !shell.overlayRendered ? (
        <MiniBar
          track={current ? { ...current, artworkUrl: artwork.url } : null}
