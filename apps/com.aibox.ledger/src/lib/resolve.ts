...
export type ResolveResult<T> = { found: true; value: T } | { found: false; candidates: string[] }

const hit = <T>(value: T): ResolveResult<T> => ({ found: true, value })
const miss = <T>(candidates: string[] = []): ResolveResult<T> => ({ found: false, candidates })
 ...
 */
export function resolveCategory(store: LedgerStore, raw: unknown, kind: FlowKind): ResolveResult<Category> {
  ...
  if (exact.length === 1 && exact[0]) return hit(exact[0])
  ...
  if (fuzzy.length === 1 && fuzzy[0]) return hit(fuzzy[0])
...
const KIND_WORDS: Array<[AccountKind, string[]]> = [
...
export function resolveAccount(
  ...
  const narrow = (rows: Account[]): ResolveResult<Account> | null => {
    if (rows.length === 1 && rows[0]) return hit(rows[0])
    if (rows.length > 1 && preferCurrency) {
      const same = rows.filter((row) => row.currency === String(preferCurrency).toUpperCase())
      if (same.length === 1 && same[0]) return hit(same[0])
    }
...
/** 项目：精确 → 唯一模糊 → 未找到。 */
export function resolveProject(store: LedgerStore, raw: unknown): ResolveResult<Project> {
  const needle = lower(raw)
  if (needle.length === 0) return miss()
...
export function resolveMember(store: LedgerStore, projectID: string, raw: unknown): ResolveResult<Member> {
  const needle = lower(raw)
...
/** 「不归项目」的显式说法。 */
export function isNoneToken(raw: unknown): boolean {
  const needle = lower(raw)
