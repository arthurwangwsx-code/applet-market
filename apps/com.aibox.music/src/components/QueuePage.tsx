...
type DragState = { from: number; to: number; startY: number }

export default function QueuePage({ ctx }: { ctx: MusicAppContext }) {
  ...
  const frequentGestures = useRowGestures('queue.mostPlayed', {
    ...
    onAction: ({ rowId, actionId }) => {
      ...
      if (actionId === 'play')
        actions.playTrack(
          row.track,
          mostPlayed.map((item) => item.track),
        )
            ...
            {ordered.map((track, position) => {
              ...
              const row = (
                <QueueRow
                  ...
                  onLongPress={
                    upNextGestures.rendered ? undefined : () => actions.trackMenu(track, { queueIndex: absolute })
                  }
                  trailing={
                    editing ? (
                      <span
