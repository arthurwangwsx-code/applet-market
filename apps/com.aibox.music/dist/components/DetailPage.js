...
export function CollectionDetail({ ctx, item }) {
    ...
    return (_jsxs("div", { className: "mu-scroll", "aria-label": "audioCollectionDetail", children: [_jsxs("div", { style: {
                ...
                }, children: [_jsx(Artwork, { url: item.artworkUrl, size: isArtist ? 130 : 180, radius: isArtist ? 65 : 12, iconSize: isArtist ? 30 : 44, shadow: isArtist ? '0 6px 12px rgba(0,0,0,0.2)' : '0 8px 14px rgba(0,0,0,0.22)' }), _jsx("span", { style: { fontSize: isArtist ? 20 : 18, fontWeight: 700, textAlign: 'center' }, children: item.title || item.name }), !isArtist && item.artist ? (_jsx("span", { className: "mu-clamp-1", style: { fontSize: 14, color: C.muted }, children: item.artist })) : null, item.url ? (_jsxs("button", { type: "button", className: "mu-btn mu-press", onClick: () => openURL(item.url), style: { display: 'flex', alignItems: 'center', gap: 6, color: C.accent, fontSize: 13 }, children: [_jsx(Icon, { name: "arrow.up.forward.app", size: 14, color: C.accent }), t('common.openInAppleMusic')] })) : null, !isArtist ? (_jsxs("div", { style: { display: 'flex', gap: 10, width: '100%', paddingTop: 4 }, children: [_jsx(HeaderButton, { icon: "play.fill", label: t('detail.play'), disabled: state.loading || tracks.length === 0, onClick: () => actions.playTrack(tracks[0], tracks) }), _jsx(HeaderButton, { icon: "shuffle", label: t('detail.shuffle'), disabled: state.loading || tracks.length === 0, onClick: () => actions.shufflePlay(tracks) })] })) : null] }), state.loading ? (_jsx("div", { style: { padding: 30, display: 'flex', justifyContent: 'center' }, children: _jsx(Spinner, { color: C.muted }) })) : null, !state.loading && tracks.length === 0 && state.albums.length === 0 ? (_jsx(EmptyState, { icon: "music.note.list", title: isArtist ? t('detail.artistFailed') : t('detail.personalized'), top: 20, action: item.url ? (_jsx("button", { type: "button", className: "mu-btn mu-press", onClick: () => openURL(item.url), style: {
...
export function CategoryList({ ctx, route }) {
    ...
    return (_jsxs("div", { className: "mu-scroll", children: [items.map((item, index) => (isSongs ? (_jsx(SongRow, { track: item, onClick: () => actions.playTrack(item, items), onLongPress: () => actions.trackMenu(item, { group: items }) }, item.musicItemId || index)) : (_jsx(CollectionRow, { item: item, onClick: () => actions.openCollection(item), onLongPress: () => actions.collectionMenu(item) }, item.musicItemId || index)))), _jsx("div", { style: { height: 24 } })] }));
}
