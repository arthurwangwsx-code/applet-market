...
import type { Account, LedgerUIContext } from '../types.js'

export default function AccountsPage({ ctx }: { ctx: LedgerUIContext }) {
              ...
              <Card padding={0}>
                {group.rows.map((account, index) => (
                  <React.Fragment key={account.id}>
                    ...
                    <AccountRow ctx={ctx} account={account} balance={balances[account.id] ?? 0} canMutate={canMutate} />
                  </React.Fragment>
                ))}
              </Card>
            </div>
...
function Column({ label, value }: { label: string; value: string }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
      ...
      <span className="lg-mono" style={{ fontSize: 16, fontWeight: 500, color: C.onAccent }}>
        {value}
      </span>
    </div>
  )
...
export function AccountDetail({ ctx, accountID }: { ctx: LedgerUIContext; accountID: string }) {
    ...
    <div className="lg-scroll" style={{ flex: '1 1 auto' }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: SPACE.s4, padding: SPACE.s4, paddingBottom: 96 }}>
        ...
        <div>
          ...
          {recent.length === 0 ? (
            <Card>
              <span style={{ fontSize: 15, color: C.muted }}>{t('acc.noEntries')}</span>
            </Card>
          ) : (
            <Card padding={0}>
