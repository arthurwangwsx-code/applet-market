...
export function EmptyState({ icon, text, actionLabel, onAction, padding = 48 }: EmptyStateProps) {
  return (
    <div
      style={{
        display: 'flex',
...
export function Card({ title, subtitle, trailing, children, padding = SPACE.s3, style }: CardProps) {
  return (
...
export function Stat({ label, value, color, valueSize = 15, align = 'flex-start' }: StatProps) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 2, alignItems: align, minWidth: 0 }}>
      <span style={{ fontSize: 12, color: C.muted }}>{label}</span>
      <span className="fin-mono" style={{ fontSize: valueSize, fontWeight: 500, color: color || C.ink }}>
        {value}
      </span>
    </div>
  )
...
export function Chip({ label, selected, onClick, variant = 'brand' }: ChipProps) {
  ...
  return (
    ...
    >
      ...
      {detail !== undefined && detail !== null ? (
        <span className="fin-mono" style={{ fontSize: 15, color: detailColor || C.muted, flex: '0 0 auto' }}>
          {detail}
        </span>
      ) : null}
      {accessory}
...
export function SwipeRow({
  ...
  const settle = React.useCallback((value: number) => {
    live.current = value
    setAnimating(true)
    setOffset(value)
