...
import type { LyricsLine, LyricsPayload, MusicCall, MusicCallResult, MusicTrack } from './types.js'

const TIME_TAG = /\[(\d{1,3}):(\d{1,2})(?:[.:](\d{1,3}))?\]/g
const OFFSET_TAG = /\[offset:\s*([+-]?\d+)\s*\]/i

/** LRC 解析（规格 §4.2）：一行多标签各产一行；`[offset:±ms]` 加到所有时间轴上；按时间升序。 */
export function parseLRC(raw: unknown): LyricsLine[] {
...
export function plainLines(raw: unknown): LyricsLine[] {
  return String(raw || '')
    .split(/\r?\n/)
    .map((line) => line.trim())
...
export function currentLineIndex(lines: LyricsLine[], displayTime: unknown): number {
 ...
 * 量化到 1/60 档，吃掉浮点微抖。
 */
export function sweepRatio(lines: LyricsLine[], index: number, displayTime: unknown): number {
  if (!Array.isArray(lines) || index < 0 || index >= lines.length) return 0
  const line = lines[index]
  if (!line || line.time === null || line.time === undefined) return 0
 ...
 *  3. 当前实现：纯文本 → 无时间轴行模型。
 */
type StructuredLyrics = { synced?: boolean; source?: string | null; lines?: Array<Partial<LyricsLine>> }

export function readLyricsPayload(result: MusicCallResult<StructuredLyrics>): LyricsPayload {
  if (!result || !result.ok) {
...
/** 取当前播放曲（或指定曲）的歌词。 */
export async function fetchLyrics(music: MusicCall, track: MusicTrack | null): Promise<LyricsPayload> {
  const args: Record<string, unknown> = {}
  if (track && track.title) {
    args.title = track.title
    if (track.artist) args.artist = track.artist
  }
  const result = await music<StructuredLyrics>('lyrics', args)
