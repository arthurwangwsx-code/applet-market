...
import type { AccountSnapshot, AccountValuation, Candle, Currency, FXMap, QuoteMap } from './types.js'

function annualizedFrom(totalPct: number, days: number): number {
  if (days <= 0) return 0
 ...
 */
export type BacktestStrategy = 'buyhold' | 'macross'
...
export function backtest(candles: Candle[], strategy: BacktestStrategy = 'buyhold'): BacktestResult | null {
  ...
  if (strategy === 'macross') {
    ...
    for (let i = 0; i < rows.length; i += 1) {
      if (holding && i > 0) nav *= (closes[i] ?? 0) / (closes[i - 1] ?? 1)
      equity.push(nav)
      if (i < 20) continue
      ...
      if (golden && !holding) {
        ...
        holding = false
        if ((closes[i] ?? 0) > entry) wins += 1
      }
    }
    if (holding && (closes.at(-1) ?? 0) > entry) wins += 1
  } else {
...
export function dcaPlan(
  ...
  if (rows.length < step || !(amount > 0)) return null
...
export function rebalance({
  ...
  for (const [symbol, row] of currentBySymbol) {
    if (row.marketValueMinor <= 0) continue
    proposals.push({
      symbol,
...
export function snapshotFor(valuation: AccountValuation, when?: number): AccountSnapshot | null {
  if (!valuation.isComplete) return null
  return {
