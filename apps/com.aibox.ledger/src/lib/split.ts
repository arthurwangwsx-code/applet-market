...
import type { Account, Member, TransactionSplit } from '../types.js'
import type { LedgerStore } from './store.js'

export const SPLIT_MODES = ['equal', 'exact', 'shares', 'percent'] as const

export interface ResolvedSplitShare {
  memberID: string
  amountMinor: number
}
 ...
 */
export function resolveSplit(split: TransactionSplit | null, totalMinor: number): ResolvedSplitShare[] {
  ...
  if (mode === 'equal') {
    ...
    shares.forEach((share, index) => {
      ...
      if (target) target.amountMinor = value < 0 ? -Math.round(-value * 100) : Math.round(value * 100)
    })
  } else if (mode === 'shares') {
    ...
    if (sum > 0)
      weights.forEach((weight, index) => {
        const target = out[index]
        if (target) target.amountMinor = Math.round((total * weight) / sum)
      })
  } else if (mode === 'percent') {
    shares.forEach((share, index) => {
      const percent = Math.max(0, Number(share.value) || 0)
      const target = out[index]
      if (target) target.amountMinor = Math.round((total * percent) / 100)
    })
  }

  const assigned = out.reduce((acc, row) => acc + row.amountMinor, 0)
  const first = out[0]
  if (first) first.amountMinor += total - assigned
  return out
}

/** 项目里的「我」（没有则 null）。 */
export function meMember(store: LedgerStore, projectID: string): Member | null {
 ...
 * `>0` = 别人欠 TA；`<0` = TA 欠别人。
 */
export function memberBalances(store: LedgerStore, projectID: string): Record<string, number> {
...
export function settlementPlan(
  ...
  const plan: Array<{ fromMemberID: string; toMemberID: string; amountMinor: number }> = []
  let ci = 0
  let di = 0
  while (ci < creditors.length && di < debtors.length) {
    ...
    if (amount > 0) plan.push({ fromMemberID: debtor.id, toMemberID: creditor.id, amountMinor: amount })
    creditor.value -= amount
    debtor.value -= amount
    if (creditor.value <= 0) ci += 1
    if (debtor.value <= 0) di += 1
  }
  return plan
}
...
export async function recordSettlement(
  ...
  const now = Date.now()
  let linked: import('../types.js').LedgerTransaction | null = null
...
/** 删除结算 → 连带软删关联流水。 */
export async function removeSettlement(store: LedgerStore, id: string) {
...
/** 项目已花（基准币，支出合计）。 */
export function projectSpentMinor(store: LedgerStore, projectID: string): number {
  let total = 0
  for (const txn of store.allTransactions()) {
    if (txn.projectID !== projectID || txn.kind !== KIND.expense) continue
...
export function projectIncomeMinor(store: LedgerStore, projectID: string): number {
  let total = 0
  for (const txn of store.allTransactions()) {
