...
import { RADIUS, SPACE, alpha, favouriteTint } from '../lib/theme.js';
import { DEFAULT_FILTER, filterIsActive } from '../lib/types.js';
...
export function MemoList(props) {
    ...
    const gestures = useRowGestures(props.regionId || 'memos', {
        ...
        onAction: ({ rowId, actionId }) => {
            const memo = rows.find((row) => row.id === rowId);
            if (memo)
                props.onAction?.(memo, actionId);
        },
    });
    if (rows.length === 0) {
        const filtered = props.query.trim() !== '' || filterIsActive(props.filter);
        if (filtered && props.memos.length > 0) {
            return (_jsxs("div", { style: { padding: `${SPACE.s8}px ${SPACE.s5}px`, textAlign: 'center' }, children: [_jsx(Icon, { name: "magnifyingglass", size: 40, color: palette.accent }), _jsx("div", { style: { fontSize: 17, fontWeight: 600, color: palette.ink, marginTop: SPACE.s3 }, children: t('noMatchTitle') }), _jsx("div", { style: { fontSize: 14, color: palette.muted, marginTop: 6 }, children: t('noMatchBody') }), filterIsActive(props.filter) ? (_jsx("div", { style: { marginTop: SPACE.s4 }, children: _jsx(SecondaryButton, { palette: palette, title: t('clearFilter'), onClick: props.onClearFilter }) })) : null] }));
        }
        return (_jsxs("div", { style: { padding: `${SPACE.s8}px ${SPACE.s5}px`, textAlign: 'center' }, children: [_jsx(Icon, { name: "mic", size: 40, color: palette.accent }), _jsx("div", { style: { fontSize: 17, fontWeight: 600, color: palette.ink, marginTop: SPACE.s3 }, children: t('emptyTitle') }), _jsx("div", { style: { fontSize: 14, color: palette.muted, marginTop: 6 }, children: props.scoped ? t('emptyScopedBody') : t('emptyBody') })] }));
    }
    return (_jsx("div", { ...gestures.regionProps, children: rows.map((memo) => (_jsx(MemoRow, { palette: palette, t: t, dark: props.dark, memo: memo, busy: props.busyIDs[memo.id], rowId: memo.id, onOpen: () => props.onOpen(memo), onMenu: gestures.rendered ? undefined : () => props.onMenu(memo) }, memo.id))) }));
}
...
export function applyFilter(memos, filter, query) {
    ...
    let rows = memos.filter((memo) => {
        ...
        if (filter.favOnly && !memo.isFavourite)
            return false;
        if (filter.withTranscript && !memo.hasTranscript)
            return false;
        if (filter.duration === 'under1m' && !(memo.duration < 60))
            return false;
        if (filter.duration === '1to5m' && !(memo.duration >= 60 && memo.duration <= 300))
            return false;
        if (filter.duration === 'over5m' && !(memo.duration >= 300))
            return false;
        const age = now - memo.createdAt;
        if (filter.date === 'today' && new Date(memo.createdAt).toDateString() !== new Date(now).toDateString())
            return false;
        if (filter.date === 'week' && age > 7 * 86_400_000)
            return false;
        if (filter.date === 'month' && age > 30 * 86_400_000)
            return false;
        if (filter.date === 'year' && age > 365 * 86_400_000)
            return false;
        return true;
    });
    rows = [...rows].sort((a, b) => {
...
export function FilterSheet(props) {
    ...
    return (_jsxs(Sheet, { palette: palette, open: props.open, onClose: props.onClose, children: [_jsxs("div", { style: { display: 'flex', alignItems: 'center', padding: SPACE.s4 }, children: [_jsx("button", { type: "button", disabled: isDefault, onClick: () => props.onChange(DEFAULT_FILTER), style: {
            ...
            ]), _jsxs("div", { style: { padding: `0 ${SPACE.s4}px ${SPACE.s6}px` }, children: [_jsx(Toggle, { palette: palette, label: t('favOnly'), value: filter.favOnly, onChange: (value) => props.onChange({ ...filter, favOnly: value }) }), _jsx(Toggle, { palette: palette, label: t('withTranscript'), value: filter.withTranscript, onChange: (value) => props.onChange({ ...filter, withTranscript: value }) })] })] }));
}
