...
function groupInteger(text: string): string {
  let out = ''
  let count = 0
...
export function money(
  ...
  currency: string,
  {
    ...
    grouping = true,
    decimals,
  }: { signed?: boolean; grouping?: boolean; decimals?: number | null } = {},
): string {
...
export function plainMajor(minor: number): string {
 ...
 */
export function majorNumberToMinor(value: number): number {
