...
import type { LedgerUIContext, Translate } from '../types.js'
import type { LedgerStore } from '../lib/store.js'

type ScenarioID = 'quick' | 'review' | 'budget' | 'anomaly' | 'subs'

const SCENARIOS: Array<{
  id: Exclude<ScenarioID, 'quick'>
  icon: string
  tint: string
  titleKey: string
  subKey: string
...
const SYSTEM =
  'You are helping the user with their personal ledger. Every number in the DATA block below ' +
...
const PROMPTS: Record<ScenarioID, string> = {
  ...
  review:
    "Review this month's spending. Use the category breakdown, daily trend and month-over-month figures in " +
    'DATA, plus the budget status, then give a short verdict and 2–3 actionable suggestions.',
  budget:
    'Check whether my budgets are realistic. Compare each category budget with actual spending in DATA ' +
    '(including the recent three-month averages), point out the biggest deviations and suggest adjustments.',
  anomaly:
    'Scan for unusual spending. Use the large expenses and month-over-month comparison in DATA and flag ' +
    'anything that stands out against history.',
  subs:
    'Tally my recurring and subscription spending. Use the repeated merchants/notes in DATA and total up my ' +
    'fixed monthly costs.',
}

/** 把真实账本算成一段紧凑的数据简报（所有金额都是基准币）。 */
function buildBrief(store: LedgerStore, locale: string, t: Translate, scenario: ScenarioID): string {
  ...
  if (scenario !== 'quick') {
    ...
    lines.push(
      `daily_trend: ${byDay.map((row) => `${row.label}=${money(row.amountMinor, base)}`).join(', ') || 'none'}`,
    )
  }
  ...
  if (scenario === 'anomaly') {
    const large = monthFlowTransactions(store, month)
      ...
      .map(
        (txn) =>
          `${money(store.reportingBaseMinor(txn), base)} ${
            txn.merchant || (txn.categoryID ? store.categoryPath(txn.categoryID) : '')
          }`,
      )
  ...
  if (scenario === 'subs') {
    const counter = new Map<string, { label: string; count: number; total: number }>()
    ...
    lines.push(
      `repeated_payees: ${
        repeated.map((row) => `${row.label}×${row.count}=${money(row.total, base)}`).join(' | ') || 'none'
      }`,
    )
  }
...
export default function AIPanel({ ctx }: { ctx: LedgerUIContext }) {
  const { store, t, locale } = ctx
  const [running, setRunning] = React.useState<ScenarioID | null>(null)
  const [answers, setAnswers] = React.useState<Partial<Record<ScenarioID, string>>>({})

  const run = async (scenario: ScenarioID) => {
    setRunning(scenario)
    try {
          ...
          {SCENARIOS.map((scenario) => (
            <Card key={scenario.id}>
              <button
                ...
                style={{ display: 'flex', alignItems: 'center', gap: SPACE.s3, width: '100%' }}
              >
                <IconBadge name={scenario.icon} size={38} color={scenario.tint} background={fade(scenario.tint, 14)} />
                <div style={{ flex: '1 1 auto', minWidth: 0, display: 'flex', flexDirection: 'column', gap: 2 }}>
