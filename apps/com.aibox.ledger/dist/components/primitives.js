...
export function Sheet({ open, onClose, title, leading, trailing, detent, children, }) {
    ...
    return (_jsx("div", { className: "lg-backdrop", onClick: (event) => {
            if (event.target === event.currentTarget)
                onClose();
        }, children: _jsxs("div", { className: "lg-sheet", style: detent ? { height: detent } : undefined, children: [_jsxs("div", { style: {
                        display: 'flex',
                        alignItems: 'center',
                        gap: SPACE.s3,
...
export function Menu({ open, onClose, items, anchorStyle, }) {
    ...
    return (_jsx("div", { className: "lg-backdrop", style: { alignItems: 'center', justifyContent: 'center' }, onClick: onClose, children: _jsx("div", { style: {
                ...
                margin: SPACE.s4,
                ...anchorStyle,
            }, onClick: (event) => event.stopPropagation(), children: items.map((item, index) => (_jsxs("button", { type: "button", className: "lg-btn", onClick: () => {
                    onClose();
                    item.onSelect();
                }, style: {
                    display: 'flex',
...
export function SwipeRow({ children, leading, trailing, disabled, }) {
    ...
    return (_jsxs("div", { style: { position: 'relative', overflow: 'hidden' }, children: [trailing ? (_jsxs("button", { type: "button", className: "lg-btn", onClick: () => {
                    setOffset(0);
                    trailing.onAction();
                }, style: {
                    position: 'absolute',
                    ...
                    gap: 4,
                    fontSize: 13,
                }, children: [trailing.icon ? _jsx(Icon, { name: trailing.icon, size: 16, color: "#FFFFFF" }) : null, _jsx("span", { children: trailing.label })] })) : null, leading ? (_jsxs("button", { type: "button", className: "lg-btn", onClick: () => {
                    setOffset(0);
                    leading.onAction();
                }, style: {
                    position: 'absolute',
