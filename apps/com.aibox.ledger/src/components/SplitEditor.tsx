...
import type { LedgerUIContext, SplitEditorRequest, SplitMode, TransactionSplit } from '../types.js'

const MODES: SplitMode[] = ['equal', 'exact', 'shares', 'percent']

interface SplitEditorProps {
  ctx: LedgerUIContext
  request: SplitEditorRequest
  onSubmit: React.MutableRefObject<() => { split: TransactionSplit | null; valid: boolean }>
}

export default function SplitEditor({ ctx, request, onSubmit }: SplitEditorProps) {
  const { store, t } = ctx
  const members = store.projectMembers(request.projectID)
  const total = request.totalBaseMinor ?? 0

  const [mode, setMode] = React.useState<SplitMode>(request.split ? request.split.mode : 'equal')
  ...
  const draft = React.useMemo<TransactionSplit>(
    () => ({
      mode,
  ...
  const amountFor = (memberID: string) => resolved.find((row) => row.memberID === memberID)?.amountMinor ?? 0
  const assigned =
    mode === 'exact'
  ...
  const placeholder: Record<SplitMode, string> = { percent: '%', shares: '1', exact: '0.00', equal: '' }

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: SPACE.s3 }}>
      <Segmented value={mode} onChange={setMode} items={MODES.map((id) => ({ id, label: t(`ent.${id}`) }))} />
      <span style={{ fontSize: 12, color: C.muted, lineHeight: 1.4 }}>{hint[mode]}</span>
      ...
      <div style={{ display: 'flex', alignItems: 'center', padding: '0 4px' }}>
        ...
        <span className="lg-mono" style={{ fontSize: 13, color: C.muted }}>
        ...
        {members.map((member, index) => {
          ...
          return (
            ...
            >
              ...
              >
                <Icon name={on ? 'checkmark.circle.fill' : 'circle'} size={18} color={on ? C.brand : C.muted} />
              </button>
