...
import type { LedgerUIContext, TransactionKind } from '../types.js'
import type { CSVImportDraft } from '../lib/csv.js'

const PREVIEW_LIMIT = 100

const GLYPH: Record<TransactionKind, { icon: string; color: string }> = {
...
const KIND_LABEL_KEY: Record<TransactionKind, string> = {
...
export default function CSVImportPreview({ ctx, draft }: { ctx: LedgerUIContext; draft: CSVImportDraft }) {
  const { t, locale } = ctx
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: SPACE.s3 }}>
      ...
      <span style={{ fontSize: 12, color: C.muted, padding: '0 4px', lineHeight: 1.4 }}>{t('csv.noWriteFooter')}</span>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: SPACE.s3 }}>
              {problems.slice(0, 50).map((problem, index) => (
                <div key={index} style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                  <span style={{ fontSize: 13, fontWeight: 500, color: C.expense }}>{t('csv.row', problem.row)}</span>
              ...
              return (
                <React.Fragment key={`${row.line}-${row.originalID}`}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: SPACE.s3, padding: SPACE.s3 }}>
                    <IconBadge name={glyph.icon} size={32} color={glyph.color} background={alpha(glyph.color, 0.16)} />
