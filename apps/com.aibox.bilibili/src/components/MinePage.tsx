...
import type { LoginQRCode, UserProfile, VideoSummary } from '../lib/types.js'
 ...
 */
const fallbackQRURL: typeof ui.qrURL = (content, options) => {
...
const qrURL: typeof ui.qrURL = typeof ui.qrURL === 'function' ? ui.qrURL : fallbackQRURL

type LoginPhase = 'checking' | 'guest' | 'qr' | 'signedIn'

export default function MinePage({ onOpen }: { onOpen: (video: VideoSummary) => void }) {
  const [phase, setPhase] = React.useState<LoginPhase>('checking')
  const [user, setUser] = React.useState<UserProfile | null>(null)
  const [qr, setQR] = React.useState<LoginQRCode | null>(null)
  const [hint, setHint] = React.useState('')
  const [historyList, setHistory] = React.useState<VideoSummary[]>([])
    ...
    return (
      <div style={{ padding: SPACE.s5, textAlign: 'center' }}>
        ...
        >
          ...
          <img
            src={qrURL(qr.url, { size: 220, level: 'M' })}
            alt="登录二维码"
