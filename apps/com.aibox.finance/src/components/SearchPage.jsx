...
const MARKET_ORDER = ['ashare', 'hk', 'us', 'fund']
...
function ResultRow({ hit, ctx, onOpen }) {
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: SPACE.s3, padding: `8px ${SPACE.s4}px`, minHeight: 52 }}>
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: 2, minWidth: 0, flex: '1 1 auto' }}>
          <span className="fin-clamp-1" style={{ fontSize: 15, color: C.ink }}>{hit.name}</span>
...
export default function SearchPage({ ctx, visible, onClose }) {
  ...
  React.useEffect(() => {
    ...
    if (!text) { setHits([]); setSearching(false); return undefined }
    setSearching(true)
    let cancelled = false
    const timer = window.setTimeout(async () => {
      ...
      setHits(rows)
      setSearching(false)
      if (rows.length > 0) quotes.refresh(rows.map((row) => row.symbol), { force: false })
    }, DEBOUNCE_MS)
  ...
  return (
    <Sheet visible={visible} onClose={onClose} maxHeight="92dvh">
      ...
      <VirtualList
        ...
        empty={query.trim() && !searching ? <EmptyState text={t('finance.search.none')} padding={40} /> : (
          searching ? (
          ...
          if (row.kind === 'header') {
            return (
              ...
              >
                ...
                {row.action === 'clear' ? (
                  <button type="button" className="fin-btn fin-press" onClick={() => store.clearRecent()} style={{ color: C.brand, fontSize: 12 }}>
