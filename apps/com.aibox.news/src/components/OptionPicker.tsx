...
interface PickerRowProps<T extends OptionValue> {
  icon?: IconName
  title: string
  value: T
...
export default function PickerRow<T extends OptionValue>({
  icon,
  title,
  value,
  options,
  ...
  return (
    <>
      ...
      <Sheet visible={open} onClose={() => setOpen(false)}>
        <div style={{ padding: `${SPACE.s4}px ${SPACE.s4}px ${SPACE.s2}px`, fontSize: 13, color: C.muted }}>
          {title}
        </div>
