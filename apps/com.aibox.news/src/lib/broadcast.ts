...
import type { NewsArticle, Translate, Unsubscribe } from '../types.js'
...
export class BroadcastController {
  ...
  dismissNotice() {
    ...
    if (this.noticeKey === null) return
    this.noticeKey = null
    this.emit()
  }

  /** 工具栏「朗读」：取当前列表投影后的 lead 序列前 20 篇。 */
  start(articles: NewsArticle[]) {
  ...
  startFrom(article: NewsArticle, sequence: NewsArticle[]) {
    const list = sequence || []
  ...
  toggle(articles: NewsArticle[]) {
    if (this.active) this.stop({ userInitiated: true })
    else this.start(articles)
  }

  stop({ userInitiated = false }: { userInitiated?: boolean } = {}) {
    this.token += 1
    this.clearTimer()
    stopSpeaking()
  ...
  async textFor(article: NewsArticle): Promise<string> {
    let body = ''
    if (article.url) {
      const cached = await this.store.readContent(article.url)
      if (cached) body = cached
    }
    if (!body && article.contentHTML) body = extract(article.contentHTML)
    if (!body) body = article.summary || ''
