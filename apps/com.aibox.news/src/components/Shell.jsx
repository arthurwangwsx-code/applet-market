...
export function NavBar({ title, onBack, backLabel, trailing }) {
  return (
    ...
    >
      <div style={{ width: 76, display: 'flex', justifyContent: 'flex-start' }}>
        {onBack ? (
          <button
            ...
            style={{ display: 'flex', alignItems: 'center', gap: 2, color: C.brand, padding: '8px 6px' }}
          >
      ...
      <div style={{ width: 76, display: 'flex', justifyContent: 'flex-end', gap: 2 }}>{trailing}</div>
    </div>
  )
}
...
export function SearchField({ value, onChange, placeholder }) {
  return (
    <div style={{ padding: `${SPACE.s2}px ${SPACE.s4}px`, background: C.bg, flex: '0 0 auto' }}>
      <div style={{
