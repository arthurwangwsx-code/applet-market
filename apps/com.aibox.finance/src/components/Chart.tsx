...
function pathFor(values: ChartValue[], { width, height, min, max, padTop = 0, padBottom = 0 }: Geometry): string {
...
function extent(series: ChartValue[][]): { min: number; max: number } {
  let min = Infinity
  let max = -Infinity
...
export function DonutChart({
  ...
  slices: Array<{ id?: string | number; value: number }>
  size?: number
  colors: string[]
}) {
  ...
  const paths = slices.map((row, index) => {
    ...
    return (
      <path
        ...
        fill={colors[index % colors.length] ?? C.brand}
      />
    )
  })
...
export function LineChart({
  ...
  return <AreaLineChart values={values} baseline={baseline} height={height} color={color} upIsRed={upIsRed} />
}
