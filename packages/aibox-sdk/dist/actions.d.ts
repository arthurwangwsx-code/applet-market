...
type Entry<K> = K extends keyof AppletActionMap ? AppletActionMap[K] : {
    input: JSONValue;
    output: JSONValue;
};
/** 某个 action 的入参类型。 */
export type ActionInput<K extends ActionName> = Entry<K> extends {
    input: infer I;
} ? I : JSONValue;
/** 某个 action 的返回类型。 */
export type ActionOutput<K extends ActionName> = Entry<K> extends {
    output: infer O;
} ? O : JSONValue;
/** 一个 action 的处理函数。 */
 ...
 */
export declare function registerAction<K extends ActionName>(name: K, handler: ActionHandler<K>): void;
/** 一次注册整张表。键必须覆盖 `AppletActionMap` 声明的**全部** action——漏一个就是编译错。 */
export type ActionHandlers = MapIsEmpty extends true ? Record<string, (input: JSONValue) => JSONValue | Promise<JSONValue>> : {
 ...
 */
export declare function registerActions(handlers: ActionHandlers): void;
/** 把结构化结果回给当前调用方（AI / 自动化）。 */
export declare function actionResult(data: JSONValue): Promise<boolean>;
export {};
