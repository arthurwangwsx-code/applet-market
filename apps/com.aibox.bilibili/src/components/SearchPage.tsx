...
import type { BangumiSummary, SearchRow, UserSummary, VideoSummary } from '../lib/types.js'
import { errorMessage } from '../lib/types.js'

/** 番剧/UP 主这类本应用没做详情的结果，点了交给宿主浏览器，而不是留一个点不动的行。 */
function BangumiRow({ item }: { item: BangumiSummary }) {
  return (
    <div
      className="bl-press"
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div className="bl-clamp2" style={{ fontSize: 14, color: C.text }}>
          {item.title}
        </div>
...
export default function SearchPage({ onOpen }: { onOpen: (video: VideoSummary) => void }) {
  const [keyword, setKeyword] = React.useState('')
  const [hot, setHot] = React.useState<string[]>([])
  ...
  return (
    ...
    >
      {searchBar}
      {body ? (
        <div className="bl-scroll" style={{ flex: 1, overflowY: 'auto' }}>
          {body}
        </div>
      ) : (
        <VirtualList
