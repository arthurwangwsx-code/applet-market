...
**分组** 系统能力投影 ｜ **方法数** 1 ｜ **声明要求** 需要在 `manifest.permissions.capabilities` 里声明 `"notifications"`。**声明 ≠ 授权**，用户仍会被逐项询问。
...
| 方法 | 背后的宿主工具 | 档位 |
|---|---|---|
| `schedule` | `notify_schedule` | `localWrite` |

### `aibox.notifications.schedule()`

投影自宿主工具 `notify_schedule`；摘要与参数以该工具的真实定义为准。

**副作用档位** `localWrite`（本地写）— 改本机数据或系统状态；首次使用会弹一次授权。

**参数**

参数取自宿主工具 `notify_schedule` 的真实 `ToolDefinition.parametersJSON`，随宿主版本变化。
...
```js
await aibox.notifications.schedule({ title: 'Break', afterMinutes: 25 })
```
...
- **宿主变体**：Full ✅ ／ Lean ⚠️ 取决于壳是否装了通知模块
- **manifest 声明**：需声明；且**宿主必须真的装了对应模块**——工具不在场时整条命名空间不注册。
- **iOS 系统授权**：需要通知授权；用户拒绝后 `schedule` 会失败。
...
- **通知补全（`list` / `cancel` / `update`）—— P2**：现在**只有 `schedule`**，排了就撤不回、也列不出来。见 主仓库 `docs/capabilities/applet/framework-capabilities.md` §5；总账 §2 里这条标的是「⚠️ 部分」。
<!-- MANUAL:END roadmap -->
