...
export default function PlayerPage({ video, onOpen }) {
    ...
    const playable = React.useMemo(() => (media?.formats || []).filter((f) => f.playable), [media]);
    ...
    return (_jsxs("div", { className: "yt-scroll", style: { height: '100%', overflowY: 'auto', background: C.bg }, children: [stageOn ? null : (_jsxs("div", { onClick: () => playable.length && start(playable[0].id), style: {
                    ...
                    background: C.surface, overflow: 'hidden',
                            ...
                            ? '这个视频没有可播放的清晰度。'
                            : '这个视频只提供分离流（音视频分开），而当前版本没有编入合流能力，所以播不了。' })), _jsxs("div", { style: { display: 'flex', gap: SPACE.s2, marginTop: SPACE.s4 }, children: [_jsx(PrimaryButton, { onClick: () => playable.length && start(playable[0].id), disabled: busy || !playable.length, children: busy ? '正在准备…' : '播放' }), _jsx("button", { type: "button", onClick: async () => { await copyText(video.url); toast('链接已复制'); }, style: {
