...
export function setNavigationTitle(title) {
    ...
    if (!nav || typeof nav.setTitle !== 'function')
        return;
    try {
 ...
 */
export function useHostMenu(onInvoke) {
    const [declared, setDeclared] = useState(false);
    const handler = useRef(onInvoke);
    handler.current = onInvoke;
    useEffect(() => {
        const menu = bridge()?.menu;
        if (!menu || typeof menu.getState !== 'function')
            return undefined;
        let cancelled = false;
        const offs = [];
        menu.getState()
 ...
 */
export function useOverlay(onInvoke) {
    ...
    useEffect(() => {
        ...
        if (!overlay || typeof overlay.getState !== 'function')
            return undefined;
        let cancelled = false;
        const offs = [];
        overlay.getState()
    ...
    const update = useCallback((items) => {
        const api = bridge();
        const overlay = api && api.overlay;
        if (!overlay || typeof overlay.update !== 'function')
            return;
        try {
