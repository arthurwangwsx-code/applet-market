...
import type { MusicAppContext, MusicCallResult, MusicCollection, MusicItem, MusicTrack } from '../lib/types.js'

const CATEGORIES: Array<{ id: keyof LibraryGroups; icon: string; key: string }> = [
...
type LibraryData = { groups: LibraryGroups; collections: MusicItem[] }
type LibraryStatus = 'loading' | 'loaded' | 'denied' | 'noSubscription' | 'empty'
type LibraryState = LibraryData & { status: LibraryStatus }

const cache: { data: LibraryData | null } = { data: null }

export default function LibraryPage({ ctx }: { ctx: MusicAppContext }) {
  const { t, store, actions } = ctx
  const [state, setState] = React.useState<LibraryState>(() =>
    cache.data
      ? { status: 'loaded', ...cache.data }
      : { status: 'loading', groups: { albums: [], playlists: [], songs: [], artists: [] }, collections: [] },
  )
  ...
  React.useEffect(() => {
    ...
    const load = async () => {
      const [albums, playlists, songs, artists, collections] = await Promise.all([
        callMusic<Record<string, MusicItem[]>>('library', { action: 'albums', limit: 60 }),
        callMusic<Record<string, MusicItem[]>>('library', { action: 'playlists', limit: 60 }),
        callMusic<Record<string, MusicItem[]>>('library', { action: 'songs', limit: 60 }),
        callMusic<Record<string, MusicItem[]>>('library', { action: 'artists', limit: 60 }),
        callMusic<Record<string, MusicItem[]>>('playlist', { action: 'list', source: 'local' }),
      ])
      if (cancelled) return
        ...
        ? CATEGORIES.map((category) => {
            ...
            return (
              <Row
                ...
                minHeight={48}
                onClick={() =>
                  actions.navigate({ name: 'category', id: category.id, title: t(category.key), items: rows })
                }
              />
            )
          })
...
function pick(result: MusicCallResult<Record<string, MusicItem[]>>, key: string): MusicItem[] {
