 ...
 * 先停掉正在读的那句（不排队、不叠音）。
 */
export async function speak(text: string, accent: Accent = 'us', rate: 'normal' | 'slow' = 'normal'): Promise<void> {
  const bridge = api()
  const value = String(text ?? '').trim()
  if (!bridge?.tts || !value) return
  try {
...
export interface SpeechProbe {
  available: boolean
  reason: SpeechUnavailable | null
  detail: string
}

/** 不弹框、不开麦克风地探一下。不可用就把 `mic.circle` 整个按钮藏掉。 */
export async function probeSpeech(locale = 'en-US'): Promise<SpeechProbe> {
  const bridge = api()
  if (!bridge?.speech) return { available: false, reason: 'recognizerUnavailable', detail: '' }
  try {
...
export function classifySpeechReason(raw: string): SpeechUnavailable {
...
export function recognize(locale = 'en-US', maxDurationMs = 15_000) {
  const bridge = api()
  if (!bridge?.speech) return Promise.resolve({ transcript: '', cancelled: true, error: 'unavailable' })
  return bridge.speech
    ...
    .catch((error: unknown) => ({ transcript: '', cancelled: true, error: normalizeError(error).message }))
}

export async function stopRecognizing(): Promise<void> {
  const bridge = api()
  if (!bridge?.speech) return
  try {
...
export async function cancelRecognizing(): Promise<void> {
  const bridge = api()
  if (!bridge?.speech) return
  try {
...
export async function partialTranscript(): Promise<string> {
  const bridge = api()
  if (!bridge?.speech) return ''
  try {
...
export async function haptic(style: 'light' | 'medium' | 'heavy' | 'soft' | 'rigid' = 'light'): Promise<void> {
  const bridge = api()
...
export interface PhotoWords {
  words: string[]
  error: 'load' | 'unsupported' | 'empty' | null
 ...
 */
export async function lookUpFromPhoto(): Promise<PhotoWords> {
  const bridge = api()
  if (!bridge?.picker || !bridge?.photos) return { words: [], error: 'unsupported', previewURL: null }
  let handle = ''
  let url: string | null = null
  try {
    const picked = (await bridge.picker.photo({ limit: 1 })) as unknown
    const first = Array.isArray(picked) ? picked[0] : picked
    if (!first || typeof first !== 'object') return { words: [], error: 'load', previewURL: null }
    const ref = first as { handle?: string; url?: string; id?: string }
  ...
  for (const args of [{ handle }, { id: handle }, { asset: handle }]) {
    try {
      const result = (await bridge.photos.ocr(args as never)) as unknown
  ...
  if (!text) return { words: [], error: 'unsupported', previewURL: url }
  const words = tokenizeOCR(text)
  return { words, error: words.length ? null : 'empty', previewURL: url }
}

function extractOCRText(result: unknown): string {
  if (typeof result === 'string') return result
  if (!result || typeof result !== 'object') return ''
  const envelope = result as { text?: unknown; details?: unknown; ok?: unknown }
 ...
 */
export async function shareWordContext(seed: string): Promise<boolean> {
  const bridge = api()
  if (!bridge?.chat || typeof bridge.chat.shareContext !== 'function') return false
  try {
