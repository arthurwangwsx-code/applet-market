...
import type { ActionName } from '@aibox/applet-sdk'
...
type SchemaValue = string | number | boolean | string[] | SchemaObject | SchemaValue[]
interface SchemaObject {
  [key: string]: SchemaValue
}

const obj = (properties: Record<string, SchemaObject>, required: string[] = []): string =>
  JSON.stringify({
    type: 'object',
    properties,
...
const str = (description: string, extra: SchemaObject = {}): SchemaObject => ({ type: 'string', description, ...extra })
const num = (description: string, extra: SchemaObject = {}): SchemaObject => ({ type: 'number', description, ...extra })
const enumOf = (values: string[], description: string, extra: SchemaObject = {}): SchemaObject => ({
  type: 'string',
  enum: values,
  description,
  ...extra,
...
/** 读型工具的公共字段。 */
const read = { effect: 'read', readOnly: true, idempotent: true } as const
/** 写型工具的公共字段。 */
const write = { effect: 'write', readOnly: false, destructive: false } as const
...
export const TOOL_DEFS: ToolDefinition[] = [
  {
    ...
    displayName: '查行情',
    summary:
      'Get delayed quotes for stocks, funds or indices — price, change, open/high/low, turnover, P/E and order book.',
  ...
  },
  {
    ...
    displayName: '搜索标的',
    summary:
      'Search instruments by name or code across A-shares, Hong Kong, US and mutual funds. Use this whenever the user names a company instead of a code.',
  ...
  },
  {
    ...
    displayName: '取K线',
    summary:
      'Fetch OHLCV candles plus locally computed indicator values (MA / MACD / KDJ / BOLL) and a window summary.',
  ...
  },
  {
    ...
    displayName: '看持仓',
    summary:
      'Value the paper portfolio (holdings with market value, cost, unrealized and daily P&L) or list recent trades. Positions with a missing quote or FX rate are reported as incomplete rather than guessed.',
  ...
  },
  {
    ...
    displayName: '模拟买卖',
    summary:
      'Record a simulated buy or sell in a paper account. Cross-currency trades lock the exchange rate at execution and fail when no rate is available.',
  ...
  },
  {
    ...
    displayName: '管理账户',
    summary:
      'List, create, rename, reset, archive or delete paper accounts, and record cash flows (deposit / withdrawal / dividend / interest / tax / fee / adjustment).',
    keywords: ['account', 'cash', 'deposit', 'withdraw', '账户', '入金', '出金', '现金'],
    ...write,
    idempotent: false,
    input: obj({
      action: enumOf(
        ['list', 'create', 'copy', 'rename', 'update', 'cashflow', 'reset', 'archive', 'unarchive', 'delete'],
        'Default list.',
      ),
      ...
      cashflow_type: enumOf(
        ['deposit', 'withdrawal', 'dividend', 'interest', 'tax', 'fee', 'adjustment'],
        'Cash flow kind.',
      ),
  ...
  },
  {
    ...
    displayName: '到价提醒',
    summary:
      'List, set, remove, enable or disable price alerts. Alerts are checked while quotes refresh in the foreground — this container has no background wake-up.',
  ...
  },
  {
    ...
    displayName: '查财务',
    summary:
      'Fetch up to eight periods of key financial indicators (revenue, net profit, ROE, gross margin, EPS, BPS with YoY) plus the current P/E and P/B.',
  ...
  },
  {
    ...
    displayName: '组合绩效',
    summary:
      'Compute portfolio performance: total and annualized return, max drawdown, volatility, Sharpe and win rate. Refreshes quotes and records a daily snapshot first; drawdown/volatility/Sharpe need at least two daily snapshots.',
  ...
  },
  {
    ...
    displayName: '组合诊断',
    summary:
      'Score portfolio structure (0..100) and list risk flags such as over-concentration, too few holdings or an excessive cash ratio. Refuses to score when any quote or FX rate is missing.',
  ...
  },
  {
    ...
    displayName: '资金流向',
    summary:
      'Fetch daily net capital flow for an A-share stock split by order size (main / super / large / medium / small), in CNY. A-shares only.',
  ...
  },
  {
    ...
    displayName: '板块行情',
    summary:
      "List industry or concept sectors ranked by change or main capital inflow, or list a sector's constituents. Needs a mainland-China network connection; returns empty otherwise.",
  ...
  },
  {
    ...
    displayName: '资金排行',
    summary:
      'Rank the whole A-share market by main capital net inflow or outflow. Needs a mainland-China network connection.',
  ...
  },
  {
    ...
    displayName: '龙虎榜',
    summary:
      'Fetch the latest Dragon-Tiger list (stocks with abnormal trading), deduplicated by code, with net buy amounts and the listing reason.',
  ...
  },
  {
    ...
    displayName: '市场情绪',
    summary:
      "Fetch today's market breadth: limit-up count, limit-down count, broken-board count, seal rate and the longest consecutive limit-up streak. Needs a mainland-China network connection.",
  ...
  },
  {
    ...
    displayName: '选股扫描',
    summary:
      'Scan the top A-share universe and filter locally by P/E, P/B, change, market cap or industry. Needs a mainland-China network connection.',
  ...
  },
  {
    ...
    displayName: '策略回测',
    summary:
      'Run a local historical backtest (buy & hold, or MA5/MA20 crossover) and report return, annualized return, max drawdown, volatility, Sharpe and win rate.',
  ...
  },
  {
    ...
    displayName: '定投模拟',
    summary:
      'Simulate a dollar-cost-averaging plan over historical prices and report invested amount, current value, average cost and the lump-sum comparison.',
  ...
  },
  {
    ...
    displayName: '横向对比',
    summary:
      'Compare several instruments (price, change, P/E, P/B, market cap) or several paper accounts (total value, P&L, return) side by side.',
  ...
  },
  {
    ...
    displayName: '再平衡提案',
    summary:
      'Propose (never execute) the trades needed to move a paper account toward target weights. Weights are normalized to 100%; nothing is ordered.',
