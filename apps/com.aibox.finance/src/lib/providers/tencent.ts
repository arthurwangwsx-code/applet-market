...
import type { Candle, CandlePeriod, FinSymbol, PriceLevel, Quote, SearchItem } from '../types.js'

const REFERER = { Referer: 'https://gu.qq.com' }

function pick(parts: string[], index: number): number | null {
...
function levels(parts: string[], priceStart: number): PriceLevel[] {
...
export function parseQuoteLine(body: string, symbol: FinSymbol): Quote | null {
 ...
 * 网络/解析失败一律返回空字典，不抛。
 */
export async function fetchQuotes(symbols: FinSymbol[]): Promise<Record<string, Quote>> {
  const usable = symbols
    .map((symbol) => ({ symbol, code: tencentQuoteCode(symbol) }))
    .filter((row): row is { symbol: FinSymbol; code: string } => typeof row.code === 'string' && row.code.length > 0)
  ...
  for (const line of String(result.body).split(';')) {
    ...
    if (!responseCode || responseBody === undefined) continue
...
const PERIOD_TOKEN: Partial<Record<CandlePeriod, string>> = { '5m': 'm5', '15m': 'm15', '30m': 'm30', '60m': 'm60' }

/** 每行 `[date, open, close, high, low, volume]`——**close 在第 3 位、high 在第 4 位**。 */
function parseCandleRows(rows: unknown): Candle[] {
  const out: Candle[] = []
...
export async function fetchDailyCandles(
  symbol: FinSymbol,
  period: Extract<CandlePeriod, 'day' | 'week' | 'month'>,
  adjust: string,
  count: number,
): Promise<Candle[]> {
  ...
  const result = await coalesce(`tencent:k:${code}:${period}:${adjust}:${n}`, () =>
    getJSON<TencentKlineResponse>(url, { headers: REFERER }),
  )
  if (!result.ok) return []
...
export async function fetchMinuteCandles(
  symbol: FinSymbol,
  period: Extract<CandlePeriod, '5m' | '15m' | '30m' | '60m'>,
  count: number,
): Promise<Candle[]> {
  const code = tencentKlineCode(symbol)
  ...
  const result = await coalesce(`tencent:m:${code}:${token}:${n}`, () =>
    getJSON<TencentKlineResponse>(url, { headers: REFERER }),
  )
  if (!result.ok) return []
  const bucket = result.body && result.body.data ? result.body.data[code] : null
 ...
 */
export async function search(query: unknown): Promise<SearchItem[]> {
  ...
  if (matchedBody === undefined) return out
