...
export default function SearchPage({ onOpen }) {
    ...
    const searchBar = (_jsxs("div", { style: { padding: SPACE.s3, background: C.bg, display: 'flex', gap: SPACE.s2 }, children: [_jsx("input", { value: keyword, onChange: (e) => setKeyword(e.target.value), onKeyDown: (e) => {
                    if (e.key === 'Enter') {
    ...
    if (state === 'loading')
        ...
        body = (_jsxs("div", { style: { padding: SPACE.s4 }, children: [_jsx("div", { style: { fontSize: 13, color: C.sub, marginBottom: SPACE.s3 }, children: "\u70ED\u641C" }), _jsx("div", { style: { display: 'flex', flexWrap: 'wrap', gap: SPACE.s2 }, children: hot.map((word) => (_jsx("button", { type: "button", onClick: () => {
                            setKeyword(word);
                            run(word);
                        }, style: {
                            border: 'none',
