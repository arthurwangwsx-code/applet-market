...
import type { Category, FlowKind } from '../types.js'
import type { LedgerStore } from './store.js'

type EntryDirection = FlowKind | 'transfer'
...
export function lastAccountID(store: LedgerStore, direction: EntryDirection): string | null {
  const key =
    direction === 'income'
  ...
  if (stored) {
    ...
    if (account && !account.isArchived) return account.id
  }
  const fallback = store.defaultAccount()
  return fallback ? fallback.id : null
}

export function rememberAccount(direction: EntryDirection, accountID: string | null): void {
  if (!accountID) return
  const key =
    direction === 'income'
      ? PREF_KEYS.lastIncomeAccountID
...
export function recentCategories(kind: FlowKind): string[] {
...
export function rememberCategory(kind: FlowKind, categoryID: string | null): void {
