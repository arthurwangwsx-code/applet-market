    ...
    control(action, taskId) {
        const apply = (t) => {
            if (action === 'pause' && (t.state === 'running' || t.state === 'queued'))
                t.state = 'paused';
            else if (action === 'resume' && (t.state === 'paused' || t.state === 'failed'))
                t.state = 'running';
            else if (action === 'cancel' && !['completed', 'failed', 'cancelled'].includes(t.state))
                t.state = 'cancelled';
        };
        ...
        if (fakeTasks.some((t) => t.state === 'running') && !fakeTimer)
            fakeTimer = setInterval(tickFakes, 400);
        notifyFakeListeners();
        return true;
    },
