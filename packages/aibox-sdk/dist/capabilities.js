...
export function isAvailable(namespace, method) {
    return available(namespace, method);
}
/** 一次判多条，全部可用才返回 true。 */
export function allAvailable(...namespaces) {
    return namespaces.every((name) => available(name));
}
/** 可用就跑，不可用返回 `fallback`——**不抛**。用于「有就用、没有就退化」的增强路径。 */
export async function ifAvailable(namespace, method, run, fallback) {
...
export function requireAvailable(namespace, method) {
    if (available(namespace, method))
        return;
 ...
 */
export async function explainAccess(target) {
    const host = bridge();
...
/** 一次拿到「注册 + 授权」两层结论。 */
export async function probe(namespace, method) {
