...
import type { PortfolioDiagnosis } from './portfolio.js'
import type {
  Account,
  AccountSnapshot,
  AccountValuation,
...
export type AccountResolution =
  ...
  | { ok: false; error: 'noAccount' | 'ambiguous' | 'notFound'; candidates: AccountDescription[] }

export interface LedgerBuyInput {
  accountID: string
  symbol: string
  name?: string
  market?: Market | null
...
export interface LedgerSellInput {
  accountID: string
  symbol: string
  quantity: number
  price: number
...
export interface LedgerCashFlowInput {
  accountID: string
  kind: CashFlowKind
  amountMinor: number
  note?: string
...
export class Ledger {
  ...
  async load(): Promise<void> {
    const saved = await storage.get<LedgerState>(LEDGER_KEY)
  ...
  async commit(next: LedgerState): Promise<boolean> {
  ...
  accountByID(id: string | null | undefined): Account | null {
   ...
   */
  resolveAccount(query?: unknown): AccountResolution {
    ...
    if (exact.length === 1) return { ok: true, account: exact[0] ?? null }
    ...
    if (currency) {
      ...
      if (matched.length === 1) return { ok: true, account: matched[0] ?? null }
      if (matched.length > 1) {
    ...
    if (fuzzy.length === 1) return { ok: true, account: fuzzy[0] ?? null }
  ...
  positionsOf(accountID: string): Position[] {
    return this.positions.filter((row) => row.accountID === accountID)
  }

  openPositionsOf(accountID: string): Position[] {
    return this.positionsOf(accountID).filter((row) => row.quantity > 0)
  }

  ordersOf(accountID: string, limit = 20): TradeOrder[] {
    return this.orders
  ...
  async buy({
    ...
    const position =
      this.positions.find((row) => row.accountID === accountID && row.instrumentSymbol === symbol) || null
    let result
    try {
      result = applyBuy({
        account,
  ...
  async sell({ accountID, symbol, quantity, price, fxRate: rate, feeMinor, note, source }: LedgerSellInput) {
    if (!this.storageHealthy) return { ok: false, error: 'storageUnavailable' }
    const account = this.accountByID(accountID)
    const position =
      this.positions.find((row) => row.accountID === accountID && row.instrumentSymbol === symbol) || null
  ...
  async addCashFlow({ accountID, kind, amountMinor, note, source }: LedgerCashFlowInput) {
    if (!this.storageHealthy) return { ok: false, error: 'storageUnavailable' }
    const account = this.accountByID(accountID)
    let result
    try {
  ...
  async updateAccount(id: string, patch: Partial<Omit<Account, 'id'>>): Promise<{ ok: boolean; error?: string }> {
    if (!this.storageHealthy) return { ok: false, error: 'storageUnavailable' }
  ...
  async copyAccount(id: string, name?: string) {
  ...
  performance(accountID: string): PerformanceResult {
    return performance({
  ...
  allocation(valuation: AccountValuation): AllocationRow[] {
    return allocation(valuation.rows)
  }

  diagnose(valuation: AccountValuation, perf: PerformanceResult | null | undefined): PortfolioDiagnosis {
    return diagnose({ valuation, perf })
  }

  fxRateFor(fromCurrency: Currency, accountCurrency: Currency, fxToCNY: FXMap | null | undefined): number | null {
    return fxRate(fromCurrency, accountCurrency, fxMapFor(accountCurrency, fxToCNY))
  }

  /** §10.7：**仅当估值完整才写**快照；按天归一，同日覆盖。 */
  async writeSnapshot(valuation: AccountValuation, when?: number) {
...
export function describeAccount(account: Account): AccountDescription {
