...
import type { DatabaseOperation, LedgerTables, LedgerTransaction, TableName } from '../types.js'

function api(): typeof aibox.db | null {
...
export const txDocID = (monthKey: number): string => `m${monthKey}`

// MARK: - 底层调用（**不吞异常**）

async function queryAll(collection: string): Promise<Array<Record<string, unknown>>> {
  const db = api()
  if (!db) throw new Error('aibox/db-unavailable')
...
async function put(collection: string, id: string, rows: unknown[]): Promise<void> {
  const db = api()
  if (!db) throw new Error('aibox/db-unavailable')
...
export async function loadAll(): Promise<{ tables: LedgerTables; tx: Record<number, LedgerTransaction[]> }> {
  ...
  for (const doc of await queryAll(TABLES)) {
    if (typeof doc._id === 'string' && isTableName(doc._id)) {
      setTable(tables, doc._id, (Array.isArray(doc.rows) ? doc.rows : []) as LedgerTables[typeof doc._id])
    }
  }
  const tx: Record<number, LedgerTransaction[]> = {}
  for (const doc of await queryAll(TX)) {
    if (typeof doc._id === 'string' && doc._id.startsWith('m')) {
      tx[Number(doc._id.slice(1))] = (Array.isArray(doc.rows) ? doc.rows : []) as LedgerTransaction[]
    }
  }
  return { tables, tx }
}
...
async function replayPendingWAL() {
  ...
  if (!pending || operations.length === 0) {
    if (pending) await drop(WAL, PENDING_ID)
    return
  }
...
async function applyOps(ops: readonly DatabaseOperation[]): Promise<void> {
  for (const op of ops) {
    if (op.del) await drop(op.c, op.id)
    else await put(op.c, op.id, op.rows)
  }
}
 ...
 */
export async function commit(ops: readonly DatabaseOperation[]): Promise<void> {
...
const TABLE_NAME_SET = new Set<string>([
  'accounts',
  'categories',
  'currencies',
  'budgets',
...
function setTable<K extends TableName>(tables: LedgerTables, name: K, rows: LedgerTables[K]): void {
  tables[name] = rows
}

function parseOperations(values: unknown[]): DatabaseOperation[] {
  const operations: DatabaseOperation[] = []
  for (const value of values) {
    if (!value || typeof value !== 'object') continue
    const row = value as Record<string, unknown>
    if ((row.c !== 'tables' && row.c !== 'tx') || typeof row.id !== 'string') continue
    if (row.del === true && row.c === 'tx') {
      operations.push({ c: 'tx', id: row.id, del: true })
    } else if (Array.isArray(row.rows)) {
      operations.push({ c: row.c, id: row.id, rows: row.rows as LedgerTransaction[] })
    }
  }
  return operations
}
