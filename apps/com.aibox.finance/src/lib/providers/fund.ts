...
import type { Candle, Quote, SearchItem } from '../types.js'
...
export async function fetchEstimate(code: string): Promise<Quote | null> {
...
export async function fetchEstimates(codes: string[]): Promise<Record<string, Quote>> {
  const rows = await runPool(codes, 4, (code) => fetchEstimate(code))
  const out: Record<string, Quote> = {}
  for (const row of rows) if (row) out[row.symbol] = row
  return out
}
...
let catalog: FundCatalogRow[] | null = null
let catalogTruncated = false
let catalogPromise: Promise<FundCatalogRow[]> | null = null

export function catalogState(): { loaded: boolean; count: number; truncated: boolean } {
 ...
 */
export async function loadCatalog(): Promise<FundCatalogRow[]> {
...
export async function search(query: unknown, limit = 20): Promise<SearchItem[]> {
...
interface FundHistoryResponse {
  Data?: { LSJZList?: Array<{ DWJZ?: unknown; FSRQ?: unknown }> }
}

export async function fetchNavHistory(code: string, count = 160): Promise<Candle[]> {
  ...
  const result = await coalesce(`fund:nav:${code}:${size}`, () =>
    getJSON<FundHistoryResponse>(url, { headers: EM_REFERER }),
  )
  if (!result.ok) return []
