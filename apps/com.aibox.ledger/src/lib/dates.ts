...
export function dayStart(value: number | Date): number {
...
/** 月键 `YYYYMM` 整数。 */
export function monthKeyOf(value: number | Date): number {
  const date = new Date(value)
...
export function monthStart(monthKey: number): number {
...
export function monthEnd(monthKey: number): number {
  const year = Math.trunc(monthKey / 100)
  const month = monthKey % 100
  return new Date(month === 12 ? year + 1 : year, month === 12 ? 0 : month, 1, 0, 0, 0, 0).getTime()
}

export function addMonths(monthKey: number, delta: number): number {
  const year = Math.trunc(monthKey / 100)
...
export function daysInMonth(monthKey: number): number {
 ...
 *  - 今天 ≥ 该月结束（查过去月）→ 0
 */
export function daysRemainingInMonth(monthKey: number, now = Date.now()): number {
  const today = dayStart(now)
  const start = monthStart(monthKey)
  const end = monthEnd(monthKey)
  if (today < start) return daysInMonth(monthKey)
...
export function monthTitle(monthKey: number, locale: string): string {
...
export function dayHeaderDate(value: number | Date, locale: string): string {
  return formatter(locale, { month: 'long', day: 'numeric', weekday: 'short' }).format(new Date(value))
}

/** 短日期 `Md`（账户详情、导入预览、下钻列表）。 */
export function shortDate(value: number | Date, locale: string): string {
  return formatter(locale, { month: 'numeric', day: 'numeric' }).format(new Date(value))
}

/** 项目日期区间用的 `MMMd`。 */
export function mediumDayDate(value: number | Date, locale: string): string {
...
export function isoTimestamp(value: number | Date): string {
...
export function parseTimestamp(text: string, fallback: number | null): number | null {
