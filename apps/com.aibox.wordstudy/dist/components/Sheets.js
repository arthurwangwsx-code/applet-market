...
export function AiCompanion(props) {
    ...
    return (_jsxs(Sheet, { palette: palette, open: props.open, onClose: props.onClose, children: [_jsxs("div", { style: { display: 'flex', alignItems: 'center', padding: SPACE.s4 }, children: [_jsxs("div", { style: { display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 15, fontWeight: 500, color: palette.ink }, children: [_jsx(Icon, { name: "sparkles", size: 15, color: palette.accent }), " ", t('companionTitle')] }), _jsx("div", { style: { flex: 1 } }), _jsx("button", { type: "button", onClick: () => void shareWordContext(`Tell me more about the English word "${props.word}".`), style: { border: 'none', background: 'transparent', color: palette.accent, fontSize: 12, cursor: 'pointer' }, children: t('sendToChat') }), _jsx("button", { type: "button", onClick: props.onClose, style: { border: 'none', background: 'transparent', color: palette.accent, fontSize: 15, cursor: 'pointer', marginLeft: SPACE.s3 }, children: t('done') })] }), _jsx("div", { style: { padding: `0 ${SPACE.s4}px`, display: 'flex', flexWrap: 'wrap', gap: 6 }, children: chips.map((chip) => (_jsx("button", { type: "button", onClick: () => void send(chip.seed), style: {
                        ...
                        }, children: message.text || '…' }, index)))] }), _jsxs("div", { style: { display: 'flex', gap: SPACE.s2, padding: `0 ${SPACE.s4}px ${SPACE.s4}px` }, children: [_jsx("input", { value: input, onChange: (event) => setInput(event.target.value), onKeyDown: (event) => { if (event.key === 'Enter')
                            void send(input); }, placeholder: t('companionPlaceholder'), style: {
