...
export async function actionSheet(
...
export async function confirmDestructive(title: string, confirmTitle: string, cancelTitle: string): Promise<boolean> {
  const bridge = typeof window !== 'undefined' ? window.aibox : undefined
...
export async function confirmAlert(title: string, message: string): Promise<void> {
  const bridge = typeof window !== 'undefined' ? window.aibox : undefined
  if (!bridge?.ui) return
  try {
...
export async function promptText(title: string, defaultValue: string): Promise<string | null> {
  const bridge = typeof window !== 'undefined' ? window.aibox : undefined
...
export async function shareClipAudio(memo: Memo): Promise<void> {
  const bridge = typeof window !== 'undefined' ? window.aibox : undefined
