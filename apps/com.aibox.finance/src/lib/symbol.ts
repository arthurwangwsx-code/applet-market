...
import type { Currency, FinSymbol, Market } from './types.js'

export const MARKETS: Market[] = ['ashare', 'hk', 'us', 'fund']
...
/** 价格小数位：基金 4 位，其余 2 位。 */
export function decimalsFor(market: Market): number {
  return market === 'fund' ? 4 : 2
}

function makeSymbol(market: Market, code: string, exchange: string | null): FinSymbol {
  return { market, code, exchange: exchange || null }
}

/** A 股交易所推断：6/9→sh，0/2/3→sz，4/8→bj，其余 sh。 */
export function exchangeForAShare(code: unknown): 'sh' | 'sz' | 'bj' {
...
/** 港股代码补 0 到 5 位。 */
export function padHK(code: unknown): string {
  const digits = String(code).replace(/\D/g, '')
  return digits.padStart(5, '0').slice(-5)
}

export function canonicalOf(symbol: FinSymbol | null | undefined): string {
  if (!symbol) return ''
 ...
 */
export function parseStrict(input: unknown): FinSymbol | null {
  const raw = String(input || '').trim()
  if (!raw) return null
  const lower = raw.toLowerCase()

  let match = /^(sh|sz|bj)(\d{6})$/.exec(lower)
  if (match) return makeSymbol('ashare', match[2]!, match[1]!)

  match = /^of(\d{6})$/.exec(lower)
  if (match) return makeSymbol('fund', match[1]!, null)

  match = /^fund(\d+)$/.exec(lower)
  if (match) return makeSymbol('fund', match[1]!.padStart(6, '0'), null)

  match = /^hk(\d+)$/.exec(lower)
  if (match) return makeSymbol('hk', padHK(match[1]!), null)

  match = /^us(.+)$/.exec(raw)
  if (match?.[1] && /^[A-Za-z0-9.\-]+$/.test(match[1])) return makeSymbol('us', match[1].toUpperCase(), null)

  if (/^\d+$/.test(raw)) {
    if (raw.length === 6) return makeSymbol('ashare', raw, exchangeForAShare(raw))
    if (raw.length === 5) return makeSymbol('hk', raw, null)
    if (raw.length === 4) return makeSymbol('hk', padHK(raw), null)
  }
  return null
}
 ...
 * 只有「用户明确在输代码」的路径才该用它。
 */
export function parseSymbol(input: unknown): FinSymbol | null {
  const strict = parseStrict(input)
  if (strict) return strict
  const raw = String(input || '').trim()
  if (raw && /^[A-Za-z.]+$/.test(raw) && raw.length <= 8) return makeSymbol('us', raw.toUpperCase(), null)
  return null
}

/** canonical 串 → symbol；解析不出来时返回 null。 */
export function symbolFromCanonical(canonical: string): FinSymbol | null {
  return parseStrict(canonical)
}

export function currencyOf(symbol: FinSymbol | null | undefined): Currency {
...
export function tencentQuoteCode(symbol: FinSymbol): string | null {
  switch (symbol.market) {
    case 'ashare':
      return `${symbol.exchange || exchangeForAShare(symbol.code)}${symbol.code}`
    case 'hk':
...
export function tencentKlineCode(symbol: FinSymbol): string | null {
  switch (symbol.market) {
    case 'ashare':
      return `${symbol.exchange || exchangeForAShare(symbol.code)}${symbol.code}`
    case 'hk':
...
export function sinaQuoteCode(symbol: FinSymbol): string | null {
  switch (symbol.market) {
    case 'ashare':
      return `${symbol.exchange || exchangeForAShare(symbol.code)}${symbol.code}`
    case 'hk':
...
export function secuCode(symbol: FinSymbol): string | null {
...
export function secid(symbol: FinSymbol): string | null {
  if (symbol.market !== 'ashare') return null
  return `${(symbol.exchange || exchangeForAShare(symbol.code)) === 'sh' ? 1 : 0}.${symbol.code}`
}

/** 从裸 A 股代码按首位数字推 secid（行业页资金榜用）。 */
export function secidFromCode(code: string, marketFlag?: string | number): string {
  if (marketFlag === 1 || marketFlag === '1') return `1.${code}`
  if (marketFlag === 0 || marketFlag === '0') return `0.${code}`
  return `${exchangeForAShare(code) === 'sh' ? 1 : 0}.${code}`
}

// —— 固定指数表（规格 §2.1，顺序不可配置）——

export const INDEX_ROWS: Array<{ canonical: string; key: string }> = [
 ...
 */
export function indexSymbol(canonical: string): FinSymbol | null {
  if (canonical.startsWith('hk')) return makeSymbol('hk', canonical.slice(2), null)
  if (canonical.startsWith('us')) return makeSymbol('us', canonical.slice(2), null)
  return parseStrict(canonical)
}

/** 自选/详情通吃的解析入口：先当指数，再 strict。 */
export function resolveSymbol(canonical: string): FinSymbol | null {
  const row = INDEX_ROWS.find((entry) => entry.canonical === canonical)
  if (row) return indexSymbol(canonical)
