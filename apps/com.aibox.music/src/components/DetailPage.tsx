...
import type { CategoryRoute, MusicAppContext, MusicCollection, MusicTrack } from '../lib/types.js'

type DetailState = { loading: boolean; tracks: MusicTrack[]; albums: MusicCollection[]; failed?: boolean }

export function CollectionDetail({ ctx, item }: { ctx: MusicAppContext; item: MusicCollection }) {
  const { t, actions, store } = ctx
  const [state, setState] = React.useState<DetailState>({ loading: true, tracks: [], albums: [] })

  React.useEffect(() => {
    let cancelled = false
    const load = async () => {
      const kind = item.type === 'artist' ? 'artist' : item.type === 'playlist' ? 'playlist' : 'album'
      const result = await callMusic<{ tracks?: MusicTrack[]; albums?: MusicCollection[] }>('get', {
        id: item.musicItemId,
        kind,
    ...
    <div className="mu-scroll" aria-label="audioCollectionDetail">
      ...
      >
        ...
        {!isArtist && item.artist ? (
          <span className="mu-clamp-1" style={{ fontSize: 14, color: C.muted }}>
            {item.artist}
          </span>
        ) : null}
        {item.url ? (
        ...
        {!isArtist ? (
          <div style={{ display: 'flex', gap: 10, width: '100%', paddingTop: 4 }}>
            <HeaderButton
              ...
              onClick={() => {
                if (firstTrack) actions.playTrack(firstTrack, tracks)
              }}
            />
            <HeaderButton
              icon="shuffle"
...
export function CategoryList({ ctx, route }: { ctx: MusicAppContext; route: CategoryRoute }) {
  const { actions } = ctx
  const items = route.items || []
  const isSongs = route.id === 'songs'
  const songItems = items as MusicTrack[]
  const collectionItems = items as MusicCollection[]
  return (
    <div className="mu-scroll">
      {(isSongs ? songItems : collectionItems).map((item, index) =>
        isSongs ? (
          <SongRow
            key={item.musicItemId || index}
            track={item}
            onClick={() => actions.playTrack(item, songItems)}
            onLongPress={() => actions.trackMenu(item, { group: songItems })}
          />
        ) : (
          <CollectionRow
            key={item.musicItemId || index}
            item={item as MusicCollection}
            onClick={() => actions.openCollection(item as MusicCollection)}
            onLongPress={() => actions.collectionMenu(item as MusicCollection)}
          />
        ),
      )}
      <div style={{ height: 24 }} />
