...
import type { AISession, DetailRoute, FinanceContext, FinSymbol, InstrumentPanelRoute, Quote } from './lib/types.js'
...
type SimpleSheetName = 'search' | 'accounts' | 'cashflow' | 'history' | 'groups'
type SheetState =
  | { name: SimpleSheetName }
  | ({ name: 'trade' | 'alert' | 'strategy'; symbolName: string } & Omit<InstrumentPanelRoute, 'name'>)
...
function useThemeSetup() {
  React.useEffect(() => {
    if (document.getElementById('__finance_css__')) return
...
export default function App() {
  ...
  const refs = React.useRef<AppServices | null>(null)
  ...
  const [tab, setTab] = React.useState<TabID>('markets')
  // 子页栈 = 宿主原生页栈的镜像（框架资产 `aibox/ui`）：进详情走 `aibox.navigation.push`，
  // 返回一律经 popstate 回来，于是最左缘左滑是**系统自己的** interactive pop。
  const subpages = useSubpageStack<DetailRoute>({ pathFor: routePath, titleFor: (row) => row.title })
  const [sheet, setSheet] = React.useState<SheetState | null>(null)
  const [aiSession, setAISession] = React.useState<AISession | null>(null)
  ...
  React.useEffect(() => {
    ...
    const boot = async () => {
      ...
      if (cancelled) return
      if (saved) quotes.hydrate(saved.rows, saved.lastUpdated)
  ...
  React.useEffect(() => {
    ...
    const probe = async () => {
      ...
      if (!cancelled) setHasAI(!!info.available && capabilities.chat)
    }
    probe()
    return () => {
      cancelled = true
  ...
  const refresh = React.useCallback(
    async (force: boolean): Promise<void> => {
      if (busy.current) return // 并发保护：正在刷新时直接返回
      busy.current = true
      setRefreshing(true)
      try {
        const symbols = [
        ...
        for (const hit of fired) {
          if (!store.settings.notifyAlerts) break
          ...
          await scheduleNotification({
            ...
            body: t(
              isUpwardCondition(hit.alert.conditionRaw) ? 'finance.notify.above' : 'finance.notify.below',
              current,
              target,
            ),
            afterMinutes: 0,
  ...
  React.useEffect(() => {
    ...
    refresh(false)
    if (!store.settings.autoRefresh) return undefined
  ...
  const openAIRef = React.useRef<(() => void) | null>(null)
    ...
    const wire = async () => {
      ...
      if (api && api.tabs && typeof api.tabs.getState === 'function') {
        try {
          ...
          if (!cancelled && state && state.rendered) {
            ...
            if (isTabID(state.selected)) setTab(state.selected)
          }
        } catch (error) {
          /* 宿主没这能力：留给自绘 TabBar */
        }
        offs.push(
          onNamespaceEvent('tabs', 'changed', (payload) => {
            if (!isRecord(payload)) return
            // `rendered` 会**在挂载之后翻转**（形态切换、控制器重建都会重发 changed）。
            // 只在启动那一刻判断一次，自绘 TabBar 就会永远缺席或永远多一条。
            const rendered = payload.rendered !== false
            setShell((current) =>
              current.tabsRendered === rendered ? current : { ...current, tabsRendered: rendered },
            )
      ...
      if (api && api.toolbar && typeof api.toolbar.getState === 'function') {
        try {
          ...
          if (!cancelled && state) setShell((current) => ({ ...current, toolbarRendered: state.rendered !== false }))
        } catch (error) {
          /* 同上 */
        }
        offs.push(
          onNamespaceEvent('toolbar', 'invoke', (payload) => {
            if (!isRecord(payload)) return
            if (payload.id === 'search' && openSearchRef.current) openSearchRef.current()
            if (payload.id === 'ai' && openAIRef.current) openAIRef.current()
          }),
        )
      }
    }
  ...
  const askAI = React.useCallback(
    async ({ identity, seed }: { identity: string; seed: string }) => {
  ...
  const ctx = React.useMemo<FinanceContext>(
    () => ({
      t,
      locale,
      store,
      ...
      actions: {
        ...
        openTrade: (canonical: string, symbol: FinSymbol, name: string) =>
          setSheet({ name: 'trade', canonical, symbol, symbolName: name }),
        openAlert: (canonical: string, symbol: FinSymbol, name: string) =>
          setSheet({ name: 'alert', canonical, symbol, symbolName: name }),
        openStrategy: (canonical: string, symbol: FinSymbol, name: string) =>
          setSheet({ name: 'strategy', canonical, symbol, symbolName: name }),
        selectAccount: (id: string) => setAccountID(id),
        toggleWatch: (canonical: string, name?: string) =>
          store.isWatched(canonical) ? store.removeWatch(canonical) : store.addWatch(canonical, { name }),
        removeWatch: (canonical: string) => store.removeWatch(canonical),
        askAI,
        askAboutSymbol: (canonical: string, name: string, quote: Quote | null) =>
          setAISession({
  ...
  return (
    <div className="fin-root">
            ...
            !route ? (
              <>
                ...
                <ToolbarButton
                  ...
                  label={t('finance.search.title')}
                  onClick={ctx.actions.openSearch}
                />
              </>
            ) : null
          }
      ...
      {!store.storageHealthy ? <StorageBanner text={t('finance.storage.banner')} /> : null}

      {!ready ? (
        <div className="fin-scroll" />
      ) : route ? (
        route.name === 'detail' ? (
          <DetailPage ctx={ctx} route={route} />
        ) : null
      ) : (
        <>
          {tab === 'markets' ? <WatchlistPage ctx={ctx} /> : null}
          {tab === 'industry' ? <IndustryPage ctx={ctx} /> : null}
          {tab === 'portfolio' ? <PortfolioPage ctx={ctx} /> : null}
          {tab === 'settings' ? <SettingsPage ctx={ctx} /> : null}
        </>
      )}

      {!shell.tabsRendered ? (
        <TabBar items={TABS.map((row) => ({ ...row, title: t(row.titleKey) }))} selected={tab} onSelect={selectTab} />
      ) : (
        <div style={{ height: 'env(safe-area-inset-bottom)', flex: '0 0 auto' }} />
      )}

      <SearchPage ctx={ctx} visible={sheetName === 'search'} onClose={closeSheet} />
      <AccountsSheet ctx={ctx} visible={sheetName === 'accounts'} onClose={closeSheet} />
      <CashFlowSheet ctx={ctx} visible={sheetName === 'cashflow'} onClose={closeSheet} />
      <HistorySheet ctx={ctx} visible={sheetName === 'history'} onClose={closeSheet} />
      <GroupsSheet ctx={ctx} visible={sheetName === 'groups'} onClose={closeSheet} />
      <TradePanel
        ctx={ctx}
        visible={sheetName === 'trade'}
        onClose={closeSheet}
        route={sheet && sheet.name === 'trade' ? { ...sheet, name: sheet.symbolName } : null}
      />
      <AlertPanel
        ctx={ctx}
        visible={sheetName === 'alert'}
        onClose={closeSheet}
        route={sheet && sheet.name === 'alert' ? { ...sheet, name: sheet.symbolName } : null}
      />
      <StrategyPage
        ctx={ctx}
        visible={sheetName === 'strategy'}
        onClose={closeSheet}
        route={sheet && sheet.name === 'strategy' ? { ...sheet, name: sheet.symbolName } : null}
      />
...
function routePath(route: DetailRoute): string {
