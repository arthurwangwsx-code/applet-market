...
export function shortStamp(millis: number) {
 ...
 */
export function relative(millis: number, locale: SupportedLocale, now = Date.now()) {
  if (!millis || !Number.isFinite(millis)) return ''
  const delta = Math.abs(now - millis)
  if (delta >= WEEK_MS) return shortStamp(millis)
  if (delta < 60 * 1000) return translate(locale, 'news.x.justNow')
...
function formatRelative(value: number, unit: Intl.RelativeTimeFormatUnit, locale: SupportedLocale) {
  try {
    const formatter = new Intl.RelativeTimeFormat(bcp47(locale), { numeric: 'always', style: 'short' })
    return formatter.format(value, unit)
  } catch (error) {
    const abs = Math.abs(value)
    const label = ({ day: 'd', hour: 'h', minute: 'min', second: 's' } as Record<string, string>)[unit] ?? unit
...
export function duration(seconds: number, t: Translate) {
  const value = Number(seconds) || 0
  if (value < 1) return t('news.diagnostics.milliseconds', (value * 1000).toFixed(0))
