...
/** 明写的 ISO 代码（票面上常见的那些）。 */
const CODE_RE =
  /\b(CNY|RMB|USD|EUR|GBP|JPY|HKD|TWD|KRW|SGD|AUD|CAD|CHF|THB|MYR|PHP|IDR|VND|INR|RUB|NZD|SEK|NOK|DKK|BRL|TRY|ILS|AED|SAR|ZAR|MXN|PLN)\b/i
...
export function detectCurrency(text: string): string | null {
  ...
  for (const sym of symbols) {
    if (s.includes(sym)) return SYMBOL_TO_CODE[sym] ?? null
  }
  if (s.includes('¥') || s.includes('￥')) {
    return /[ぁ-んァ-ヶ]|円|税込|合計/.test(s) ? 'JPY' : 'CNY'
  }
  if (s.includes('元') || s.includes('人民币')) return 'CNY'
  return null
}
 ...
 */
const TOTAL_HINTS =
  /(合计|總計|总计|应付|實付|实付|付款金额|消费金额|总额|total|amount due|amount paid|grand total|subtotal)/i
const MONEY_RE =
  /(?:[¥￥$€£₩₹₽฿₫₪₺]|HK\$|NT\$|US\$|R\$|S\$|RM|Rp)?\s*(\d{1,3}(?:[,，]\d{3})*(?:\.\d{1,2})?|\d+(?:\.\d{1,2})?)/g

export function detectAmount(text: string): { amount: number | null; confident: boolean } {
  const lines = String(text || '').split(/\r?\n/)
  const parse = (raw: string | undefined): number => Number.parseFloat(String(raw).replace(/[,，]/g, ''))

  // 一轮：带「合计」这类词的行
  for (const line of lines) {
    if (!TOTAL_HINTS.test(line)) continue
    const found = [...line.matchAll(MONEY_RE)].map((m) => parse(m[1])).filter((n) => Number.isFinite(n) && n > 0)
    if (found.length) return { amount: Math.max(...found), confident: true }
  }
  // 二轮：全文里带货币符号的数字，取最大
  const all: number[] = []
  for (const line of lines) {
    for (const m of line.matchAll(MONEY_RE)) {
      ...
      if (!/[¥￥$€£₩₹₽฿₫₪₺]|HK\$|NT\$|US\$|R\$|S\$|RM|Rp/.test(m[0])) continue
      const n = parse(m[1])
      if (Number.isFinite(n) && n > 0) all.push(n)
    }
  }
  if (all.length) return { amount: Math.max(...all), confident: false }
  return { amount: null, confident: false }
}

/** 认日期。支持 2026-08-06 / 2026/8/6 / 08-06 / 2026年8月6日。认不出回 null（页面用今天）。 */
export function detectDate(text: string, now = Date.now()): number | null {
  const s = String(text || '')
 ...
 * 只回一个候选串给页面做默认值，用户可改。
 */
export function detectPayee(text: string): string {
  ...
  for (const line of lines.slice(0, 6)) {
    ...
    if (/\d{3,}/.test(line)) continue // 单号、电话
    // 分隔线判据：**一个字母或数字都没有**。
    ...
    if (/^\S*\s*[x×]\s*\d+\s+[\d.]+$/i.test(line)) continue // "拿铁 x1  32.00"
    return line
  }
  return ''
}
 ...
 */
export function parseReceipt(text: string, now = Date.now()) {
