...
function HistoryRow({ row, rank, t, onClick, onLongPress, onAdd, }) {
    ...
    return (_jsxs("div", { className: "mu-press", style: { display: 'flex', alignItems: 'center', gap: 12, padding: `6px ${SPACE.s4}px` }, onPointerDown: () => {
            ...
            if (timer.current)
                clearTimeout(timer.current);
        }, onClick: onClick, children: [rank !== null ? (_jsx("span", { className: "mu-mono", style: { fontSize: 15, fontWeight: 600, width: 22, color: rank < 3 ? C.accent : C.muted }, children: rank + 1 })) : null, _jsx(Artwork, { url: row.track.artworkUrl, size: 44, radius: 6, iconSize: 16 }), _jsxs("div", { style: { display: 'flex', flexDirection: 'column', gap: 1, minWidth: 0, flex: '1 1 auto' }, children: [_jsx("span", { className: "mu-clamp-1", style: { fontSize: 14, fontWeight: 500, color: C.ink }, children: row.track.title }), row.track.artist ? (_jsx("span", { className: "mu-clamp-1", style: { fontSize: 11, color: C.muted }, children: row.track.artist })) : null] }), _jsx("span", { style: { fontSize: 11, color: C.muted }, children: t('history.plays', row.count) }), _jsx("button", { type: "button", className: "mu-btn mu-press", onClick: (event) => {
                    event.stopPropagation();
                    onAdd();
                }, style: { padding: '6px 0 6px 10px', color: C.accent }, children: _jsx(Icon, { name: "text.append", size: 15, color: C.accent }) })] }));
}
