...
import type { NewsArticle, NewsContext, ReadHistoryEntry, SavedArticle } from '../types.js'
...
export default function SavedPage({ ctx }: { ctx: NewsContext }) {
  ...
  const menuFor = React.useCallback(
    (article: NewsArticle) => {
      const actions: Array<{ key: string; icon: IconName; label: string; onSelect: () => void | Promise<void> }> = []
      if (ctx.hasAI) {
        actions.push({
          key: 'analyze',
          icon: 'sparkles',
    ...
    if (items.length === 0) {
      ...
      return (
        <div className="news-scroll">
          <EmptyState icon={icon} text={ctx.t(key)} />
        </div>
      )
    }
    return (
          ...
          swipe={
            page === 0
              ? {
                  ...
                  onAction: (article: NewsArticle) => ctx.actions.unsave(article.id),
                }
              : {
                  label: ctx.t('news.action.delete'),
                  onAction: (article: NewsArticle) => ctx.actions.removeHistory(article.id),
                }
          }
        />
        <div style={{ height: 24 }} />
  ...
  return (
    <>
      <Segmented
        ...
        index={index}
        onSelect={(next) => {
          if (pagerRef.current) pagerRef.current.slideTo(next)
          else setIndex(next)
        }}
      />
...
function toArticle(row: SavedArticle): NewsArticle {
  return {
    id: row.id,
    title: row.title,
    url: row.url,
...
function historyToArticle(row: ReadHistoryEntry): NewsArticle {
  return {
    id: row.articleKey,
    title: row.title,
    url: row.url,
