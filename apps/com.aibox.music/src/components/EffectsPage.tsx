...
import type { MusicAppContext } from '../lib/types.js'

const PRESETS = ['flat', 'bass', 'vocal', 'treble', 'rock', 'pop', 'jazz', 'classical']
const RATE_PRESETS = [0.75, 1, 1.25, 1.5, 2]

export default function EffectsPage({ ctx }: { ctx: MusicAppContext }) {
  const { t, music } = ctx
    ...
    <div className="mu-scroll">
      <Card style={{ marginTop: SPACE.s4 }}>
        <Row
          ...
          accessory={
            <Switch value={!!effects.enabled} onChange={(value) => music.updateEffects('enable', { enabled: value })} />
          }
          last
        />
      </Card>
      ...
      <Card>
        <div style={{ padding: `12px ${SPACE.s4}px` }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', paddingBottom: 4 }}>
            ...
            <span className="mu-mono" style={{ fontSize: 15, color: C.muted }}>
              {rateLabel(effects.rate)}
            </span>
          </div>
          <input
            className="mu-slider"
            ...
            {PRESETS.concat(preset === 'custom' ? ['custom'] : []).map((name) => {
              ...
              return (
                <button
                  ...
                  className="mu-btn mu-press"
                  onClick={() => {
                    if (name !== 'custom') music.updateEffects('preset', { preset: name })
                  }}
                  style={{
                    padding: '6px 12px',
                    borderRadius: 999,
