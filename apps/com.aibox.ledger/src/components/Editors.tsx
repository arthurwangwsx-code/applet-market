...
import type { Account, AccountKind, LedgerUIContext, Member, Project } from '../types.js'

type SubmitRef<T> = React.MutableRefObject<() => T>
...
export function AccountEditor({
  ...
  onSubmit: SubmitRef<AccountFormValue>
}) {
  const { store, t } = ctx
  ...
  const [menu, setMenu] = React.useState<'kind' | 'currency' | null>(null)
...
export function ProjectEditor({
  ...
  onSubmit: SubmitRef<ProjectFormValue>
}) {
  const { store, t } = ctx
  const [name, setName] = React.useState(editing ? editing.name : '')
...
export function BudgetEditor({
  ...
  monthKey: number
  categoryID: string | null
  onSubmit: SubmitRef<BudgetFormValue>
}) {
  const { store, t } = ctx
...
export function MemberEditor({
  ...
  editing: Member | null
  order: number
  onSubmit: SubmitRef<MemberFormValue>
}) {
  const { t } = ctx
  const [name, setName] = React.useState(editing ? editing.name : '')
  const [isMe, setIsMe] = React.useState(editing ? !!editing.isMe : false)
  const [colorHex, setColor] = React.useState(
    editing ? editing.colorHex : (MEMBER_COLORS[(order ?? 0) % MEMBER_COLORS.length] ?? '#3A83D0'),
  )
...
export function ReconcileSheet({
  ...
  ctx: LedgerUIContext
  account: Account
  onSubmit: SubmitRef<ReconcileFormValue>
}) {
  const { store, t } = ctx
...
export function RateEditor({
  ...
  ctx: LedgerUIContext
  code: string
  onSubmit: SubmitRef<RateFormValue>
}) {
  const { store, t } = ctx
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: SPACE.s3 }}>
      ...
      >
        ...
        <input
          ...
          onChange={(event) => setText(event.target.value)}
        />
        <span className="lg-mono" style={{ fontSize: 18, fontWeight: 500, color: C.muted }}>
          {store.baseCode}
        </span>
      </div>
    </div>
