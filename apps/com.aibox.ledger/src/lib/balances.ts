...
import type { Account, BalanceSnapshot } from '../types.js'
import type { LedgerStore } from './store.js'

/** 估值型账户（investment）走快照，其余从流水派生。 */
export function derivesBalanceFromFlow(account: Account): boolean {
 ...
 */
export function balanceMinor(store: LedgerStore, account: Account, asOf = Date.now()): number {
  ...
  for (const txn of store.allTransactions()) {
    ...
    if (txn.occurredOn >= cutoff) continue
...
export function balancesByAccount(store: LedgerStore, asOf = Date.now()): Record<string, number> {
  const cutoff = dayStart(asOf) + DAY_MS
  ...
  for (const txn of store.allTransactions()) {
    ...
    if (out[txn.accountID] === undefined || out[txn.accountID] === null) continue
    out[txn.accountID] = (out[txn.accountID] ?? 0) + signedAmountMinor(txn)
  }
  for (const account of store.accounts) {
    if (out[account.id] !== null) continue
 ...
 */
export function netWorth(store: LedgerStore, balances: Readonly<Record<string, number>>) {
  let net = 0
  let assets = 0
  let liabilities = 0
  for (const account of store.accounts) {
    ...
    net += base
    if (base > 0) assets += base
    else liabilities += -base
  }
 ...
 * 两者都在**同一次 mutate** 里落地。
 */
export async function setBalance(store: LedgerStore, account: Account, targetMinor: number, now = Date.now()) {
  if (!derivesBalanceFromFlow(account)) {
  ...
  if (delta === 0) return { ok: true, delta: 0, noop: true }
