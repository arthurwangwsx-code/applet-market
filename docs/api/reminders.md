...
> Read and manage reminders through the host reminders module.

**分组** 系统能力投影 ｜ **方法数** 1 ｜ **声明要求** 需要在 `manifest.permissions.capabilities` 里声明 `"reminders"`。**声明 ≠ 授权**，用户仍会被逐项询问。
...
| 方法 | 背后的宿主工具 | 档位 |
|---|---|---|
| `items` | `schedule_reminders` | `meta` |

### `aibox.reminders.items()`

投影自宿主工具 `schedule_reminders`；摘要与参数以该工具的真实定义为准。

**副作用档位** `meta`（元操作）— 改容器自身的声明或路由，不碰宿主数据。

**参数**

参数取自宿主工具 `schedule_reminders` 的真实 `ToolDefinition.parametersJSON`，随宿主版本变化。
...
```js
await aibox.reminders.items({ action: 'list' })
```
...
- **宿主变体**：Full ✅ ／ Lean ⚠️ 取决于壳是否装了提醒事项模块
