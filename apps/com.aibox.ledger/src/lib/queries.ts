 ...
 */
export function filterTransactions(store: LedgerStore, filter: QueryFilter = {}): LedgerTransaction[] {
  ...
  return store.allTransactions().filter((txn) => {
    ...
    if (tag && !txn.tags.some((row) => row.toLowerCase() === tag)) return false
    if (
 ...
 */
export function matchesSearch(store: LedgerStore, txn: LedgerTransaction, query: string): boolean {
...
export function buckets(
  ...
  const push = (key: string, label: string, amount: number, colorHex: string | null = null) => {
  ...
  for (const txn of transactions) {
    ...
    switch (dimension) {
      case 'byCategory': {
        ...
        push(
          rootID ?? UNCATEGORIZED_KEY,
          category ? category.name : (labels.uncategorized ?? 'Uncategorized'),
          amount,
          category ? category.colorHex : null,
        )
        break
      }
      case 'bySubcategory': {
        ...
        push(
          txn.categoryID ?? UNCATEGORIZED_KEY,
      ...
      case 'byAccount': {
        const account = store.account(txn.accountID)
        push(
          txn.accountID ?? UNCATEGORIZED_KEY,
          account ? account.name : '—',
          amount,
          account ? account.colorHex : null,
        )
        break
      }
      case 'byTag': {
      ...
      case 'byProject': {
        const project = txn.projectID ? store.project(txn.projectID) : null
        push(
          txn.projectID ?? NO_PROJECT_KEY,
          project ? project.name : (labels.noProject ?? 'No project'),
          amount,
          project ? project.colorHex : null,
        )
        break
      }
      default:
...
export function periodRange(period: QueryPeriod, now = Date.now()): { from: number | null; to: number | null } {
