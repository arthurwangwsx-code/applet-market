...
import type { CurrencyRow as LedgerCurrencyRow, LedgerUIContext } from '../types.js'

export default function CurrencyManager({ ctx, mode = 'list' }: { ctx: LedgerUIContext; mode?: 'list' | 'add' }) {
        ...
        {canMutate ? (
          <>
            <button
              type="button"
              className="lg-btn"
              onClick={() => actions.openAddCurrency()}
              aria-label={t('cur.add')}
            >
            ...
            <button
              ...
              onClick={refresh}
              disabled={refreshing}
              aria-label={t('cur.refresh')}
            >
              {refreshing ? (
                <Spinner size={15} color={C.brand} />
              ) : (
                <Icon name="arrow.clockwise" size={15} color={C.brand} />
              )}
            </button>
          </>
        ) : null}
...
function CurrencyRow({ ctx, row, canMutate }: { ctx: LedgerUIContext; row: LedgerCurrencyRow; canMutate: boolean }) {
  ...
  return (
    <button
      ...
      className="lg-btn"
      onClick={() => {
        if (!row.isBase && canMutate) actions.editRate(row.code)
      }}
      {...longPress}
      style={{ display: 'flex', alignItems: 'center', gap: SPACE.s3, width: '100%', padding: SPACE.s3 }}
    >
