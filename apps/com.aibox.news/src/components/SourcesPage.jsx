      ...
      <SwipeRow actionLabel={ctx.t('news.action.delete')} onAction={() => ctx.actions.removeFeed(feed.id)}>
        ...
        >
          <span style={{ width: 22, display: 'flex', justifyContent: 'center', flex: '0 0 auto' }}>
            <Icon name={TOPIC_ICON[feed.topic] || 'newspaper'} size={14} color={C.brand} />
          </span>
          <span style={{ display: 'flex', flexDirection: 'column', gap: 2, flex: '1 1 auto', minWidth: 0 }}>
            <span className="news-clamp-1" style={{ fontSize: 15, color: C.ink }}>{feed.title}</span>
          ...
          <Toggle
            ...
            label={feed.title}
            onChange={(next) => ctx.actions.setFeedEnabled(feed.id, next)}
          />
        </div>
      </SwipeRow>
      <ActionSheet
