...
import type { LedgerTransaction, LedgerUIContext, Translate } from '../types.js'
import type { LedgerStore } from '../lib/store.js'

const TONE: Record<'ink' | 'muted' | 'income' | 'expense', string> = {
  ink: C.ink,
  muted: C.muted,
  income: C.income,
  expense: C.expense,
...
function SummaryCard({ store, t }: { store: LedgerStore; t: Translate }) {
  ...
  const body = (
    ...
    >
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 2, flex: '0 0 auto' }}>
        ...
        {conversion ? (
          <span className="lg-mono" style={{ fontSize: 10, color: TONE[conversion.tone] }}>
            {conversion.text}
          </span>
        ) : null}
      </div>
...
function DayGroup({
  ...
  store: LedgerStore
  group: { day: number; rows: LedgerTransaction[] }
  t: Translate
  locale: string
  onOpen: (transaction: LedgerTransaction) => void
  onDelete: (transaction: LedgerTransaction) => void | Promise<void>
  canMutate: boolean
}) {
  ...
  return (
    <div style={{ marginBottom: SPACE.s4 }}>
      <div style={{ display: 'flex', alignItems: 'baseline', padding: `0 4px ${SPACE.s2}px` }}>
        <span style={{ fontSize: 13, fontWeight: 500, color: C.muted }}>{dayHeaderTitle(group.day, locale, t)}</span>
        <div style={{ flex: '1 1 auto' }} />
        {total ? (
...
export default function TransactionsPage({ ctx }: { ctx: LedgerUIContext }) {
  const { store, t, locale, query, actions, canMutate } = ctx
  const [visibleMonths, setVisibleMonths] = React.useState(3)
  const [typeFilter, setTypeFilter] = React.useState<'all' | 'expense' | 'income' | 'transfer'>('all')
  ...
  const [menu, setMenu] = React.useState<'type' | 'account' | 'project' | null>(null)
  ...
  const typeLabel: Record<typeof typeFilter, string> = {
    all: t('x.all'),
    expense: t('x.expense'),
    income: t('x.income'),
  ...
  const header = (
    <div style={{ display: 'flex', flexDirection: 'column', gap: SPACE.s4, padding: SPACE.s4, paddingBottom: 0 }}>
      ...
      <div className="lg-chips">
        <Chip label={typeLabel[typeFilter]} selected={typeFilter !== 'all'} onClick={() => setMenu('type')} />
  ...
  return (
    <>
      <VirtualList
        ...
        header={header}
        footer={
          groups.length === 0 ? <div style={{ padding: `${SPACE.s6}px ${SPACE.s4}px 96px` }}>{empty}</div> : footer
        }
        renderRow={(group) => (
