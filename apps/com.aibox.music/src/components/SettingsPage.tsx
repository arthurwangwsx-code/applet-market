...
import type { MusicAppContext, RepeatMode } from '../lib/types.js'

const REPEAT_MODES: RepeatMode[] = ['off', 'one', 'all']

export default function SettingsPage({ ctx }: { ctx: MusicAppContext }) {
        ...
        <Row
          ...
          onClick={() =>
            actions.pickOption({
              ...
              onPick: (mode) => {
                if (mode === 'off' || mode === 'one' || mode === 'all') void music.setRepeat(mode)
              },
            })
          }
        />
