...
function normalizeRecordAction(raw, args) {
    ...
    if (mapped === 'add' || mapped === 'update' || mapped === 'delete')
        return mapped;
...
function normalizeType(raw) {
    const text = String(raw ?? '')
        .trim()
        .toLowerCase();
    if (text === 'expense' || text === 'income' || text === 'transfer')
        return text;
...
export async function actionRecord(store, args, locale) {
    ...
    if (action === 'update') {
        ...
        if (args.amount !== undefined && args.amount !== null)
            patch.amountMinor = majorNumberToMinor(Number(args.amount));
        if (args.note !== undefined)
            patch.note = String(args.note);
        if (args.merchant !== undefined)
            patch.merchant = String(args.merchant);
        if (args.tags !== undefined)
            patch.tags = Array.isArray(args.tags) ? args.tags : [];
    ...
    if (type === 'transfer') {
        ...
        const result = await recordTransfer(store, {
            ...
            amountMinor,
            toAmountMinor: args.to_amount !== undefined && args.to_amount !== null ? majorNumberToMinor(Number(args.to_amount)) : null,
            occurredOn,
...
export async function actionQuery(store, args, locale) {
    ...
    if (args.tag)
        filter.tag = String(args.tag);
    if (args.keyword)
        filter.keyword = String(args.keyword);
    if (args.reimbursable !== undefined)
...
export async function actionStats(store, args, locale, labels) {
    ...
    const metric = args.metric === 'income' || args.metric === 'net' ? args.metric : 'expense';
    const filter = { dateFrom: range.from, dateTo: range.to };
    if (args.project) {
        const resolved = resolveProject(store, args.project);
        if (!resolved.found)
    ...
    if (args.compare_previous && range.from !== null && range.to !== null) {
        ...
        previousTotal = buckets(store, previous, dimension, metric, locale, labels).reduce((sum, bucket) => sum + bucket.amountMinor, 0);
    }
...
function normalizePeriod(value) {
    ...
    return allowed.includes(period) ? period : 'this_month';
}
