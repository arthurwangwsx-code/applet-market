...
export class NewsStore {
  feeds: NewsFeed[]
  saved: SavedArticle[]
  read: ReadHistoryEntry[]
  settings: NewsSettings
  contentIndex: Record<string, ContentIndexEntry>
  ready: boolean
  ...
  async load() {
    const [feeds, saved, read, settings, contentIndex] = await Promise.all([
      storage.get<unknown>(KEYS.feeds, null),
      storage.get<unknown>(KEYS.saved, null),
      storage.get<unknown>(KEYS.read, null),
      storage.get<unknown>(KEYS.settings, null),
      storage.get<unknown>(KEYS.contentIndex, null),
    ])
    this.feeds = asArray<NewsFeed>(feeds)
    this.saved = asArray<SavedArticle>(saved)
    this.read = asArray<ReadHistoryEntry>(read)
    this.settings = {
      ...DEFAULT_SETTINGS,
      ...(settings && typeof settings === 'object' ? (settings as Partial<NewsSettings>) : {}),
    }
    this.contentIndex =
      contentIndex && typeof contentIndex === 'object' && !Array.isArray(contentIndex)
        ? (contentIndex as Record<string, ContentIndexEntry>)
        : {}
  ...
  async updateSettings(patch: Partial<NewsSettings>) {
   ...
   */
  async addFeed({ title, endpoint, kind, topic, explicitKind = false }: AddFeedInput): Promise<NewsFeed> {
  ...
  async setFeedEnabled(id: string, enabled: boolean) {
  ...
  async moveFeed(id: string, direction: number) {
    ...
    this.feeds = this.feeds.map((feed) => {
      if (feed.id === a.id) return { ...feed, sortOrder: orderB }
      if (feed.id === b.id) return { ...feed, sortOrder: orderA }
      return feed
    })
    await this.persistFeeds()
  }

  async markFeedFetched(id: string, at = Date.now()) {
    let changed = false
    this.feeds = this.feeds.map((feed) => {
    ...
    if (changed) await storage.set(KEYS.feeds, json(this.feeds))
  }
  ...
  async save(article: NewsArticle) {
    if (this.saved.some((entry) => entry.id === article.id)) return
    this.saved = [
      {
        id: article.id,
        url: article.url,
  ...
  async markRead(article: NewsArticle) {
    const now = Date.now()
    const entry = {
  ...
  async markUnread(articleKey: string) {
  ...
  async removeHistory(articleKey: string) {
    return this.markUnread(articleKey)
  }

  // MARK: 时间线快照

  async loadSnapshot(): Promise<TimelineSnapshot | null> {
    const snapshot = await storage.get<unknown>(KEYS.snapshot, null)
    if (!snapshot || typeof snapshot !== 'object') return null
    const row = snapshot as Partial<TimelineSnapshot>
    return { articles: asArray<NewsArticle>(row.articles), savedAt: Number(row.savedAt) || 0 }
  }

  async persistSnapshot(articles: NewsArticle[]) {
    await storage.set(
      KEYS.snapshot,
      json({
  ...
  static shardKey(url: string) {
    return `news.content.${fnv1aHex(normalizeURL(url))}`
  }

  async readContent(url: string): Promise<string | null> {
    const key = NewsStore.shardKey(url)
    const shard = await storage.get<unknown>(key, null)
    if (!shard || typeof shard !== 'object' || Array.isArray(shard) || !('text' in shard)) return null
    const text = (shard as { text?: unknown }).text
  ...
  async writeContent(url: string, text: string) {
    if (!text) return
    const key = NewsStore.shardKey(url)
  ...
  async clearContentCache() {
    for (const key of Object.keys(this.contentIndex)) await storage.remove(key)
    this.contentIndex = {}
    await storage.set(KEYS.contentIndex, json(this.contentIndex))
    this.emit()
  }
