...
import type { AddDownloadsInput, DownloadDestination, DownloadPriority } from '../types.js'

const DESTINATIONS: Array<{ kind: DownloadDestination; label: string; icon: string; hint: string }> = [
...
const PRIORITIES: Array<{ id: DownloadPriority; label: string }> = [
  { id: 'high', label: '优先' },
  { id: 'normal', label: '普通' },
  { id: 'low', label: '后台' },
]
...
interface AddSheetProps {
  open: boolean
  onClose: () => void
  onSubmit: (request: AddDownloadsInput) => void | Promise<void>
  onPaste: (() => Promise<string>) | null
}

export default function AddSheet({ open, onClose, onSubmit, onPaste }: AddSheetProps) {
  const [text, setText] = React.useState('')
  const [destination, setDestination] = React.useState<DownloadDestination>('sandbox')
  const [folder, setFolder] = React.useState('')
  const [priority, setPriority] = React.useState<DownloadPriority>('normal')
  ...
  return (
    ...
    >
      ...
      {text && !urls.length ? (
        <div style={{ marginTop: SPACE.s2, fontSize: 12.5, color: C.failed }}>没找到 http(s) 链接。</div>
      ) : null}
      {urls.length > 1 ? (
      ...
      <div style={{ display: 'flex', gap: SPACE.s2, marginTop: SPACE.s2 }}>
        {PRIORITIES.map((p) => (
          <Chip key={p.id} active={priority === p.id} onClick={() => setPriority(p.id)}>
            {p.label}
          </Chip>
        ))}
      </div>
