...
export async function recorderAvailability() {
    const bridge = api();
    if (!bridge?.audio)
        return { available: false, reason: 'unavailable', background: false };
    try {
...
export async function recordStart(input) {
    const bridge = api();
    if (!bridge?.audio)
        return { started: false, error: 'aibox/unavailable' };
    try {
 ...
 * 布尔，就必然出现「点了没反应」的按钮。
 */
export async function transcribeAvailability(locale) {
    const bridge = api();
 ...
 * 所以调用方**不要**并发提交一批。
 */
export async function transcribeClip(handle, locale) {
    const bridge = api();
    ...
    if (!handle)
        return { ...empty, error: 'aibox/invalid-args: missing handle' };
    try {
...
export function localeTag(preference) {
    if (preference === 'auto')
        return undefined;
...
async function write(collection, document) {
    const store = db();
    const doc = document;
    if (!store) {
...
export function emptyArtifacts(memoID) {
    return {
        memoID,
        summaryText: '',
        summaryPoints: [],
...
// —— 视图模型 ——
export function clipToMemo(clip) {
    return {
        id: clip.id,
        title: clip.title,
...
export function withSnippet(memo, query, transcript) {
    if (!query)
        return memo;
    if (memo.title.toLowerCase().includes(query.toLowerCase()))
        return memo;
