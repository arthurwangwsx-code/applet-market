...
import type { MusicStatus, MusicTrack, PlayHistoryRow } from './types.js'

async function readStatus(): Promise<MusicStatus | null> {
  const result = await callMusic<MusicStatus>('status', {})
...
export async function nowPlayingSummary() {
  const [statusResult, queueResult, timerResult] = await Promise.all([
    callMusic<MusicStatus>('status', {}),
    callMusic<Array<MusicTrack & { index?: number }>>('queue', { action: 'list' }),
  ...
  let lyrics: { available: boolean; synced: boolean; currentLine: string | null; note: string | null } = {
    available: false,
    synced: false,
    currentLine: null,
    note: null,
...
export async function playMostPlayed(input: { limit?: number }) {
  const limit = Math.max(1, Math.min(50, Number(input && input.limit) || 20))
  const history = await storage.get<PlayHistoryRow[]>('music.playHistory')
...
export async function resumeLast() {
  const ui = await storage.get<{ lastTrack?: MusicTrack; lastPosition?: number }>('music.uiState')
