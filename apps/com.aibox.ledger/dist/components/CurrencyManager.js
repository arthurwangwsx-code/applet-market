...
function CurrencyRow({ ctx, row, canMutate }) {
    ...
    return (_jsxs("button", { type: "button", className: "lg-btn", onClick: () => {
            if (!row.isBase && canMutate)
                actions.editRate(row.code);
        }, ...longPress, style: { display: 'flex', alignItems: 'center', gap: SPACE.s3, width: '100%', padding: SPACE.s3 }, children: [_jsxs("div", { style: { flex: '1 1 auto', minWidth: 0, display: 'flex', flexDirection: 'column', gap: 2 }, children: [_jsxs("span", { style: { display: 'flex', alignItems: 'center', gap: 6 }, children: [_jsx("span", { className: "lg-mono", style: { fontSize: 15, fontWeight: 500, color: C.ink }, children: `${row.code} ${currencySymbol(row.code)}` }), row.isBase ? (_jsx("span", { style: {
                                    fontSize: 10,
                                    fontWeight: 500,
                                    color: C.brand,
