...
export function QueueRow({ track, isCurrent, isPlaying, onClick, onLongPress, trailing, rowId, }) {
    ...
    return (_jsxs("div", { className: "mu-press", "data-row-id": rowId, style: { display: 'flex', alignItems: 'center', gap: 12, padding: `6px ${SPACE.s4}px` }, ...press.bind, onClick: () => {
            if (!press.consumed() && onClick)
                onClick();
        }, children: [_jsxs("div", { style: { position: 'relative' }, children: [_jsx(Artwork, { url: track.artworkUrl, size: 36, radius: 6, iconSize: 16, tint: isCurrent ? C.accent : C.muted, background: isCurrent
...
export function SongRow({ track, onClick, onLongPress, trailing, artworkSize = 40, rowId, }) {
    const press = useLongPress(() => onLongPress && onLongPress());
    return (_jsxs("div", { className: "mu-press", "aria-label": "audioSongRow", "data-row-id": rowId, style: { display: 'flex', alignItems: 'center', gap: 12, padding: `6px ${SPACE.s4}px` }, ...press.bind, onClick: () => {
            if (!press.consumed() && onClick)
                onClick();
        }, children: [_jsx(Artwork, { url: track.artworkUrl, size: artworkSize, radius: 6, iconSize: 16 }), _jsxs("div", { style: { display: 'flex', flexDirection: 'column', gap: 1, minWidth: 0, flex: '1 1 auto' }, children: [_jsx("span", { className: "mu-clamp-1", style: { fontSize: 14, color: C.ink }, children: track.title }), track.artist ? (_jsx("span", { className: "mu-clamp-1", style: { fontSize: 11, color: C.muted }, children: track.artist })) : null] }), trailing !== undefined ? trailing : _jsx(Icon, { name: "play.circle.fill", size: 22, color: C.accent })] }));
}
/** 合集行（艺人 / 专辑 / 歌单）：44×44 封面，**艺人是圆形**。 */
export function CollectionRow({ item, onClick, onLongPress, rowId, }) {
    const press = useLongPress(() => onLongPress && onLongPress());
    const circular = item.type === 'artist';
    return (_jsxs("div", { className: "mu-press", "aria-label": "audioCollectionRow", "data-row-id": rowId, style: { display: 'flex', alignItems: 'center', gap: 12, padding: `8px ${SPACE.s4}px` }, ...press.bind, onClick: () => {
            if (!press.consumed() && onClick)
                onClick();
        }, children: [_jsx(Artwork, { url: item.artworkUrl, size: 44, radius: circular ? 22 : 6, iconSize: 18 }), _jsxs("div", { style: { display: 'flex', flexDirection: 'column', gap: 1, minWidth: 0, flex: '1 1 auto' }, children: [_jsx("span", { className: "mu-clamp-1", style: { fontSize: 14, fontWeight: 500, color: C.ink }, children: item.title || item.name }), item.artist || item.curator ? (_jsx("span", { className: "mu-clamp-1", style: { fontSize: 11, color: C.muted }, children: item.artist || item.curator })) : null] }), _jsx(Icon, { name: "chevron.right", size: 13, color: C.muted })] }));
}
...
export function LocalTrackRow({ track, onClick, onLongPress }) {
    ...
    return (_jsxs("div", { className: "mu-press", style: { display: 'flex', alignItems: 'center', gap: 12, padding: `6px ${SPACE.s4}px`, height: 60 }, ...press.bind, onClick: () => {
            if (!press.consumed() && onClick)
                onClick();
