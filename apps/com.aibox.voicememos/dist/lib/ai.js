...
async function generate(input) {
    const api = ai();
    if (!api)
        throw new AiError('aibox/ai-unavailable');
    try {
 ...
 */
export async function summarize(transcript, template) {
 ...
 */
export async function actionItems(transcript) {
    const text = clip(transcript.trim());
    if (!text)
        throw new AiError('empty-transcript');
    const raw = await generate({
 ...
 */
export async function chapters(transcript, segments) {
    const text = clip(transcript.trim());
    if (!text)
        throw new AiError('empty-transcript');
    const raw = await generate({
 ...
 * 用户随时可以切回去看原文。
 */
export async function cleanTranscript(transcript) {
    const text = clip(transcript.trim());
    if (!text)
        throw new AiError('empty-transcript');
...
export async function correct(input) {
    ...
    if (turns.length === 0)
        throw new AiError('empty-result');
    return turns;
}
/** AI 给的 label 若形如 `S1`（S + 纯数字）或为空 → 本地化成「说话人 N」；否则用真名。 */
export function speakerDisplayName(label, index, template) {
    ...
    if (match)
        return template.replace('{n}', match[1] ?? trimmed);
    return trimmed;
}
// —— 翻译（规格 §4.10 / §13.5） ——
