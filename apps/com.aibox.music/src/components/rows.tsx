...
import type { MusicCollection, MusicTrack } from '../lib/types.js'

interface TrackRowActions {
  onClick?: () => void
  onLongPress?: () => void
  rowId?: string
}
...
export function QueueRow({
  ...
  return (
    <div
      ...
      {...press.bind}
      onClick={() => {
        if (!press.consumed() && onClick) onClick()
      }}
    >
      ...
      {Number(track.duration) > 0 ? (
        <span className="mu-mono" style={{ fontSize: 11, color: C.muted }}>
...
export function SongRow({
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 1, minWidth: 0, flex: '1 1 auto' }}>
        <span className="mu-clamp-1" style={{ fontSize: 14, color: C.ink }}>
          {track.title}
        </span>
        {track.artist ? (
          <span className="mu-clamp-1" style={{ fontSize: 11, color: C.muted }}>
...
export function LocalTrackRow({ track, onClick, onLongPress }: TrackRowActions & { track: MusicTrack }) {
  const press = useLongPress(() => onLongPress && onLongPress())
...
export function NumberedTrackRow({
  index,
  track,
  onClick,
  onLongPress,
