...
declare module 'aibox/ui' {
  ...
  export interface QROptions {
    /** 边长，**CSS 点**（默认 240，上限 512）。不要自己乘 devicePixelRatio。 */
    size?: number
    /** 纠错级别，默认 M。小尺寸印链接时用 Q 或 H。 */
    level?: 'L' | 'M' | 'Q' | 'H'
  }

  /**
   * 把文本编码成一张**本地渲染**的二维码 PNG URL，可直接放进 `<img src>`。
   *
   * 扫码登录、分享链接、配网码都用它。**绝不要**改调第三方二维码服务
   * （`api.qrserver.com` 之类）——那等于把内容发给一个无关第三方，
   * 如果内容是一次性登录凭据，谁拿到谁就能登录用户的账号。这条通道下内容不出设备。
   *
   * 内容上限 1024 字节，超出宿主回 413（DevTools 可见）。
   */
  export function qrURL(content: string, options?: QROptions): string

  /** 命名空间别名：`ui.qr.url(...)`。 */
  export const qr: { url: typeof qrURL }
