...
import type { AlertCondition, FinanceContext, InstrumentPanelRoute } from '../lib/types.js'
...
export default function AlertPanel({
  ...
  const submit = async () => {
    if (!canSubmit || value === null || !canonical) return
    ...
    <Sheet visible={visible} onClose={onClose}>
      ...
      >
        <Segmented
          ...
          items={(Object.keys(CONDITION_KEYS) as AlertCondition[]).map((id) => ({ id, label: t(CONDITION_KEYS[id]) }))}
        />
        ...
        {rows.length > 0 ? (
          <div style={{ background: C.surface, borderRadius: RADIUS.card, padding: `0 ${SPACE.s3}px` }}>
            {rows.map((row, index) => (
              <Row
                key={row.id}
                title={`${t(CONDITION_KEYS[row.conditionRaw])} ${
                  isPercentCondition(row.conditionRaw)
                ...
                accessory={
                  <div style={{ display: 'flex', alignItems: 'center', gap: SPACE.s3 }}>
                    <Toggle
                      checked={row.enabled}
                      onChange={(next) => alerts.setEnabled(row.id, next)}
