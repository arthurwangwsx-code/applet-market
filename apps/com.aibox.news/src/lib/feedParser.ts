...
import type { NewsArticle, NewsTopic } from '../types.js'
...
function tailText(element: Element) {
  let buffer = ''
...
function emptyItem(): ParsedFeedItem {
...
/**
 * 桥会按 `maxBytes`（我们显式传 1MB，不传时默认 200KB）截断响应体，超长 feed 会变成不闭合的 XML。
 * 这里把内容剪到最后一条完整的 item/entry，再补上闭合标签重试一次——
 * 否则一个大 feed 会整份丢失（而不是少几条）。
 */
function repairTruncatedXML(xml: string): XMLDocument | null {
 ...
 */
export function parseFeed(xml: string, source: FeedSource, now = Date.now(), max = MAX_ITEMS_PER_FEED): NewsArticle[] {
...
function walk(element: Element, item: ParsedFeedItem, state: ParserState, root: Element) {
...
function handleStart(element: Element, item: ParsedFeedItem, state: ParserState, root: Element) {
...
function handleEnd(element: Element, item: ParsedFeedItem, state: ParserState, root: Element) {
  if (element === root) return
  const name = element.nodeName.toLowerCase()
...
function commit(item: ParsedFeedItem, source: FeedSource, now: number): NewsArticle | null {
