...
export const SDK_COMPATIBILITY_SCHEMA = 1;
function injectedMetadata() {
    try {
...
export function containerInfo() {
    ...
    const current = typeof protocol?.current === 'string'
        ? protocol.current
    ...
    const supported = Array.isArray(protocol?.supported)
        ? protocol.supported.filter((value) => typeof value === 'string')
    ...
    const modules = Array.isArray(metadata?.runtimeModules)
        ...
        : [];
    const delivery = metadata && modules.includes('aibox/sdk') ? 'host-shared' : isApplet() ? 'legacy-bundled' : 'web-preview';
    return {
 ...
 */
export function supports(feature, minimumVersion) {
    ...
    switch (name) {
        ...
        case 'container':
            return (info.containerVersion !== null && (!minimumVersion || versionAtLeast(info.containerVersion, minimumVersion)));
        case 'bridge': {
            if (!minimumVersion)
                return info.bridgeProtocol.current !== null;
    ...
    if (name.startsWith('runtime:')) {
        ...
        if (!info.runtimeModules.includes(moduleName))
            return false;
 ...
 */
export function checkCompatibility(requirement = {}) {
    const info = containerInfo();
    const errors = [];
    const warnings = [];
    if (requirement.minContainerVersion &&
    ...
    for (const raw of requirement.capabilities ?? []) {
        const item = typeof raw === 'string' ? capabilityRequirement(raw) : raw;
...
function capabilityRequirement(value) {
