...
import type { ResolvedVideo } from '@aibox/applet-sdk'
...
import type { VideoCapabilities, VideoProgress, VideoSummary, WatchProgress } from '../lib/types.js'
import { errorMessage } from '../lib/types.js'

const PROGRESS_KEY = 'watch-progress'
const HISTORY_KEY = 'watch-history'

type PlayerState = 'resolving' | 'ready' | 'error'
...
export default function PlayerPage({ video, onOpen }: { video: VideoSummary; onOpen: (video: VideoSummary) => void }) {
  const [state, setState] = React.useState<PlayerState>('resolving')
  const [media, setMedia] = React.useState<ResolvedVideo | null>(null)
  const [error, setError] = React.useState('')
  // `null` = 还没探测出来。**别用 true 当初值**：那会让页面先显示「能播」再跳成「不能播」。
  const [caps, setCaps] = React.useState<VideoCapabilities | null>(null)
  ...
  const [related, setRelated] = React.useState<VideoSummary[]>([])
  const pending = React.useRef<PendingProgress | null>(null)
  ...
  React.useEffect(() => {
    ...
    resolve(video.url)
      .then((r) => {
        ...
        loadPref<VideoSummary[]>(HISTORY_KEY, [])
          .then((list) => {
  ...
  React.useEffect(() => {
    let lastWhole = -1
    const off = onVideoProgress((snapshot: VideoProgress) => {
  ...
  React.useEffect(
    () => () => {
      ...
      if (!p) return
      loadPref<WatchProgress>(PROGRESS_KEY, {})
  ...
  const start = React.useCallback(
    async (formatID: string) => {
      ...
      try {
        ...
        const saved = await loadPref<WatchProgress>(PROGRESS_KEY, {})
        await play({
          sourceURL: video.url,
          formatID,
  ...
  return (
    <div className="yt-scroll" style={{ height: '100%', overflowY: 'auto', background: C.bg }}>
      ...
      {stageOn ? null : (
        <div
          onClick={() => {
            if (firstPlayable) start(firstPlayable.id)
          }}
          style={{
            position: 'relative',
            width: '100%',
      ...
      <div style={{ padding: SPACE.s4 }}>
        ...
        {playable.length ? (
          <div style={{ marginTop: SPACE.s4 }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: C.sub, marginBottom: SPACE.s2 }}>清晰度</div>
