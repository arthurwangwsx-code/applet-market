...
import type { AISession, NewsArticle, NewsContext } from '../types.js'
...
] as const satisfies ReadonlyArray<{ key: string; icon: IconName; labelKey: string; seedKey: string }>
...
function timelineDigest(articles: NewsArticle[], limit = 40) {
  return articles
    .slice(0, limit)
    ...
    <Sheet visible={!!session} onClose={onClose}>
      <div style={{ display: 'flex', flexDirection: 'column', height: '78dvh' }}>
        ...
        <div
          ...
          style={{ display: 'flex', gap: 8, overflowX: 'auto', padding: `0 ${SPACE.s4}px ${SPACE.s2}px` }}
        >
