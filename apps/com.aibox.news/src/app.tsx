...
import { NavBar, SearchField, TabBar, ToolbarButton } from './components/Shell.js'
...
import AddSourcePage from './components/AddSourcePage.js'
import DiagnosticsPage from './components/DiagnosticsPage.js'
import ClusterDetail from './components/ClusterDetail.js'
import BroadcastBar, { BroadcastNotice } from './components/BroadcastBar.js'
...
import { isAvailable } from '@aibox/applet-sdk'
import type { AISession, BrowserMode, NewsArticle, NewsContext, NewsRoute, Translate, Unsubscribe } from './types.js'
...
export default function App() {
  ...
  // 子页栈 = 宿主原生页栈的镜像（框架资产 `aibox/ui`）：进子页走 `aibox.navigation.push`，
  // 返回一律经 popstate 回来，于是最左缘左滑是**系统自己的** interactive pop。
  const subpages = useSubpageStack<NewsRoute>({
    pathFor: routePath,
  ...
  }>({ hasAI: false, hasTTS: false, hasKB: false, browserModes: [] })
  ...
  React.useEffect(() => {
    ...
    const wire = async () => {
      ...
      if (api && api.tabs && typeof api.tabs.getState === 'function') {
        ...
        offs.push(
          onNamespaceEvent('tabs', 'changed', (payload: unknown) => {
            if (!payload || typeof payload !== 'object') return
            const state = payload as { rendered?: boolean; selected?: TabID }
      ...
      if (api && api.toolbar && typeof api.toolbar.getState === 'function') {
        ...
        offs.push(
          onNamespaceEvent('toolbar', 'invoke', (payload: unknown) => {
            if (payload && typeof payload === 'object' && 'id' in payload && payload.id === 'ai')
              openAIRef.current(null)
          }),
        )
        offs.push(
          onNamespaceEvent('toolbar', 'searchChanged', (payload: unknown) => {
  ...
  const ctx = React.useMemo<NewsContext>(
    () => ({
      t,
      locale,
      now,
      ...
      actions: {
        ...
        openCluster: (item) =>
          navigate({ name: 'cluster', title: item.lead.title, articles: [item.lead, ...item.related] }),
        toggleSaved: (article) => (store.savedKeys.has(article.id) ? store.unsave(article.id) : store.save(article)),
        unsave: (id) => store.unsave(id),
        toggleRead: (article) =>
          store.readKeys.has(article.id) ? store.markUnread(article.id) : store.markRead(article),
  ...
  return (
    <div className="news-root">
      ...
      <BroadcastBar
        ...
        t={t}
        onOpenCurrent={() => {
          if (broadcast.current) ctx.actions.openArticle(broadcast.current)
        }}
      />
...
function routePath(route: NewsRoute) {
  if (!route) return '#/'
  if (route.name === 'source') return `#/source/${encodeURIComponent((route.feed && route.feed.id) || '')}`
  return `#/${route.name}`
}

function routeTitle(route: NewsRoute, t: Translate) {
  if (!route) return ''
  switch (route.name) {
    case 'settings':
...
function renderRoute(route: NewsRoute, ctx: NewsContext) {
  switch (route.name) {
    case 'settings':
      return <SettingsPage ctx={ctx} />
    case 'addSource':
      return <AddSourcePage ctx={ctx} />
    case 'diagnostics':
      return <DiagnosticsPage ctx={ctx} />
    case 'cluster':
      return <ClusterDetail ctx={ctx} cluster={route} />
    case 'source':
      return <FeedPage ctx={ctx} sourceFeed={route.feed} />
    default:
      return null
  }
}
