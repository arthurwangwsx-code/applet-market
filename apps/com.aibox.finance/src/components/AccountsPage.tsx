...
import type { Account, CashFlowKind, FinanceContext } from '../lib/types.js'
...
export function AccountsSheet({ ctx, visible, onClose }: SheetProps) {
  const { t, ledger } = ctx
  const [creating, setCreating] = React.useState(false)
  const [name, setName] = React.useState('')
  const [currency, setCurrency] = React.useState<AccountCurrency>('CNY')
            ...
            >
              ...
              <div style={{ flex: '1 1 auto' }}>
                <Segmented
                  ...
                  items={CURRENCIES.map((id) => ({ id, label: id }))}
                />
              </div>
            </div>
...
function AccountRow({ ctx, account, last }: { ctx: FinanceContext; account: Account; last: boolean }) {
  const { t, ledger } = ctx
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: SPACE.s2, padding: `${SPACE.s2}px 0 ${SPACE.s3}px` }}>
          {(
            [
              ...
              {
                id: 'archive',
                label: t('finance.account.archive'),
                onClick: () => ledger.updateAccount(account.id, { isArchived: true }),
              },
              ...
              ...(canShare ? [{ id: 'export', label: t('finance.history.export'), onClick: exportArchive }] : []),
              {
                ...
                danger: true,
                onClick: () => ledger.deleteAccount(account.id),
              },
            ] as Array<{ id: string; label: string; danger?: boolean; onClick: () => unknown }>
          ).map((button) => (
            <button
              key={button.id}
              type="button"
...
export function CashFlowSheet({ ctx, visible, onClose }: SheetProps) {
  const { t, ledger, accountID } = ctx
  const [kind, setKind] = React.useState<CashFlowKind>('deposit')
  ...
  const canSubmit = account !== null && value !== null && Number.isFinite(value) && value > 0
    ...
    <Sheet visible={visible} onClose={onClose}>
      ...
      >
        ...
        <div style={{ background: C.surface, borderRadius: RADIUS.card, padding: `0 ${SPACE.s3}px` }}>
          <Field
            label={t('finance.cashflow.amount')}
            value={amount}
            onChange={setAmount}
            placeholder="0"
...
/** 交易历史（全量流水）。 */
export function HistorySheet({ ctx, visible, onClose }: SheetProps) {
          ...
          rows.map((order) => (
            ...
            >
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 2, flex: '1 1 auto', minWidth: 0 }}>
                <span className="fin-clamp-1" style={{ fontSize: 14 }}>
                  {order.name}
                </span>
                <span className="fin-mono" style={{ fontSize: 12, color: C.muted }}>
                  {isoDate(order.tradedAt)}
                </span>
              </div>
