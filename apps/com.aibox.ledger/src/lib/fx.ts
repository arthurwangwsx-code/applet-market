...
import type { CurrencyRow, LedgerTransaction } from '../types.js'

export const RATE_ENDPOINT_HOST = 'open.er-api.com'

export function baseCurrency(currencies: readonly CurrencyRow[]): CurrencyRow | null {
  return currencies.find((row) => row.isBase) ?? currencies[0] ?? null
}

export function baseCode(currencies: readonly CurrencyRow[]): string {
  const base = baseCurrency(currencies)
  return base ? base.code : 'CNY'
}

export function currencyRow(currencies: readonly CurrencyRow[], code: string): CurrencyRow | null {
  const upper = String(code ?? '').toUpperCase()
  return currencies.find((row) => row.code === upper) ?? null
}

/** 该币种是否可用于统计（基准币恒可用）。 */
export function hasUsableRate(currencies: readonly CurrencyRow[], code: string): boolean {
 ...
 *   未配置汇率/≤0   → 0        ← 从汇总里排除
 */
export function toBaseMinor(currencies: readonly CurrencyRow[], minor: number, code: string): number {
...
/** 任意币 → 任意币（先过基准币）。 */
export function convertMinor(currencies: readonly CurrencyRow[], minor: number, from: string, to: string): number {
 ...
 */
export function reportingBaseMinor(currencies: readonly CurrencyRow[], txn: LedgerTransaction): number {
 ...
 */
export function applyPostingSnapshot(
...
type HTTPGetJSON = (url: string, options?: { timeoutMs?: number }) => Promise<HTTPJSONResult>

export async function fetchRates(base: string, httpGetJSON: HTTPGetJSON): Promise<Record<string, number> | null> {
