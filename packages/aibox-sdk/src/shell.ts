...
export async function tabsState(): Promise<TabsState | null> {
  ...
  try { return await host.tabs.getState(); } catch { return null; }
}
...
export async function selectTab(id: string): Promise<TabsState | null> {
  const host = bridge();
  if (!host?.tabs) return null;
  try { return await host.tabs.select(id); } catch (error) { throw normalizeError(error); }
}
...
export async function setTabBadge(id: string, badge: string | null): Promise<void> {
  const host = bridge();
  if (!host?.tabs) return;
  try { await host.tabs.update({ items: { [id]: { badge } } }); } catch { /* 角标是装饰，失败不该影响主流程 */ }
}
...
export async function sceneState(): Promise<SceneState | null> {
  ...
  try { return await host.scene.getState(); } catch { return null; }
}
...
export async function setTitle(title: string): Promise<void> {
  ...
  try { await host.navigation.setTitle(title); } catch { /* 标题是装饰 */ }
}
...
export async function haptic(kind: 'light' | 'medium' | 'heavy' | 'success' | 'warning' | 'error' = 'light'): Promise<void> {
