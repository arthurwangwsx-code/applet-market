...
import type { Currency, Market, Trend } from './types.js'
...
export function formatPriceFor(value: number | null | undefined, market: Market): string {
...
export function formatCompactCurrency(value: number, currency: Currency = 'CNY'): string {
  if (!Number.isFinite(value)) return '—'
  const sign = value < 0 ? '-' : ''
...
/** 千分位只加在整数部分，小数部分原样。 */
export function groupInteger(text: unknown): string {
...
export function toLots(volume: number): string {
 ...
 * ——调用方据此落到中性 muted 灰。
 */
export function trendKey(value: number | null | undefined): Trend {
  if (value === null || value === undefined || !Number.isFinite(value) || value === 0) return 'flat'
  return value > 0 ? 'up' : 'down'
}

export function trendColor(value: number | null | undefined, upIsRed = true): string {
...
export function trendTint(value: number | null | undefined, upIsRed = true, alpha = 0.13): string {
  const key = trendKey(value)
...
export function isoDate(timestamp: number | string | Date): string {
...
export function shanghaiYMD(timestamp?: number | string | Date): string {
 ...
 */
export function formatStamp(timestamp: number | null | undefined, locale: string): string {
  if (!timestamp) return '—'
...
export function formatShortStamp(timestamp: number | null | undefined, now?: number): string {
  if (!timestamp) return '—'
  const date = new Date(timestamp)
