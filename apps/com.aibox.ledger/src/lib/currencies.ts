...
type CatalogEntry = { readonly code: string; readonly symbol: string; readonly decimals: number }
const BY_CODE = new Map<string, CatalogEntry>(CURRENCY_CATALOG.map((row) => [row.code, row]))
