...
function MoreButton(props) {
    return (_jsx("button", { type: "button", onClick: props.onClick, style: { border: 'none', background: 'transparent', color: props.palette.accent, fontSize: 17, cursor: 'pointer', width: 44, height: 44 }, "aria-label": "More", children: _jsx(Icon, { name: "ellipsis", size: 17 }) }));
}
function Centered({ children }) {
    return (_jsx("div", { style: { flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 32, textAlign: 'center' }, children: children }));
}
...
function OriginalTab(props) {
    const { palette, t } = props;
    const paragraphs = useMemo(() => (props.transcript?.fullText ?? '').split(/\n+/).map((line) => line.trim()).filter(Boolean), [props.transcript?.fullText]);
    return (_jsxs("div", { style: { display: 'flex', flexDirection: 'column', gap: SPACE.s4 }, children: [props.transcript?.isEdited ? (_jsxs("div", { style: { fontSize: 12, color: palette.muted }, children: [_jsx(Icon, { name: "pencil", size: 11 }), " ", t('edited')] })) : null, props.chaptersBusy ? (_jsx("div", { style: { background: palette.surface, borderRadius: RADIUS.field, padding: SPACE.s4, fontSize: 14, color: palette.muted }, children: t('findingChapters') })) : props.chapters.length ? (_jsxs("div", { style: { background: palette.surface, borderRadius: RADIUS.field, padding: SPACE.s4 }, children: [_jsxs("div", { style: { fontSize: 15, fontWeight: 600, color: palette.accent, marginBottom: SPACE.s2 }, children: [_jsx(Icon, { name: "list", size: 13 }), " ", t('chapters')] }), props.chapters.map((chapter, index) => (_jsxs("button", { type: "button", disabled: !props.hasAudio, onClick: () => props.onSeek(chapter.start), style: {
...
function CorrectedTab(props) {
    ...
    return (_jsxs("div", { style: { display: 'flex', flexDirection: 'column', gap: SPACE.s4 }, children: [_jsxs("div", { style: { display: 'flex', alignItems: 'center', gap: SPACE.s2, flexWrap: 'wrap' }, children: [_jsx(Icon, { name: "person.2", size: 13, color: palette.muted }), _jsxs("select", { value: mode, onChange: (event) => setMode(event.target.value), style: { border: `1px solid ${palette.line}`, borderRadius: 8, padding: '6px 8px', fontSize: 13, background: palette.surface, color: palette.ink }, children: [_jsx("option", { value: "none", children: t('speakerModeNone') }), _jsx("option", { value: "auto", children: t('speakerModeAuto') }), _jsx("option", { value: "named", children: t('speakerModeNamed') })] }), mode === 'named' ? (_jsx("input", { type: "number", min: 2, max: 6, value: count, onChange: (event) => {
                        ...
                        }, style: { width: 56, border: `1px solid ${palette.line}`, borderRadius: 8, padding: '6px 8px', fontSize: 13, background: palette.surface, color: palette.ink } })) : null, _jsx("div", { style: { flex: 1 } }), _jsxs("button", { type: "button", disabled: !context.text.trim(), onClick: run, style: {
                        ...
                        }, children: [_jsx(Icon, { name: artifacts.correctionTurns.length ? 'refresh' : 'sparkles', size: 12 }), ' ', artifacts.correctionTurns.length ? t('recorrectAction') : t('correctAction')] })] }), mode === 'named' ? (_jsx("div", { style: { display: 'flex', flexDirection: 'column', gap: 6 }, children: new Array(count).fill(0).map((_, index) => (_jsxs("div", { style: { display: 'flex', alignItems: 'center', gap: SPACE.s2, background: palette.surface, borderRadius: 8, padding: '6px 10px' }, children: [_jsx("span", { style: { width: 8, height: 8, borderRadius: 4, background: colors[index % colors.length] } }), _jsx("input", { value: names[index] ?? '', onChange: (event) => setNames((current) => {
                                const next = [...current];
                                next[index] = event.target.value;
                                return next;
                            }), placeholder: t('speakerName', { n: index + 1 }), style: { flex: 1, border: 'none', background: 'transparent', fontSize: 14, color: palette.ink, outline: 'none' } })] }, index))) })) : null, busy ? _jsx("div", { style: { fontSize: 14, color: palette.muted, textAlign: 'center' }, children: t('correcting') }) : null, !busy && artifacts.correctionTurns.length ? (_jsxs(_Fragment, { children: [artifacts.correctionStatus === 'stale' ? (_jsxs("div", { style: { fontSize: 12, color: palette.muted }, children: [_jsx(Icon, { name: "warning", size: 11 }), " ", t('staleTranscriptChanged')] })) : null, _jsx("div", { style: { fontSize: 11, color: palette.muted }, children: t('correctionNoTimestamps') }), _jsx("div", { style: { display: 'flex', flexDirection: 'column', gap: SPACE.s3 }, children: artifacts.correctionTurns.map((turn, index) => {
...
function TranslationTab(props) {
    ...
    return (_jsxs("div", { style: { display: 'flex', flexDirection: 'column', gap: SPACE.s4 }, children: [_jsxs("div", { style: { display: 'flex', alignItems: 'center', gap: SPACE.s2, flexWrap: 'wrap' }, children: [_jsx(Icon, { name: "globe", size: 13, color: palette.muted }), _jsx("select", { value: lang, onChange: (event) => setLang(event.target.value), style: { border: `1px solid ${palette.line}`, borderRadius: 8, padding: '6px 8px', fontSize: 13, background: palette.surface, color: palette.ink }, children: TRANSLATION_LANGS.map((code) => _jsx("option", { value: code, children: LANG_NAME[code] }, code)) }), _jsx("div", { style: { flex: 1 } }), _jsxs("button", { type: "button", disabled: !source.trim(), onClick: run, style: {
                            border: 'none', borderRadius: 999, padding: '8px 14px', fontSize: 13, fontWeight: 500,
                            color: palette.onAccent, background: palette.accent, cursor: 'pointer',
...
function ClipPlayer(props) {
    ...
    return (_jsxs("div", { style: { ...dock(SPACE.s4), display: 'flex', flexDirection: 'column', gap: SPACE.s3 }, children: [_jsx(StaticWaveform, { palette: palette, peaks: peaks, progress: duration > 0 ? position / duration : 0 }), _jsx("audio", { ref: audioRef, src: memo.url, preload: "metadata", onTimeUpdate: (event) => setPosition(event.currentTarget.currentTime), onLoadedMetadata: (event) => {
                    ...
                    if (audioRef.current)
                        ...
                        }, "aria-label": playing ? t('pause') : t('play'), children: _jsx(Icon, { name: playing ? 'pause' : 'play', size: 20, color: palette.onAccent }) }), _jsx("button", { type: "button", onClick: () => skip(15), style: iconButton(palette), "aria-label": "+15s", children: _jsx(Icon, { name: "goforward", size: 21 }) }), _jsxs("span", { style: { fontSize: 12, color: palette.muted, minWidth: 38, textAlign: 'right', fontFamily: 'ui-monospace, monospace' }, children: ["-", clockFlat(Math.max(0, duration - position))] })] })] }));
}
/**
