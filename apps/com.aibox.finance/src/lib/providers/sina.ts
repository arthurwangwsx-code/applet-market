...
import type { FXMap, FinSymbol, Quote } from '../types.js'
...
function parseAShare(parts: string[], symbol: FinSymbol): Quote | null {
...
function parseHK(parts: string[], symbol: FinSymbol): Quote | null {
...
function parseUS(parts: string[], symbol: FinSymbol): Quote | null {
 ...
 */
export async function fetchFX(): Promise<FXMap | null> {
  ...
  for (const line of String(result.body).split('\n')) {
    ...
    if (!keyMatch || !bodyMatch) continue
    const pair = keyMatch[1]
    const body = bodyMatch[1]
    if (!pair || body === undefined) continue
...
interface SinaNewsResponse {
  result?: { data?: { feed?: { list?: Array<{ id?: unknown; create_time?: unknown; rich_text?: unknown }> } } }
}

export async function fetchNewsFeed(limit = 20): Promise<NewsFeedItem[]> {
  ...
  const result = await getJSON<SinaNewsResponse>(url)
  if (!result.ok) return []
  const list =
    result.body &&
    result.body.result &&
