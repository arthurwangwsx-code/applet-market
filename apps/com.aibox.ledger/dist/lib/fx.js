...
export function hasUsableRate(currencies, code) {
    ...
    if (!row)
        return String(code ?? '').toUpperCase() === baseCode(currencies);
    if (row.isBase)
        return true;
...
export function toBaseMinor(currencies, minor, code) {
    ...
    if (!row)
        return upper === baseCode(currencies) ? value : 0;
    if (row.isBase)
        return value;
    const rate = Number(row.rateToBase);
    if (!row.rateConfigured || !(rate > 0))
        return 0;
...
export function convertMinor(currencies, minor, from, to) {
    ...
    if (!targetRow)
        return target === baseCode(currencies) ? base : 0;
    if (targetRow.isBase)
        return base;
    const rate = Number(targetRow.rateToBase);
    if (!targetRow.rateConfigured || !(rate > 0))
        return 0;
...
export function reportingBaseMinor(currencies, txn) {
    ...
    if (snapshot !== null && snapshot !== undefined && snapshotCode) {
        ...
        if (String(snapshotCode).toUpperCase() === current)
