...
import type { BacktestStrategy, DCAFrequency } from '../lib/strategy.js'
import type { Candle, FinanceContext, InstrumentPanelRoute } from '../lib/types.js'
...
type StrategyTab = 'backtest' | 'plan'
...
export default function StrategyPage({
  ...
  const [tab, setTab] = React.useState<StrategyTab>('backtest')
  const [horizon, setHorizon] = React.useState<Horizon>('1y')
  const [strategy, setStrategy] = React.useState<BacktestStrategy>('buyhold')
  const [frequency, setFrequency] = React.useState<DCAFrequency>('monthly')
  ...
  return (
    <Sheet visible={visible} onClose={onClose}>
      <SheetHeader
        title={`${name || ''} · ${t('finance.strat.title')}`}
        onClose={onClose}
