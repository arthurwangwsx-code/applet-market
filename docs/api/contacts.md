...
> Find or create contacts through the host Contacts tools.

**分组** 系统能力投影 ｜ **方法数** 2 ｜ **声明要求** 需要在 `manifest.permissions.capabilities` 里声明 `"contacts"`。**声明 ≠ 授权**，用户仍会被逐项询问。
...
| 方法 | 背后的宿主工具 | 档位 |
|---|---|---|
| `find` | `people_find` | `read` |
| `create` | `people_create` | `localWrite` |

### `aibox.contacts.find()`

投影自宿主工具 `people_find`；摘要与参数以该工具的真实定义为准。

**副作用档位** `read`（读取）— 只读，不改任何状态；不触发用户确认。

**参数**

参数取自宿主工具 `people_find` 的真实 `ToolDefinition.parametersJSON`，随宿主版本变化。
...
```js
await aibox.contacts.find({ query: 'Arthur' })
```

### `aibox.contacts.create()`

投影自宿主工具 `people_create`；摘要与参数以该工具的真实定义为准。

**副作用档位** `localWrite`（本地写）— 改本机数据或系统状态；首次使用会弹一次授权。

**参数**

参数取自宿主工具 `people_create` 的真实 `ToolDefinition.parametersJSON`，随宿主版本变化。
写代码前用 `await aibox.capabilities()` 或 `applet_read action=capabilities` 读当前 schema。
...
- **宿主变体**：Full ✅ ／ Lean ⚠️ 取决于壳是否装了通讯录模块
