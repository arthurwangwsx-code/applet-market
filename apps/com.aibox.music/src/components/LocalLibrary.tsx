...
import type { MusicAppContext, MusicTrack, Translate } from '../lib/types.js'

const LIMIT = 500
const ROW_HEIGHT = 60

type LocalMode = 'albums' | 'artists' | 'songs' | 'genres'
type GroupRowData = {
  title: string
  subtitle?: string
  tracks: MusicTrack[]
...
type DerivedGroups = Record<'albums' | 'artists' | 'genres', GroupRowData[]>
type Drill = { title: string; tracks: MusicTrack[] }

export default function LocalLibrary({ ctx }: { ctx: MusicAppContext }) {
  const { t, actions } = ctx
  const [mode, setMode] = React.useState<LocalMode>('albums')
  const [state, setState] = React.useState<{
    loading: boolean
    tracks: MusicTrack[]
    issues: unknown[]
    scanning: boolean
  }>({ loading: true, tracks: [], issues: [], scanning: false })
  const [drill, setDrill] = React.useState<Drill | null>(null)

  const load = React.useCallback(async ({ scan = false }: { scan?: boolean } = {}) => {
    setState((current) => ({ ...current, loading: current.tracks.length === 0, scanning: !!scan }))
    const result = await callMusic<{ tracks?: MusicTrack[]; scanIssues?: unknown[] }>('local', {
  ...
  const playFrom = (list: MusicTrack[], index: number) => {
    const track = list[index]
    if (track) actions.playTrack(track, list)
  }
  ...
  const rows: Array<GroupRowData & MusicTrack & { key: string }> =
    mode === 'songs'
  ...
  return (
    <VirtualList
      ...
      renderRow={(row, index) => {
        ...
        return (
          <GroupRow row={row} mode={mode} t={t} onClick={() => setDrill({ title: row.title, tracks: row.tracks })} />
        )
      }}
      footer={<div style={{ height: 24 }} />}
    />
...
function Toolbar({ t, scanning, onRescan }: { t: Translate; scanning: boolean; onRescan: () => void }) {
  return (
    <div
      style={{
        display: 'flex',
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 2, minWidth: 0, flex: '1 1 auto' }}>
        <span className="mu-clamp-1" style={{ fontSize: 15, color: C.ink }}>
          {row.title}
        </span>
        {subtitle ? (
      ...
      {mode === 'genres' ? (
        <span className="mu-mono" style={{ fontSize: 13, color: C.muted }}>
          {row.tracks.length}
        </span>
      ) : null}
    </button>
...
function derive(tracks: MusicTrack[]): DerivedGroups {
  const albums = new Map<string, GroupRowData>()
  const artists = new Map<string, GroupRowData>()
  const genres = new Map<string, GroupRowData>()
  ...
  const sortByTitle = (a: GroupRowData, b: GroupRowData) => String(a.title).localeCompare(String(b.title))
  return {
