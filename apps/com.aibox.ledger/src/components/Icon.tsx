...
// 这里按记账模块实际用到的符号名手绘一套等价图形（几何近似，非像素级复刻），
// 统一 24×24 viewBox、currentColor。未收录的名字回落到一个中性圆点，绝不渲染成空白。

import React from 'react'

const S: React.SVGProps<SVGPathElement> = {
  fill: 'none',
  stroke: 'currentColor',
  strokeWidth: 1.7,
  strokeLinecap: 'round',
...
const F: React.SVGProps<SVGPathElement> = { fill: 'currentColor', stroke: 'none' }
...
const ICONS = I as unknown as Record<string, readonly [string | null, (string | null)?]>
...
export default function Icon({ name, size = 17, color = 'currentColor', style }: IconProps) {
  const key = ALIASES[name] ?? name
  const shape = ICONS[key]
  const box = { display: 'block', flex: '0 0 auto', color, ...style }
  if (!shape) {
    return (
      <svg viewBox="0 0 24 24" width={size} height={size} style={box} aria-hidden="true">
        <circle cx="12" cy="12" r="4.6" fill="currentColor" opacity="0.55" />
      </svg>
    )
  }
...
export function IconBadge({ name, size = 36, color, background, style }: IconProps & { background?: string }) {
  return (
    <div
      style={{
        width: size,
    ...
    >
      <Icon name={name} size={Math.round(size * 0.42)} color={color} />
    </div>
  )
}
...
export function Spinner({ size = 16, color = 'currentColor' }) {
  return (
    <svg className="lg-spin" viewBox="0 0 24 24" width={size} height={size} style={{ display: 'block', color }}>
