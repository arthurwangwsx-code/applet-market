...
import type { LedgerActionContext } from '../types.js'
 ...
 */
export function registerLedgerActions(context: () => LedgerActionContext): () => void {
  ...
  for (const [name, handler] of Object.entries(ACTION_HANDLERS)) {
    api.action.register(name, async (input) => {
      ...
      const args: LedgerActionArgs =
        input && typeof input === 'object' && !Array.isArray(input) ? (input as LedgerActionArgs) : {}
      // 库还没打开（冷启动 headless 调用）→ 先打开再执行。
      if (store.state === 'unopened') await store.open(locale)
      try {
