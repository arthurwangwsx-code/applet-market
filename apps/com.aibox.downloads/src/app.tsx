...
import type { AddDownloadsInput, DownloadTask, DownloadControlAction, NoticeState } from './types.js'
...
const ACTIVE_STATES: DownloadTask['state'][] = ['queued', 'running', 'paused']
...
function useTasks() {
  ...
  React.useEffect(() => {
    ...
    const boot = async () => {
      ...
      off = onEvent<DownloadTask>('download.progress', (task) => {
        if (!task || !task.taskId) {
          refresh()
          return
        }
...
export default function App() {
  ...
  const [engine, setEngine] = React.useState<{ available: boolean; reason?: string }>({ available: true })
  ...
  const control = React.useCallback(
    async (action: DownloadControlAction, taskId?: string) => {
      tap('light')
        ...
        setTasks((current) =>
          current
            .map((t) => {
              ...
              if (action === 'pause' && ACTIVE_STATES.includes(t.state)) return { ...t, state: 'paused' as const }
              if (action === 'resume' && (t.state === 'paused' || t.state === 'failed'))
                return { ...t, state: 'queued' as const }
              if (action === 'cancel') return { ...t, state: 'cancelled' as const }
              return t
            })
  ...
  const submit = React.useCallback(
    async (request: AddDownloadsInput) => {
      const result = await addDownloads(request)
      setNotice(
        result.ok ? { tone: 'success', text: result.text } : { tone: 'error', text: result.error || '入队失败' },
      )
      if (result.ok) {
        tap('medium')
        setTab('active')
      ...
      if (api && api.tabs && typeof api.tabs.getState === 'function') {
        try {
          ...
          if (!cancelled && state && state.rendered) {
            ...
            if (state.selected === 'active' || state.selected === 'done') setTab(state.selected)
          }
        } catch (error) {
          /* 宿主没这能力：留给自绘 TabBar */
        }
