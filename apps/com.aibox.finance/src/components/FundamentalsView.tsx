...
import type { FinanceContext, FinSymbol } from '../lib/types.js'
...
interface FundamentalsState {
  ...
  flows: Awaited<ReturnType<typeof fetchFundFlow>>
  dividends: Awaited<ReturnType<typeof fetchDividends>>
  announcements: Awaited<ReturnType<typeof fetchAnnouncements>>
}

export default function FundamentalsView({ ctx, symbol }: { ctx: FinanceContext; symbol: FinSymbol }) {
  const { t, settings } = ctx
  const canonical = canonicalOf(symbol)
  const [state, setState] = React.useState<FundamentalsState>({
    loading: true,
    financials: [],
    flows: [],
    dividends: [],
      ...
      {recentFlows.length > 0 ? (
        <Card title={t('finance.flow.title')}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: SPACE.s2, marginBottom: SPACE.s2 }}>
            ...
            <span
              className="fin-mono"
              style={{ fontSize: 15, fontWeight: 500, color: trendColor(cumulative, upIsRed) }}
            >
        ...
        <Card title={t('finance.div.title')}>
          {state.dividends.slice(0, 5).map((row) => (
            <div key={`${row.reportDate}-${row.plan}`} style={{ display: 'flex', gap: SPACE.s2, padding: '4px 0' }}>
              ...
              <span className="fin-clamp-2" style={{ fontSize: 12, color: C.ink }}>
                {row.plan}
              </span>
            </div>
          ))}
