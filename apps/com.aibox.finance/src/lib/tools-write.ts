...
import type { ActionInput } from '@aibox/applet-sdk'
import type { ToolAccountResolution, ToolDependencies, InstrumentResolution } from './tools.js'
import type { Quote } from './types.js'

interface WriteHelpers {
  account: (name?: unknown) => ToolAccountResolution
  resolveInstrument: (input: unknown, options?: { strict?: boolean }) => Promise<InstrumentResolution>
  quoteJSON: (quote: Quote | null | undefined, canonical: string) => unknown
}

/** `deps` = { store, ledger, quotes, alerts }；`helpers` = { account, resolveInstrument, quoteJSON }。 */
export function createWriteHandlers(deps: ToolDependencies, helpers: WriteHelpers) {
  const { store, ledger, quotes, alerts } = deps
  const { account, resolveInstrument, quoteJSON } = helpers

  return {
    async finance_watch(args: ActionInput<'finance_watch'>) {
      ...
      if (action === 'delete_group') {
        ...
        if (!group)
          return { ok: false, error: `No group named "${args.group}".`, groups: store.groups.map((row) => row.name) }
    ...
    async finance_trade(args: ActionInput<'finance_trade'>) {
    ...
    async finance_account(args: ActionInput<'finance_account'>) {
      const action = args.action || 'list'
      if (action === 'list') {
        return {
          ok: true,
      ...
      if (!resolved.ok) return resolved
      if (action === 'rename')
        return ledger.updateAccount(resolved.account.id, { name: String(args.name || '').trim() })
      if (action === 'update')
        return ledger.updateAccount(resolved.account.id, { note: args.note, isRealCopy: !!args.is_real_copy })
    ...
    async finance_alert(args: ActionInput<'finance_alert'>) {
      const action = args.action || 'list'
      if (action === 'list') {
        return {
          ok: true,
