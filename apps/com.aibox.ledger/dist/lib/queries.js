...
export function filterTransactions(store, filter = {}) {
    ...
    return store.allTransactions().filter((txn) => {
        if (kinds && !kinds.has(txn.kind))
            return false;
        if (!includeNonFlow && !countsInFlow(txn))
            return false;
        if (from !== null && txn.occurredOn < from)
            return false;
        if (to !== null && txn.occurredOn >= to)
            return false;
        if (filter.accountID && txn.accountID !== filter.accountID)
            return false;
        if (filter.projectID !== undefined && filter.projectID !== null && txn.projectID !== filter.projectID)
            return false;
...
export function buckets(store, transactions, dimension, metric, locale = 'en', labels = {}) {
    ...
    for (const txn of transactions) {
        ...
        if (metric === 'income' && txn.kind !== KIND.income)
            continue;
        if (metric === 'net' && !countsInFlow(txn))
            continue;
        ...
        switch (dimension) {
            case 'byCategory': {
                ...
                push(rootID ?? UNCATEGORIZED_KEY, category ? category.name : (labels.uncategorized ?? 'Uncategorized'), amount, category ? category.colorHex : null);
                break;
            }
            case 'bySubcategory': {
                const category = txn.categoryID ? store.category(txn.categoryID) : null;
                push(txn.categoryID ?? UNCATEGORIZED_KEY, category ? store.categoryPath(txn.categoryID) : (labels.uncategorized ?? 'Uncategorized'), amount, category ? category.colorHex : null);
                break;
            }
            case 'byAccount': {
                const account = store.account(txn.accountID);
            ...
            case 'byTag': {
                const tags = txn.tags ?? [];
                if (tags.length === 0)
                    push(UNTAGGED_KEY, labels.noTag ?? 'No tag', amount);
                else
                    for (const tag of tags)
                        push(`tag:${String(tag).toLowerCase()}`, String(tag), amount);
                break;
            }
            case 'byDay':
            ...
            case 'byProject': {
                const project = txn.projectID ? store.project(txn.projectID) : null;
                push(txn.projectID ?? NO_PROJECT_KEY, project ? project.name : (labels.noProject ?? 'No project'), amount, project ? project.colorHex : null);
                break;
            }
            default:
                break;
    ...
    if (dimension === 'byDay' || dimension === 'byMonth')
