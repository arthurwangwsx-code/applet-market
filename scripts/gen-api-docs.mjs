...
function extractAdapterDescriptor(src, source) {
  ...
  const decl =
    /(?:^|\n)\s*(?:public\s+|private\s+|internal\s+)?(?:let|var)\s+descriptor\s*(?::\s*AppletCapabilityDescriptor\s*)?(?:=|\{)/.exec(
      src,
    )
  if (!decl) return null
...
function collectRows(schema, prefix, rows, depth) {
  ...
  for (const [key, value] of Object.entries(props)) {
    ...
    if (value.type === 'object' && value.properties) collectRows(value, p, rows, depth + 1)
      ...
      collectRows(value.items, `${p}[]`, rows, depth + 1)
...
function renderParams(parametersJSON, toolName) {
  ...
  try {
    ...
    return '_(schema 无法解析，请检查宿主源码)_'
  }
  ...
  const body = rows.map((r) => `| \`${r.path}\` | ${r.type} | ${r.required ? '✓' : ''} | ${esc(r.desc)} |`).join('\n')
  const strict =
...
function renderNamespace(entry, snap) {
  ...
  out.push('')
  out.push(
    `**分组** ${group.title} ｜ **方法数** ${entry.methods.length} ｜ **声明要求** ${availabilityLine(ns, snap)}`,
  )
  out.push('')
