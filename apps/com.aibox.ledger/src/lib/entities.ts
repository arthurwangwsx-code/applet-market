...
import type { Draft, LedgerStore } from './store.js'
import type { Account, AccountKind, Category, FlowKind, Member, Project } from '../types.js'

const nextOrder = <T extends { sortOrder: number }>(rows: readonly T[]): number =>
...
type AccountPatch = Partial<Pick<Account, 'name' | 'kind' | 'includeInNetWorth' | 'creditLimitMinor' | 'isArchived'>>
...
type CategoryPatch = Partial<Pick<Category, 'name' | 'isArchived' | 'colorHex' | 'systemImage'>>
...
type ProjectPatch = Partial<
  Pick<
    Project,
    'name' | 'note' | 'systemImage' | 'colorHex' | 'startOn' | 'endOn' | 'budgetMinor' | 'isArchived' | 'isActive'
  >
>
...
export async function createAccount(store: LedgerStore, input: AccountInput) {
  const name = String(input.name ?? '').trim()
  if (name.length === 0) return { ok: false, reason: 'emptyName' }
...
export async function updateAccount(store: LedgerStore, id: string, patch: AccountPatch) {
  const ok = await store.mutate((draft) => {
    ...
    if (patch.includeInNetWorth !== undefined) next.includeInNetWorth = !!patch.includeInNetWorth
    if (patch.creditLimitMinor !== undefined) next.creditLimitMinor = Math.max(0, Math.round(patch.creditLimitMinor))
...
export function archiveAccount(store: LedgerStore, id: string, archived = true) {
  return updateAccount(store, id, { isArchived: archived })
}

// MARK: - 分类

export async function createCategory(store: LedgerStore, input: CategoryInput) {
  const name = String(input.name ?? '').trim()
  if (name.length === 0) return { ok: false, reason: 'emptyName' }
  const kind: FlowKind = input.kind === 'income' ? 'income' : 'expense'
...
export async function updateCategory(store: LedgerStore, id: string, patch: CategoryPatch) {
  const ok = await store.mutate((draft) => {
    ...
    if (patch.name !== undefined) next.name = String(patch.name).trim()
    if (patch.isArchived !== undefined) next.isArchived = !!patch.isArchived
    if (patch.colorHex !== undefined) next.colorHex = patch.colorHex
    if (patch.systemImage !== undefined) next.systemImage = patch.systemImage
    rows[index] = next
    return true
  })
  return ok ? { ok: true } : { ok: false, reason: 'persistence' }
...
function ensureCurrency(draft: Draft, code: string, rate?: number) {
...
export async function addCurrency(store: LedgerStore, code: string, rate?: number) {
  const upper = String(code ?? '').toUpperCase()
  if (upper.length !== 3) return { ok: false, reason: 'invalidCode' }
  if (store.currencyRow(upper)) return { ok: true, duplicate: true }
  const ok = await store.mutate((draft) => {
...
export async function setRate(store: LedgerStore, code: string, rate: number) {
  const value = Number(rate)
  if (!Number.isFinite(value) || value <= 0) return { ok: false, reason: 'invalidRate' }
  const ok = await store.mutate((draft) => {
    const rows = draft.table('currencies')
 ...
 */
export async function setBaseCurrency(store: LedgerStore, code: string) {
  const upper = String(code ?? '').toUpperCase()
  const target = store.currencyRow(upper)
  if (!target) return { ok: false, reason: 'unknownCurrency' }
  if (target.isBase) return { ok: true, noop: true }
  const divisor = Number(target.rateToBase)
  if (!target.rateConfigured || !(divisor > 0)) return { ok: false, reason: 'rateNeeded' }
  const ok = await store.mutate((draft) => {
    const rows = draft.table('currencies')
 ...
 */
export async function applyFetchedRates(store: LedgerStore, rates: Record<string, number> | null) {
  if (!rates) return { ok: false, reason: 'unavailable' }
  const updates: Array<{ code: string; rateToBase: number }> = []
  for (const row of store.currencies) {
    if (row.isBase || row.manualRate) continue
    const inverse = Number(rates[row.code])
    if (!Number.isFinite(inverse) || inverse <= 0) continue
    const next = 1 / inverse
    if (row.rateConfigured && Math.abs(next - Number(row.rateToBase)) < 1e-12) continue
...
export async function createProject(store: LedgerStore, input: ProjectInput) {
  const name = String(input.name ?? '').trim()
  if (name.length === 0) return { ok: false, reason: 'emptyName' }
  const project = {
    id: newID(),
  ...
  const ok = await store.mutate((draft) => {
    ...
    if (project.isActive) {
      for (let i = 0; i < rows.length; i += 1) {
        const row = rows[i]
        if (row?.isActive) rows[i] = { ...row, isActive: false }
      }
    }
    rows.push(project)
  })
...
export async function updateProject(store: LedgerStore, id: string, patch: ProjectPatch) {
  const ok = await store.mutate((draft) => {
    const rows = draft.table('projects')
    const index = rows.findIndex((row) => row.id === id)
    if (index < 0) return false
    ...
    if (patch.systemImage !== undefined) next.systemImage = patch.systemImage
    if (patch.colorHex !== undefined) next.colorHex = patch.colorHex
    if (patch.startOn !== undefined) next.startOn = patch.startOn === null ? null : dayStart(patch.startOn)
    if (patch.endOn !== undefined) next.endOn = patch.endOn === null ? null : dayStart(patch.endOn)
    if (patch.budgetMinor !== undefined) next.budgetMinor = Math.max(0, Math.round(patch.budgetMinor))
    if (patch.isArchived !== undefined) next.isArchived = !!patch.isArchived
    if (patch.isActive !== undefined) {
      ...
      if (next.isActive) {
        for (let i = 0; i < rows.length; i += 1) {
          const row = rows[i]
          if (row && row.id !== id && row.isActive) rows[i] = { ...row, isActive: false }
        }
      }
    }
...
/** 设为当前项目；`null` 清空。 */
export async function activateProject(store: LedgerStore, id: string | null) {
  const ok = await store.mutate((draft) => {
    const rows = draft.table('projects')
    for (let i = 0; i < rows.length; i += 1) {
      const row = rows[i]
      ...
      if (row.isActive !== shouldBeActive) rows[i] = { ...row, isActive: shouldBeActive }
    }
    return true
  })
  return ok ? { ok: true } : { ok: false, reason: 'persistence' }
...
export async function addMember(store: LedgerStore, projectID: string, input: MemberInput) {
  const name = String(input.name ?? '').trim()
  if (name.length === 0) return { ok: false, reason: 'emptyName' }
  ...
  const ok = await store.mutate((draft) => {
    ...
    if (member.isMe) {
      for (let i = 0; i < rows.length; i += 1) {
        const row = rows[i]
        if (row && row.projectID === projectID && row.isMe) rows[i] = { ...row, isMe: false }
      }
    }
    rows.push(member)
  })
...
export async function updateMember(
  store: LedgerStore,
  id: string,
  patch: Partial<Pick<Member, 'name' | 'colorHex' | 'isMe'>>,
) {
  const ok = await store.mutate((draft) => {
    const rows = draft.table('members')
    const index = rows.findIndex((row) => row.id === id)
 ...
 */
export async function removeMember(store: LedgerStore, id: string) {
  ...
  const ok = await store.mutate((draft) => {
    for (const txn of affected) {
      const next = { ...txn }
      if (next.payerMemberID === id) next.payerMemberID = null
      if (next.split) {
