...
/**
 * 计时器 —— TypeScript + Vite 标准工程的完整范例（AI fork 的起点）。
 *
 * 这个应用刻意示范了小应用交付里最容易漏的四件事：
 *  ① **入口是 `export default function App`**，由 `index.html` 的 `createRoot` 挂载。
    ...
    <div style={{ minHeight: '100dvh', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 24, padding: '32px 16px 0' }}>
        <Dial
          ...
          label={running ? running.label : '准备开始'}
        ...
        {!isAvailable('haptics') && (
          <div className="ax-muted" style={{ fontSize: 12 }}>
            这台设备没有触觉反馈，计时结束只有视觉提示。
          </div>
        )}
      </div>

      <div style={{ marginTop: 24, paddingBottom: safeBottom + keyboard.height }}>
