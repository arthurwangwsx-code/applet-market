...
function useReceiptScan({ t, setInput, setMerchant, setDay, amountFilled, merchantFilled, accountCurrency, }) {
    ...
    const scan = React.useCallback(async () => {
        ...
        try {
            ...
            if (!res.ok) {
                setScanHint(res.reason === 'cancelled' ? '' : res.reason === 'noVision' ? t('entry.scanNoVision') : t('entry.scanFailed'));
                return;
            }
            if (res.empty) {
            ...
            if (parsed.currency && accountCurrency && parsed.currency !== accountCurrency) {
                notes.push(t('entry.scanCurrencyMismatch').replace('{found}', parsed.currency).replace('{account}', accountCurrency));
            }
...
export default function EntryEditor({ ctx, editing, onClose, }) {
    ...
    const transferRateMissing = type === 'transfer' &&
        fromAccount &&
        toAccount &&
    ...
    return (_jsxs("div", { style: { display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0 }, children: [_jsx("div", { style: { padding: `${SPACE.s3}px ${SPACE.s4}px 0` }, children: _jsx(Segmented, { value: type, onChange: changeType, disabled: isEditing, items: [
                        ...
                        { id: 'transfer', label: t('x.transfer') },
                                    ...
                                    color: C.muted,
                                    lineHeight: 1.5,
                                }, children: receipt.scanHint })) : null, _jsx(FieldCard, { icon: "calendar", label: t('x.date'), children: _jsx("input", { className: "lg-field", type: "date", style: { textAlign: 'right', fontSize: 15 }, value: day, onChange: (event) => setDay(event.target.value) }) }), _jsx(FieldCard, { icon: "text.alignleft", label: t('x.note'), children: _jsx("input", { className: "lg-field", style: { textAlign: 'right', fontSize: 15 }, value: note, onChange: (event) => setNote(event.target.value) }) }), type !== 'transfer' ? (_jsxs(_Fragment, { children: [_jsxs("button", { type: "button", className: "lg-btn", onClick: () => setExpanded((value) => !value), style: {
                                            display: 'flex',
                                            alignItems: 'center',
                                            gap: 6,
...
function refundLabel(store, id, t) {
    ...
    const name = txn.merchant || (txn.categoryID ? store.category(txn.categoryID)?.name : null) || txn.note || t('x.expense');
