...
export function ReviewPage(props) {
    ...
    return (_jsx(PushPage, { palette: palette, title: t('reviewTitle'), onBack: props.onBack, children: _jsxs("div", { style: { display: 'flex', flexDirection: 'column', minHeight: '100%', padding: `${SPACE.s4}px 0 ${SPACE.s5}px` }, children: [_jsxs("div", { style: { padding: `0 ${SPACE.s5}px` }, children: [_jsx("div", { style: { height: 4, borderRadius: 2, background: palette.line, overflow: 'hidden' }, children: _jsx("div", { style: { width: `${(index / queue.length) * 100}%`, height: '100%', background: palette.accent } }) }), _jsxs("div", { style: { fontSize: 12, color: palette.muted, marginTop: 6 }, children: [index + 1, " / ", queue.length] })] }), _jsx("div", { style: { flex: 1 } }), _jsx("div", { style: {
                        ...
                        display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: SPACE.s3,
                    }, children: !flipped ? (_jsxs(_Fragment, { children: [_jsxs("div", { style: { display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 12, fontWeight: 500, color: palette.accent }, children: [_jsx(Icon, { name: kindIcon(exercise.kind), size: 12 }), " ", kindLabel(t, exercise.kind)] }), exercise.kind === 'listening' ? (_jsxs(_Fragment, { children: [_jsx("button", { type: "button", onClick: () => void speak(current.text, 'us'), style: { border: 'none', background: 'transparent', color: palette.accent, fontSize: 52, cursor: 'pointer', lineHeight: 1 }, "aria-label": t('speakAloud'), children: _jsx(Icon, { name: "speaker", size: 52 }) }), _jsx("div", { style: { fontSize: 14, color: palette.muted, textAlign: 'center' }, children: t('listeningHint') })] })) : (_jsx("div", { style: { fontSize: exercise.kind === 'cloze' ? 18 : 16, fontWeight: 500, color: palette.ink, textAlign: 'center' }, children: exercise.prompt ?? t('promptFallback') })), _jsx("input", { value: answer, onChange: (event) => setAnswer(event.target.value), onKeyDown: (event) => {
                                ...
                                } })] })) : (_jsxs(_Fragment, { children: [verdict !== null ? (_jsxs("div", { style: { fontSize: 13, fontWeight: 500, color: verdict ? palette.green : palette.orange }, children: [_jsx(Icon, { name: verdict ? 'check' : 'refresh', size: 13 }), " ", verdict ? t('answerCorrect') : t('answerKeepPracticing')] })) : null, _jsxs("div", { style: { display: 'flex', alignItems: 'center', gap: SPACE.s2 }, children: [_jsx("span", { style: { fontSize: 28, fontWeight: 500, color: palette.ink }, children: current.text }), _jsx("button", { type: "button", onClick: () => void speak(current.text, 'us'), style: { border: 'none', background: 'transparent', color: palette.accent, cursor: 'pointer' }, "aria-label": t('speakAloud'), children: _jsx(Icon, { name: "speaker", size: 18 }) })] }), entry?.phoneticUK ? _jsxs("div", { style: { fontSize: 14, color: palette.muted }, children: ["/", entry.phoneticUK, "/"] }) : null, _jsx("div", { style: { height: 1, background: palette.line, width: '100%' } }), current.brief ? _jsx("div", { style: { fontSize: 16, color: palette.ink, textAlign: 'center' }, children: current.brief }) : null, entry?.examTags.length ? (_jsx("div", { style: { display: 'flex', flexWrap: 'wrap', gap: 6, justifyContent: 'center' }, children: entry.examTags.map((tag) => (_jsx("span", { style: { fontSize: 12, color: palette.muted, background: palette.line, borderRadius: 999, padding: '4px 9px' }, children: tag }, tag))) })) : null, current.note ? (_jsxs("div", { style: { width: '100%', background: alpha(palette.accent, 0.08), borderRadius: RADIUS.field, padding: SPACE.s3 }, children: [_jsx("div", { style: { fontSize: 11, fontWeight: 500, color: palette.muted }, children: t('savedContext') }), _jsx("div", { style: { fontSize: 14, color: palette.ink, marginTop: 4 }, children: current.note })] })) : null, current.kind === 'word' ? (_jsx("button", { type: "button", onClick: () => props.onOpenWord(current.text), style: { border: 'none', background: 'transparent', color: palette.accent, fontSize: 12, cursor: 'pointer' }, children: t('viewFullDetail') })) : null] })) }), _jsx("div", { style: { flex: 1 } }), !flipped ? (_jsxs("div", { style: { padding: `0 ${SPACE.s5}px`, display: 'flex', flexDirection: 'column', gap: SPACE.s2 }, children: [_jsx(PrimaryButton, { palette: palette, title: t('checkAnswer'), block: true, disabled: !answer.trim(), onClick: () => {
                                setVerdict(isCorrect(answer, exercise.answer));
                                setFlipped(true);
                            } }), _jsx("button", { type: "button", onClick: () => {
                                setVerdict(null);
                                setFlipped(true);
                            }, style: { border: 'none', background: 'transparent', color: palette.muted, fontSize: 14, padding: 8, cursor: 'pointer' }, children: t('showAnswer') })] })) : (_jsxs("div", { style: { padding: `0 ${SPACE.s5}px`, display: 'flex', gap: SPACE.s3 }, children: [_jsx(GradeButton, { palette: palette, label: t('gradeForgot'), tint: palette.red, onClick: () => void grade('forgot') }), _jsx(GradeButton, { palette: palette, label: t('gradeFuzzy'), tint: palette.orange, onClick: () => void grade('fuzzy') }), _jsx(GradeButton, { palette: palette, label: t('gradeKnow'), tint: palette.green, onClick: () => void grade('know') })] }))] }) }));
}
...
function Centered({ children }) {
    return (_jsx("div", { style: { display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 40, textAlign: 'center' }, children: children }));
}
