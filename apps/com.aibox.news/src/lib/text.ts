...
export type Hash64 = [number, number]

/** [hi, lo] = [hi, lo] * [pHi, pLo]，模 2^64。 */
function mul64(hi: number, lo: number, pHi: number, pLo: number): Hash64 {
  const l0 = lo & 0xffff
  const l1 = lo >>> 16
  const p0 = pLo & 0xffff
  const p1 = pLo >>> 16
...
export function fnv1a64(text: string): Hash64 {
...
function legacyUTF8(text: string): number[] {
  const out: number[] = []
...
function decodeEntity(entity: string): string | null {
  if (Object.prototype.hasOwnProperty.call(ENTITIES, entity)) return ENTITIES[entity] ?? null
...
export function plain(html: unknown) {
...
export function firstImageURL(html: unknown): string | null {
  const source = String(html == null ? '' : html)
...
export function simhash(text: string): Hash64 {
  ...
  for (const token of tokens) {
    ...
    for (let i = 0; i < 32; i += 1) votes[i] = (votes[i] ?? 0) + (((lo >>> i) & 1) === 1 ? 1 : -1)
    for (let i = 0; i < 32; i += 1) votes[i + 32] = (votes[i + 32] ?? 0) + (((hi >>> i) & 1) === 1 ? 1 : -1)
  }
  let lo = 0
  let hi = 0
  for (let i = 0; i < 32; i += 1) if ((votes[i] ?? 0) > 0) lo = (lo | (1 << i)) >>> 0
  for (let i = 0; i < 32; i += 1) if ((votes[i + 32] ?? 0) > 0) hi = (hi | (1 << i)) >>> 0
  return [hi, lo]
}

export function isZeroHash(pair: Hash64) {
...
export function hamming(a: Hash64, b: Hash64) {
...
export function stableKey(url: string, guid: string, title: string) {
