 ...
 * 否则原样显示（用户自建名）。
 */
export function groupLabel(t: Translate, name: string | null | undefined): string {
  if (!name) return ''
  if (name === 'finance.watch.group.all' || name.startsWith('group.')) return t(name)
  return name
}

/** 账户名同理：种子账户名是键，用户改过的是字面量。 */
export function accountLabel(t: Translate, name: string): string {
  if (name === 'account.default.name') return t(name)
  return name
}
