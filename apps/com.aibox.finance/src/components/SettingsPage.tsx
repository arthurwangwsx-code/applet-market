...
import type { FinanceContext, FinanceSettings, QuoteSource } from '../lib/types.js'

const INTERVALS = [15, 30, 60, 120]
const SOURCES: QuoteSource[] = ['automatic', 'tencent', 'sina']

export default function SettingsPage({ ctx }: { ctx: FinanceContext }) {
  const { t, store, settings, actions } = ctx
  const set = (patch: Partial<FinanceSettings>) => store.updateSettings(patch)
...
/** 自选分组管理（设置 → 管理 → 自选分组）。 */
export function GroupsSheet({ ctx, visible, onClose }: { ctx: FinanceContext; visible: boolean; onClose: () => void }) {
  const { t, store } = ctx
