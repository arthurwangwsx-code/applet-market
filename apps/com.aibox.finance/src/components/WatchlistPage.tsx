...
import type { DataState } from '../lib/quotes.js'
...
import type { FinanceContext, Quote, Translate, WatchItem } from '../lib/types.js'
import Icon from './Icon.js'
import { Chip, ChipRow, EmptyState, Menu, PullRefresh, Spinner, SwipeRow } from './primitives.js'
import { C, RADIUS, SPACE } from './theme.js'

const SORTS = ['manual', 'changeDescending', 'changeAscending'] as const
type WatchlistSort = (typeof SORTS)[number]

type WatchlistRow = { kind: 'header'; id: string; label: string } | { kind: 'item'; id: string; item: WatchItem }

interface IndexCardProps {
  row: (typeof INDEX_ROWS)[number]
  quote: Quote | null
  t: Translate
  upIsRed: boolean
...
function IndexCard({ row, quote, t, upIsRed, onOpen }: IndexCardProps) {
  return (
    <button
      type="button"
    ...
    >
      ...
      <span
        className="fin-mono"
        style={{ fontSize: 12, color: quote ? trendColor(quote.changePct, upIsRed) : 'transparent' }}
      >
...
/** 自选行。现价中性、pill 半透明。 */
function WatchRow({ canonical, name, quote, upIsRed, onOpen }: WatchRowProps) {
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: 2, minWidth: 0, flex: '1 1 auto' }}>
        <span className="fin-clamp-1" style={{ fontSize: 16, fontWeight: 500, color: C.ink }}>
          {name}
        </span>
...
/** 页脚「更新于…」。三行互斥语义见 §2.6。 */
function Footer({ ctx, state }: { ctx: FinanceContext; state: DataState }) {
...
export default function WatchlistPage({ ctx }: { ctx: FinanceContext }) {
  const { t, store, quotes, settings, actions } = ctx
  const scrollRef = React.useRef<HTMLDivElement>(null)
  const [group, setGroup] = React.useState<string | null>(null) // null = 全部
  const [sort, setSort] = React.useState<WatchlistSort>('manual')
  ...
  const sortRows = React.useCallback(
    ...
    [sort, quotes, ctx.quoteVersion],
  ) // eslint-disable-line react-hooks/exhaustive-deps

  // 扁平化成 VirtualList 的行（分组头也是一行，让长自选也能虚拟滚动）。
  const rows = React.useMemo<WatchlistRow[]>(() => {
    ...
    const orphans = sortRows(store.items.filter((item) => !store.groups.some((row) => row.id === item.groupID)))
  ...
  return (
    ...
    >
      <VirtualList
        ...
        header={header}
        footer={
          quotes.lastUpdated || ctx.refreshing || quotes.lastFailed ? (
            <Footer ctx={ctx} state={dataState} />
          ) : (
