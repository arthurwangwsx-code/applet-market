...
export function clockFlat(seconds: number): string {
  const total = Math.floor(safeSeconds(seconds))
  return `${Math.floor(total / 60)}:${String(total % 60).padStart(2, '0')}`
}

/** 录音面板计时：`m:ss.cc`（百分秒）。 */
export function clockCentis(ms: number): string {
...
export function clockPadded(seconds: number): string {
  const total = Math.floor(safeSeconds(seconds))
  const h = Math.floor(total / 3600)
  const m = Math.floor((total % 3600) / 60)
  const s = total % 60
...
export function mediumDateTime(ms: number, locale: string): string {
  try {
 ...
 */
export function defaultTitle(prefix: string, locale: string, at = new Date()): string {
  let stamp: string
  try {
...
export function exportMarkdown(input: ExportInput): string {
  const lines: string[] = [`# ${input.memo.title}`, '']
 ...
 */
export function exportSRT(input: ExportInput): string {
 ...
 * 两端截断处加 `…`。
 */
export function snippetOf(text: string, query: string): string | undefined {
