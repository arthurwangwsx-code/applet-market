...
import type { LyricsPayload, MusicStatus, MusicTrack, RGBColor, Translate } from '../lib/types.js'
...
export default function NowPlaying({
  ...
  mode: 'album' | 'lyrics'
  onSetMode: (mode: 'album' | 'lyrics') => void
  isFavorite: boolean
  onToggleFavorite: () => void
  onOpenTrackMenu: () => void
  scrub: { begin: (ratio: number) => void; update: (ratio: number) => void; end: () => void }
  onPrevious: () => void
  onNext: () => void
  onTogglePlay: () => void
      ...
      >
        {mode === 'lyrics' ? (
          ...
          <>
            ...
            <CoverArea url={artworkURL} playing={status.isPlaying} onTap={toggleMode} onLyrics={toggleMode} />
...
function Ambient({ url, color, dim }: { url: string | null; color: RGBColor | null; dim: boolean }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, flex: '0 0 auto' }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 3, minWidth: 0, flex: '1 1 auto' }}>
        ...
        {track && track.artist ? (
          <span className="mu-clamp-1" style={{ fontSize: 17, color: WHITE.secondary }}>
            {track.artist}
          </span>
        ) : null}
      </div>
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: '0 0 auto', paddingBottom: 8 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 2, minWidth: 0, flex: '1 1 auto', marginLeft: 4 }}>
        ...
        {track && track.artist ? (
          <span className="mu-clamp-1" style={{ fontSize: 13, color: WHITE.secondary }}>
            {track.artist}
          </span>
        ) : null}
      </div>
...
function FailureCard({ status, onRetry, t }: { status: MusicStatus; onRetry: () => void; t: Translate }) {
...
function reasonText(lastError: string | null, t: Translate): string {
