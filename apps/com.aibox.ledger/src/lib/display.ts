...
import type { LedgerTransaction, Translate } from '../types.js'
import type { LedgerStore } from './store.js'

export const COLOR_TOKEN = { ink: 'ink', muted: 'muted', income: 'income', expense: 'expense' } as const

/** 明细行右侧金额。返回 `{ text, tone }`，tone 由页面映射到 CSS 变量。 */
export function entryAmount(txn: LedgerTransaction) {
  switch (txn.kind) {
    case KIND.income:
...
export function entryGlyph(store: LedgerStore, txn: LedgerTransaction) {
...
export function kindFallbackTitle(store: LedgerStore, txn: LedgerTransaction, t: Translate): string {
  switch (txn.kind) {
    case KIND.transferOut:
    case KIND.transferIn: {
...
/** 主标题：商家 → 分类名 → kind 兜底。 */
export function entryTitle(store: LedgerStore, txn: LedgerTransaction, t: Translate): string {
...
export function entrySubtitle(store: LedgerStore, txn: LedgerTransaction): string {
  const parts: string[] = []
...
export function entryPathTitle(store: LedgerStore, txn: LedgerTransaction, t: Translate): string {
  if (txn.categoryID) {
...
export function entryConversion(store: LedgerStore, txn: LedgerTransaction, t: Translate) {
...
export function dayHeaderTitle(day: number, locale: string, t: Translate, now = Date.now()): string {
...
export function dayExpenseTotal(store: LedgerStore, rows: readonly LedgerTransaction[]): string | null {
  let total = 0
