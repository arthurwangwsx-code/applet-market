...
import type { MusicCall, MusicTrack, RGBColor } from './types.js'
...
export async function artworkDataURL(url: unknown): Promise<string | null> {
  ...
  const task = Promise.resolve(imageURL(key, { width: 300 }))
    .then((value) => {
      ...
      if (dataURLCache.size >= MAX_CACHE && oldest !== undefined) dataURLCache.delete(oldest)
...
export function sizedArtworkURL(url: unknown, size: number): string {
 ...
 */
export async function dominantColor(dataURL: unknown): Promise<RGBColor | null> {
  ...
  try {
    ...
    if (colorCache.size >= MAX_CACHE && oldest !== undefined) colorCache.delete(oldest)
...
function averageColor(pixels: Uint8ClampedArray): RGBColor | null {
  let r = 0
  let g = 0
  let b = 0
  let count = 0
...
function boost(color: RGBColor): RGBColor {
  const max = Math.max(color.r, color.g, color.b)
  const min = Math.min(color.r, color.g, color.b)
  const saturation = max === 0 ? 0 : (max - min) / max
  // 太灰的封面（黑白照）直接原样返回，硬拉饱和度只会出脏色。
  if (saturation < 0.08 || max === 0) return color
...
export function rgba(color: RGBColor | null | undefined, alpha: number): string {
  if (!color) return `rgba(255,107,107,${alpha})`
...
export async function backfillArtworkURL(
  ...
  if (!track || !track.musicItemId) return null
  const key = stableKey(track)
  if (!key || backfilled.has(key)) return store.artworkURL(track)
  backfilled.add(key)
  const query = [track.title, track.artist].filter(Boolean).join(' ')
  if (!query) return null
  const result = await music<{ songs?: MusicTrack[] }>('search', { query, types: ['song'], limit: 5 })
  const songs = result.json && Array.isArray(result.json.songs) ? result.json.songs : []
  const hit =
    songs.find((row) => String(row.musicItemId) === String(track.musicItemId)) ||
