...
import type { MusicAppContext, OptionRequest, RepeatMode, TabID } from '../lib/types.js'
...
type Submenu = { title: string; items: MenuItem[] }
...
export default function OptionsMenu({
  ...
  const close = (run?: () => void | Promise<void>) => {
    onClose()
    if (run) void run()
  }
                ...
                <>
                  ...
                  <MenuRow
                    ...
                    onClick={() =>
                      setSubmenu({
                        title: t('menu.repeat'),
                        items: (['off', 'one', 'all'] as RepeatMode[]).map((mode) => ({
                          id: mode,
...
/** 通用单选面板（设置页的循环模式等复用它）。 */
export function OptionPicker({ request, onClose }: { request: OptionRequest | null; onClose: () => void }) {
