...
import type { DownloadControlAction, DownloadFilterState, DownloadRequest, DownloadTask } from '../types.js'
...
function notifyFakeListeners() {
  for (const fn of fakeListeners) fn()
}

const memory = {
  enqueue({ url, filename }: DownloadRequest) {
    fakeSeq += 1
  ...
  control(action: DownloadControlAction, taskId?: string) {
    const apply = (t: DownloadTask) => {
    ...
    if (action === 'clearFinished' || (action === 'remove' && !taskId)) {
      const keep = fakeTasks.filter((t) =>
        action === 'clearFinished' ? !['completed', 'failed', 'cancelled'].includes(t.state) : false,
      )
      fakeTasks.length = 0
      fakeTasks.push(...keep)
    } else if (taskId) {
...
export const downloads = {
  ...
  async enqueue(request: DownloadRequest) {
    const api = bridge()
    if (!api || !capabilities.download) return memory.enqueue(request)
    try {
  ...
  async list(filter?: { state?: DownloadFilterState }): Promise<DownloadTask[]> {
    const api = bridge()
    ...
    try {
      ...
      return Array.isArray(items) ? (items as DownloadTask[]) : []
    } catch (error) {
      return []
    }
  },
  async control(action: DownloadControlAction, taskId?: string): Promise<boolean> {
    const api = bridge()
    if (!api || !capabilities.download) return memory.control(action, taskId)
    try {
      if (taskId) {
        if (action === 'clearFinished') return false
        if (action === 'pause') await api.download.pause({ taskId })
        else if (action === 'resume') await api.download.resume({ taskId })
        else if (action === 'cancel') await api.download.cancel({ taskId })
        else if (action === 'remove') await api.download.remove({ taskId })
        else return false
        return true
      }
      if (action === 'pause') await api.download.pauseAll({})
      else if (action === 'resume') await api.download.resumeAll({})
      else if (action === 'cancel') await api.download.cancelAll({})
      else if (action === 'clearFinished') await api.download.clearFinished({})
      else return false
      return true
    } catch (error) {
      return false
  ...
  async subscribe() {
    const api = bridge()
    if (!api || !capabilities.download) return false
    try {
  ...
  async openIn(taskId: string): Promise<boolean> {
    const api = bridge()
    if (!api || !capabilities.download) return false
    try {
  ...
  async share(taskId: string): Promise<boolean> {
    const api = bridge()
    if (!api || !capabilities.download) return false
    try {
  ...
  async availability(): Promise<{ available: boolean; reason?: string }> {
    const api = bridge()
    if (!api || !capabilities.download) return { available: false, reason: 'preview' }
    try {
...
export function matchesState(filter: string, state: string): boolean {
...
export function tap(style: 'light' | 'medium' | 'heavy' | 'soft' | 'rigid' = 'light'): void {
  const api = bridge()
  if (!api || !capabilities.haptics) return
  try {
...
export const storage = {
  ...
  async set(key: string, value: aibox.JSONValue): Promise<boolean> {
    const api = bridge()
